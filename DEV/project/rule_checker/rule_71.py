...
def detection_range_programwise(device_in_program_df:pd.DataFrame, program_name:str, program_comment_data:dict):
    match_outcoil_dict = {}
    T_coil_found = False
    ...
    if not device_in_program_df.empty:
        contact_coil_df = device_in_program_df[
                device_in_program_df['OBJECT_TYPE_LIST'].str.lower().isin(['contact', 'coil'])
            ]
        if not contact_coil_df.empty:
            for _, contact_coil_row in contact_coil_df.iterrows():
                ...
                if isinstance(contact_coil_operand, str) and contact_coil_operand:
                    if (contact_coil_row['OBJECT_TYPE_LIST'].lower() == 'coil' and not B_contact_found) or not contact_coil_operand.startswith("T"):
                        T_coil_found = False
                        B_contact_found = False
                    
                    if contact_coil_row['OBJECT_TYPE_LIST'].lower() == 'coil' and contact_coil_operand.startswith("T"):
                        T_coil_found = True
                        T_coil_operand = contact_coil_operand

                    if contact_coil_row['OBJECT_TYPE_LIST'].lower() == 'contact' and negated_operand == 'true':
                ...
                if T_coil_found and B_contact_found:
                    match_outcoil_dict[index] = {"B_contact" : B_contact_operand, "T_coil" : T_coil_operand, "rung_number":rung_number}
                    index+=1
...
def detection_range_functionwise(device_in_function_df:pd.DataFrame, function_name:str, function_comment_data:dict):
    match_outcoil_dict = {}
    T_coil_found = False
    B_contact_found = False
    B_contact_operand = ""
    ...
    if not device_in_function_df.empty:
        contact_coil_df = device_in_function_df[
                device_in_function_df['OBJECT_TYPE_LIST'].str.lower().isin(['contact', 'coil'])
            ]
        if not contact_coil_df.empty:
            for _, contact_coil_row in contact_coil_df.iterrows():
                attr = ast.literal_eval(contact_coil_row['ATTRIBUTES'])
...
def execute_rule_71_programwise(input_program_file:str, 
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            if is_task_name_in_range:
                ...
                device_in_program_df = curr_program_df[curr_program_df['BODY'].str.lower() == devicein_section_name]
                detection_results = detection_range_programwise(device_in_program_df=device_in_program_df, program_name=program, program_comment_data=program_comment_data)
                if detection_results:
                    for k,v in detection_results.items():
                        ...
                        status = "NG"
                        ng_name = ng_content
                        match_rung_df = device_in_program_df[device_in_program_df['RUNG'] == v['rung_number']]
                        get_series_connect_data = get_series_contacts(pl.from_pandas(match_rung_df))
                        ...
                        for series_contact in series_contact_operands_only:
                            ...
                            if both_coil_contact_present:
                                for series_contact_operand in series_contact:
                                    ...
                                    if isinstance(series_contact_operand, str) and series_contact_operand:
                                        comment = get_the_comment_from_program(series_contact_operand, program, program_comment_data)
                                        if (regex_pattern_check(communication_comment, comment) or regex_pattern_check(network_comment, comment)) and regex_pattern_check(normal_comment, comment):
                                            status = "OK"
                                            ng_name = ""
                                            break
                        ...
                        rung_number = v.get('rung_number') - 1 if v.get('rung_number') !=-1 else -1
                        output_rows.append({
                            "Result": status,
                            "Task": program,
                            "Section": "DeviceIn",
...
def execute_rule_71_functionwise(input_function_file:str, 
    ...
    try:
        ...
        for function in unique_function_values:
            ...
            if is_task_name_in_range:
                ...
                device_in_function_df = curr_function_df[curr_function_df['BODY_TYPE'].str.lower() == devicein_section_name]
                detection_results = detection_range_functionwise(device_in_function_df=device_in_function_df, function_name=function, function_comment_data=function_comment_data)
                if detection_results:
                    for k,v in detection_results.items():
                        T_coil = v['T_coil']
                        B_contact = v["B_contact"]
                        status = "NG"
                        ng_name = ng_content
                        match_rung_df = device_in_function_df[device_in_function_df['RUNG'] == v['rung_number']]
                        get_series_connect_data = get_series_contacts(pl.from_pandas(match_rung_df))
                        series_contact_operands_only = [[item.get('operand') for item in sublist] for sublist in get_series_connect_data]
                        for series_contact in series_contact_operands_only:
                            both_coil_contact_present = B_contact in series_contact
                            if both_coil_contact_present:
                                for series_contact_operand in series_contact:
                                    ...
                                    if isinstance(series_contact_operand, str) and series_contact_operand:
                                        comment = get_the_comment_from_function(series_contact_operand, function, function_comment_data)
                                        if (regex_pattern_check(communication_comment, comment) or regex_pattern_check(network_comment, comment)) and regex_pattern_check(normal_comment, comment):
                                            status = "OK"
                                            ng_name = ""
                                            break
