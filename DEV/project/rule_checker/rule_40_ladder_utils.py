...
def remove_cross_row_duplicates(df):
    ...
    for idx, row in df.iterrows():
        if "_TO_" in str(row["function_name"]) or "move" in str(row["function_name"]).lower() or "@move" in str(row["function_name"]).lower():
            continue
...
def replace_to_outputs_with_inputs(df):
    ...
    # Build TO output → input map
    for _, row in df.iterrows():
        function_name = str(row["function_name"]).lower()
        if "_to_" in function_name or "move" in function_name or "@move" in function_name:
            for out_col in output_cols:
                out_val = row[out_col]
                if pd.notna(out_val):  # avoid None/NaN
                    to_map[out_val] = row["input1"]

    # Replace values in other rows
    for idx, row in df.iterrows():
        function_name = str(row["function_name"]).lower()
        if "_to_" in function_name or "move" in function_name or "@move" in function_name:
            continue  # skip rows that are TO/MOVE
        for col in input_cols:
            val = row[col]
            if pd.notna(val) and val in to_map:
                df.at[idx, col] = to_map[val]
...
#     out_list=[]
    ...
    ladder_df = pd.read_csv("C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Data_Modelling/version2/csv_json_output_file/data_model_Rule_25_programwise.csv")

    ladder_df=ladder_df[ladder_df['PROGRAM'] == 'P111_Absodex_ZAD']
    ladder_df=ladder_df[ladder_df['BODY'] == 'AutoRun']
    ladder_df=ladder_df[ladder_df['RUNG'] == 4]
