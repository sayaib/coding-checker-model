...
ng_content =  {"cc1":"加工や検査工程において連続不良の検出が実施できていないためNG",
...
# ===============================================================================================
def detection_range_programwise(fault_section_df:pd.DataFrame, program:str, program_comment_data:dict, consecutive_comment:str, fault_comment:str):
            ...
            for _, coil_row in coil_df.iterrows():
                ...
                if isinstance(coil_operand, str) and coil_operand and 'AL' in coil_operand:
                    ...
                    if isinstance(coil_comment, list) and coil_comment:
                        if regex_pattern_check(consecutive_comment, coil_comment) and (regex_pattern_check(fault_comment, coil_comment) or regex_pattern_check("NG", coil_comment)):
                            status = "OK"
...
def checking_first_pattern(current_rung_df:pd.DataFrame, coil_operand:str):
    ...
    if coil_operand in all_self_holding_contact:
        block_df = current_rung_df[current_rung_df['OBJECT_TYPE_LIST'].str.lower() == 'block']
        for _, block_row in block_df.iterrows():
            attr = ast.literal_eval(block_row['ATTRIBUTES'])
            block_operand = attr.get('typeName')

            if isinstance(block_operand, str) and block_operand and "ctd" in block_operand.lower():
                return True
            
    return False


def checking_second_pattern(current_program_df:pd.DataFrame, current_rung_df:pd.DataFrame):
    ...
    if not current_program_df.empty:
        ...
        coil_current_program_df = current_program_df[current_program_df['OBJECT_TYPE_LIST'].str.lower() == 'coil']
        for _, coil_current_program_row in coil_current_program_df.iterrows():
            ...
            if isinstance(coil_current_df_operand, str) and coil_current_df_operand and coil_current_df_operand in all_contact:
                match_contact_as_coil_block_df = current_program_df[(current_program_df['BODY'] == coil_current_program_row['BODY']) &
                ...
                if not match_contact_as_coil_block_df.empty:
                    for _, match_contact_as_coil_block_row in match_contact_as_coil_block_df.iterrows():
                        attr = ast.literal_eval(match_contact_as_coil_block_row['ATTRIBUTES'])
                        block_operand = attr.get('typeName')
...
def check_detail_2_programwise(current_program_df:pd.DataFrame, fault_section_df:pd.DataFrame, detection_result:dict, program:str, program_comment_data:dict, consecutive_comment:str, fault_comment:str):
    status = "NG"
    rung_number = -1
    target_coil = ""
        ...
        if not coil_df.empty:
            for _, coil_row in coil_df.iterrows():
                ...
                if isinstance(coil_operand, str) and coil_operand and 'AL' in coil_operand:
                    ...
                    if isinstance(coil_comment, list) and coil_comment:
                        if regex_pattern_check(consecutive_comment, coil_comment) and (regex_pattern_check(fault_comment, coil_comment) or regex_pattern_check("NG", coil_comment)):
                            current_rung_df = fault_section_df[fault_section_df['RUNG'] == coil_row['RUNG']]
                            ...
                            """
                            first_pattern_status = checking_first_pattern(current_rung_df=current_rung_df, coil_operand=coil_operand)

                            second_pattern_status = False
                            if not first_pattern_status:
                                second_pattern_status = checking_second_pattern(current_program_df=current_program_df, current_rung_df=current_rung_df)
...
def execute_rule_34_programwise(input_program_file:str, 
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            if program.lower() in task_names:
                ...
                detection_result = detection_range_programwise(fault_section_df=fault_section_df,
                                            program=program,
                ...
                cc2_result = check_detail_2_programwise(current_program_df=current_program_df,
