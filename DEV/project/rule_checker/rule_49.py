...
def execute_rule_49(input_file : str, 
                ...
                if len(ladder_fault)>0:
                    ...
                    for rung_num, rung_name, coil_attribute in zip(outcoil_rung_list, outcoil_rung_name_list, outcoil_attributes_list):
                        ...
                        if re.search(AL_var_pattern, coil_operand):
                            if regex_pattern_check(open_comment_pattern, coil_comment_dict) and regex_pattern_check(safety_comment_pattern, coil_comment_dict):
                                ...
                                for rung_num_co, rung_name_co, contact_attribute in zip(contact_rung_list, contact_rung_name_list, contact_attributes_list):
                                    ...
                                    #Check2
                                    if all([contact_type=='false', contact_edge=='falling']):
                                        if regex_pattern_check(master_on_pattern, contact_comment_dict) and regex_pattern_check(complete_pattern, contact_comment_dict):
                                            ...
                                            detail_dict={}
                                            detail_dict['target_rung']=rung_of_interest
                                            detail_dict['A_contact_operand']=contact_operand
                                            ...
                                            ####################################
                                            logger.warning(f" A Contact with falling edge: {contact_operand} detected in \n program:{program} \n Body : Fault \n Rung: {rung_num}")
                                    ...
                                    if contact_type=='true':
                                        if regex_pattern_check(safety_confirmation_pattern, contact_comment_dict):
                                            series_operand_list.append(contact_operand)
                                            ...
                                            detail_dict={}
                                            detail_dict['target_rung']=rung_of_interest
                                            detail_dict['B_contact_operand']=contact_operand
                        ...
                        if all([check_2_flag==1, check_3_flag==1]):
                            ...
                            rung_df=ladder_fault.filter(ladder_fault["RUNG"] == rung_of_interest)
 
                            
                            series_contacts_list=get_series_contacts(rung_df)
                            series_contacts_operands= [[entry.get('operand', 'NONE') for entry in sublist] for sublist in series_contacts_list]
                            
                            
                            series_contacts_operands_set_list=[ set(ele) for ele in series_contacts_operands ]
                        ...
                        if all([check_1_flag==1, check_2_flag==0, check_2_NG_flag==0]):
                            ...
                            sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                'RungNo':[rung_of_interest], "Target":[detail_dict], 
                                'CheckItem':rule_49_check_item,
                                'Detail': ['A contact falling edge, with master on and complete not found'],  'Status': ['']}
                        ...
                        if all([check_1_flag==1, check_3_flag==0,check_3_NG_flag==0 ]):
                            ...
                            sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                'RungNo':[rung_of_interest], "Target":[detail_dict], 
                                'CheckItem':rule_49_check_item,
                                'Detail': ['B contact with safety confirmatione not found'],  'Status': ['']}
                        ...
                        if all([check_1_flag==1, check_4_1_flag==0, check_4_1_NG_flag==0]):
                            ...
                            sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                'RungNo':[rung_of_interest], "Target":[detail_dict], 
                                'CheckItem':rule_49_check_item,
                                'Detail': ['A Contact and B Contact not in series connection'],  'Status': ['']}
                        ...
                        if all([check_1_flag==1, check_4_2_flag==0, check_4_2_NG_flag==0] ):
                    ...
                    if all([check_1_NG_flag==0, AL_coil_found_flag==0 ]):
                        ...
                        sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                ...
                                'CheckItem':rule_49_check_item,
                                'Detail': ['AL variable with open_comment and safety_comment not found'],  'Status': ['']}
