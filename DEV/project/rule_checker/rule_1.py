...
check_detail_content = {"cc1":"Check the rungs in the range detected by ③ in order of decreasing number, and check all outcoils there one by one. If it is an AutoRun section, but ③ is not detected, it is assumed to be NG.", 
...
ng_content = {"cc1":"AutoRunセクション内にサイクル終了コイルが無いため、1サイクルが成り立っていない可能性有 (There is no 1 cycle end outcoil in the AutoRun section, so a cycle may not be formed.)", 
...
def extract_cycle_start_end_programwise(program_df:pd.DataFrame, 
                                        section_name:str, 
                                        section_name_with_star:str, 
                                        program_comment_data:dict) ->dict:

    logger.info("Rule 1 Checking if cycle start/end exists and retrieving corresponding rung number and operand information")
    ...
    if not program_df.empty:
        ...
        for program_name in unique_program_values:
            ...
            logger.info(f"Rule 1 Execute Program {program_name} and section {section_name}, {section_name}")
            ...
            if not curr_program_df.empty:
                ...
                autorun_section_df = curr_program_df[curr_program_df['BODY'].str.lower().isin([section_name, section_name_with_star])]

                if not autorun_section_df.empty:
                    try:
                        coil_df = autorun_section_df[autorun_section_df['OBJECT_TYPE_LIST'].str.lower()=="coil"]
                            ...
                            for _, coil_row in coil_df.iterrows():
                                ...
                                if isinstance(coil_operand,str) and coil_operand:
                                    ...
                                    if isinstance(coil_comment, list) and coil_comment:
                                        if regex_pattern_check(cycle_comment, coil_comment) and regex_pattern_check(start_comment, coil_comment) and cycle_start_exist_rung_number==-1:
                                            cycle_start_exist_rung_number = coil_row['RUNG']
                                            cycle_start_operand = attr.get("operand")
                                        if regex_pattern_check(cycle_comment, coil_comment) and regex_pattern_check(end_comment, coil_comment) and cycle_end_exist_rung_number==-1:
                                            cycle_end_exist_rung_number = coil_row['RUNG']
                                            cycle_end_operand = attr.get("operand")

                    except:
                        logger.info(f"Rule 1 Error processing program {program_name} for cycle start/end extraction.")
                        continue

            cycle_start_end_exist_rung_number_dict[program_name] = [cycle_start_exist_rung_number, cycle_end_exist_rung_number]
            cycle_start_end_operand_dict[program_name] = [cycle_start_operand, cycle_end_operand]
    else:
...
def check_detail_2_programwise(autorun_section_df:pd.DataFrame, 
                               outcoil_operand:str, 
                               start_rung:int, 
                               end_rung:int) -> Tuple[str, int]:

    logger.info(f"Rule 1 Executing check detail 2 for outcoil {outcoil_operand}")

    try:

        contact_coil_df = autorun_section_df[(autorun_section_df['RUNG'] >= start_rung) & (autorun_section_df['RUNG'] <= end_rung)]
...
def check_detail_3_programwise(autorun_section_df:pd.DataFrame, 
                               outcoil_operand:str, 
                               current_rung:int ,
                               start_rung:int, 
                               end_rung:int) -> Tuple[str, int]:
    ...
    try:
        ...
        if not autorun_section_df.empty:
            if start_rung >=0 and end_rung >=0:
                contact_next_rung_to_end_rung_df = autorun_section_df[(autorun_section_df['OBJECT_TYPE_LIST'].str.lower() == "contact") & (autorun_section_df['RUNG'] >= start_rung) & (autorun_section_df['RUNG'] <= end_rung)]
            else:
    ...
    try:
        ...
        if not contact_coil_current_df.empty:
            for _, current_contact_row in contact_coil_current_df.iterrows():
                ...
                if re.search(re.escape(outcoil_operand), attr['operand']) and current_contact_row['OBJECT_TYPE_LIST'].lower()=="coil":
                    is_rule_3_1_valid = True
                    continue

                if is_rule_3_1_valid:
                    if re.search(re.escape(outcoil_operand), attr['operand']) and current_contact_row['OBJECT_TYPE_LIST'].lower()=="contact" and attr['negated']=='false':
                        return "OK", 3.1
    except Exception as e:
...
def check_detail_4_programwise(autorun_section_df:pd.DataFrame, 
                               outcoil_operand:str, 
                               current_rung:int, 
                               start_rung:str, 
                               end_rung:str, 
                               program_name:str, 
                               program_comment_data:dict) -> Tuple[str, int]:
    ...
    try:
        if not autorun_section_df.empty and current_rung >=0:
            self_holding_operand = check_self_holding(autorun_section_df[autorun_section_df['RUNG']==current_rung])
    ...
    try:
        if outcoil_operand and isinstance(outcoil_operand, str):
            coil_comment = get_the_comment_from_program(outcoil_operand, program_name, program_comment_data)
            if isinstance(coil_comment, list) and coil_comment:
                if regex_pattern_check("異常", coil_comment) or regex_pattern_check("NG", coil_comment):
                    return "OK" , 4.1
    except Exception as e:
    ...
    try:
        ...
        logger.info(f"Rule 1 Executing check detail 4.2 for current rung {current_rung} and  outcoil {outcoil_operand}")
        ...
        if not contact_coil_current_df.empty:
            for _, current_contact_row in contact_coil_current_df.iterrows():
                attr = ast.literal_eval(current_contact_row['ATTRIBUTES'])
                if outcoil_operand and isinstance(outcoil_operand, str):
                    if re.search(re.escape(outcoil_operand), attr['operand']) and current_contact_row['OBJECT_TYPE_LIST'].lower()=="coil" and not is_rule_4_2_current_rung_valid:
                        is_rule_4_2_current_rung_valid = True
                        continue

                if is_rule_4_2_current_rung_valid:
                    if outcoil_operand and isinstance(outcoil_operand, str) and outcoil_operand == attr.get('operand') and attr.get('negated') == 'false' and current_contact_row['OBJECT_TYPE_LIST'].lower()=="contact" and not is_contact_present_current_rung:
                ...
                if is_rule_4_2_current_rung_valid and is_contact_present_current_rung:
                    if current_contact_row['OBJECT_TYPE_LIST'].lower()=="coil":
                        coil_operand = attr.get('operand')
                        self_holding_operand = check_self_holding(contact_coil_current_df[contact_coil_current_df['RUNG']==current_rung])

                        if coil_operand and isinstance(coil_operand, str) and self_holding_operand:
                            if coil_operand in self_holding_operand:
                                return "OK", 4.2
        
        logger.info(f"Rule 1 Executing check detail 4.2 for next rung to end rung and  outcoil {outcoil_operand}")
        ...
        if not contact_coil_next_rung_to_end_rung_df.empty:
            for _, contact_coil_row in contact_coil_next_rung_to_end_rung_df.iterrows():
                ...
                attr = ast.literal_eval(contact_coil_row['ATTRIBUTES'])
                if contact_coil_row['OBJECT_TYPE_LIST'].lower()== "contact" and outcoil_operand==attr['operand']:
                    is_contact_operand_present_next_rung = True
                    current_rung_number = contact_coil_row['RUNG']
                    continue

                if is_contact_operand_present_next_rung and contact_coil_row['OBJECT_TYPE_LIST'].lower()=="coil" and contact_coil_row['RUNG']==current_rung_number:
                    attr = ast.literal_eval(contact_coil_row['ATTRIBUTES'])

                    coil_operand = attr.get('operand')
                    self_holding_operand = check_self_holding(contact_coil_next_rung_to_end_rung_df[contact_coil_next_rung_to_end_rung_df['RUNG']==contact_coil_row['RUNG']])
                    if coil_operand in self_holding_operand:
                        return "OK", 4.2
    ...
    try:
        ...
        if not autorun_section_df.empty and current_rung>=0:
            ...
            (autorun_section_df['OBJECT_TYPE_LIST'].str.lower() == "contact") &
            (autorun_section_df['RUNG'] == current_rung)].iloc[0]
        else:
        ...
        if not current_rung_first_contact_df.empty:
            first_contact_attr = ast.literal_eval(current_rung_first_contact_df['ATTRIBUTES'])
            ...
            for rung_number in range(prev_rung, start_rung, -1):
                self_holding_operand = check_self_holding(autorun_section_df[autorun_section_df['RUNG']==rung_number])
...
def check_detail_cycle_start_end_info_programwise(program_df:pd.DataFrame, 
                                                  program_name:str, 
    ...
    try:
        ...
        if not coil_df.empty:
            for _, coil_row in coil_df.iterrows():
                ...
                logger.info(f"Rule 1 Executing all check detail for coil {attr['operand']} of program {program_name} and section {section_name}")

                if attr.get('operand') and isinstance(attr.get('operand'), str):

                    try:
                        cc2_status, cc2_rule_number = check_detail_2_programwise(autorun_section_df=autorun_section_df, outcoil_operand=attr['operand'], start_rung=cycle_start_end_rung_number[0], end_rung=coil_row["RUNG"]-1)
                    except:
                        logger.error(f"Rule 1 Error in check_detail_2_programwise for {attr['operand']}")
                        cc2_status, cc2_rule_number = "NG", -1

                    try:
                        cc3_status, cc3_rule_number = check_detail_3_programwise(autorun_section_df=autorun_section_df, outcoil_operand=attr['operand'], current_rung=coil_row["RUNG"], start_rung=coil_row["RUNG"]+1, end_rung=cycle_start_end_rung_number[1])
                    except:
                        logger.error(f"Rule 1 Error in check_detail_3_programwise for {attr['operand']}")
                        cc3_status, cc3_rule_number = "NG", -1

                    try:
                        cc4_status, cc4_rule_number = check_detail_4_programwise(autorun_section_df=autorun_section_df, outcoil_operand=attr['operand'], current_rung=coil_row["RUNG"], start_rung=cycle_start_end_rung_number[0], end_rung=cycle_start_end_rung_number[1] ,program_name=program_name, program_comment_data=program_comment_data)
                    except:
                        logger.error(f"Rule 1 Error in check_detail_4_programwise for {attr['operand']}")
                    ...
                    for index, (status, rule_number, check_key) in enumerate(check_results):
                        ...
                        #     "SECTION_NAME": "AutoRun",
                        #     "RULE_NUMBER": 1,
                        #     "CHECK_NUMBER": rule_number,
                        #     "RUNG_NUMBER": coil_row["RUNG"]-1,  
                        #     "RULE_CONTENT": rule_content_1,
                        #     "CHECK_CONTENT": check_detail_content.get(check_key),
                        #     "TARGET_OUTCOIL": attr['operand'],
                        #     "STATUS": status,
...
def extract_cycle_start_end_functionwise(function_df:pd.DataFrame, 
                                         section_name:str, 
                                         section_name_with_star:str, 
    ...
    if not function_df.empty:
        ...
        for function_name in unique_function_values:
            ...
            logger.info(f"Rule 1 Execute function {function_name} and section {section_name}, section {section_name}")
            ...
            curr_function_df = function_df[function_df['FUNCTION_BLOCK'] == function_name].copy()

            if not curr_function_df.empty:

                autorun_section_df = curr_function_df[curr_function_df['BODY_TYPE'].str.lower().isin([section_name, section_name_with_star])]
                ...
                if not autorun_section_df.empty:
                    try:
                        ...
                        if not coil_df.empty:
                            for _, coil_row in coil_df.iterrows():
                                attr = ast.literal_eval(coil_row['ATTRIBUTES'])
                                coil_comment = get_the_comment_from_function(attr['operand'], function_name, function_comment_data)
                                if isinstance(coil_comment, list) and coil_comment:
                                    if regex_pattern_check(cycle_comment, coil_comment) and regex_pattern_check(start_comment, coil_comment) and cycle_start_exist_rung_number==-1:
                                        cycle_start_exist_rung_number = coil_row['RUNG']
                                        cycle_start_operand = attr.get("operand")
                        ...
                        logger.info(f"Rule 1 Error processing function {function_name} for cycle start/end extraction.")
                        continue

            cycle_start_end_exist_rung_number_dict[function_name] = [cycle_start_exist_rung_number, cycle_end_exist_rung_number]
            cycle_start_end_operand_dict[function_name] = [cycle_start_operand, cycle_end_operand]
    else:
...
# def rule_1_check_detail():
def check_detail_2_functionwise(autorun_section_df:pd.DataFrame, 
                                outcoil_operand:str, 
                                start_rung:int, 
                                end_rung:int) -> Tuple[str, int]:
...
def check_detail_3_functionwise(autorun_section_df:pd.DataFrame, 
                                outcoil_operand:str, 
                                current_rung:int ,
                                start_rung:int, 
                                end_rung:int) -> Tuple[str, int]:
...
def check_detail_4_functionwise(autorun_section_df:pd.DataFrame, 
                                outcoil_operand:str, 
                                current_rung:int, 
                                start_rung:str,
                                end_rung:str, 
                                function_name:str, 
                                function_comment_data:dict) -> Tuple[str, int]:
    ...
    try:
        if outcoil_operand and isinstance(outcoil_operand, str):
            coil_comment = get_the_comment_from_function(outcoil_operand, function_name, function_comment_data)
            if isinstance(coil_comment, list) and coil_comment:
                if regex_pattern_check("異常", coil_comment) or regex_pattern_check("NG", coil_comment):
                    return "OK" , 4.1
    except Exception as e:
...
def check_detail_cycle_start_end_info_functionwise(function_df:pd.DataFrame, 
                                                   function_name:str, 
    ...
    try:
        ...
        if not coil_df.empty:
            for _, coil_row in coil_df.iterrows():
                ...
                logger.info(f"Rule 1 Executing all check detail for coil {attr['operand']} of function {function_name} and section {section_name}")

                if attr.get('operand') and isinstance(attr.get('operand'), str):

                    try:
                        cc2_status, cc2_rule_number = check_detail_2_functionwise(autorun_section_df=autorun_section_df, outcoil_operand=attr['operand'], start_rung=cycle_start_end_rung_number[0], end_rung=coil_row["RUNG"]-1)
                    except:
                        logger.error(f"Rule 1 Error in check_detail_2_functionwise for {attr['operand']}")
                        cc2_status, cc2_rule_number = "NG", -1

                    try:
                        cc3_status, cc3_rule_number = check_detail_3_functionwise(autorun_section_df=autorun_section_df, outcoil_operand=attr['operand'], current_rung=coil_row["RUNG"], start_rung=coil_row["RUNG"]+1, end_rung=cycle_start_end_rung_number[1])
                    except:
                        logger.error(f"Rule 1 Error in check_detail_3_functionwise for {attr['operand']}")
                        cc3_status, cc3_rule_number = "NG", -1

                    try:
                        cc4_status, cc4_rule_number = check_detail_4_functionwise(autorun_section_df=autorun_section_df, outcoil_operand=attr['operand'], current_rung=coil_row["RUNG"], start_rung=cycle_start_end_rung_number[0], end_rung=cycle_start_end_rung_number[1] ,function_name=function_name, function_comment_data=function_comment_data)
                    except:
                        logger.error(f"Rule 1 Error in check_detail_4_functionwise for {attr['operand']}")
                        cc4_status, cc4_rule_number = "NG", -1
...
def execute_rule_1_programwise(input_program_file:str, 
    ...
    try:
        ...
        program_cycle_start_end_rungnumber_info , program_cycle_start_end_operand_info = extract_cycle_start_end_programwise(program_df=program_df, section_name=section_name, section_name_with_star=section_name_with_star, program_comment_data=program_comment_data)
        for program_name, cycle_start_end_rung_number in program_cycle_start_end_rungnumber_info .items():
            if cycle_start_end_rung_number[0] != -1 and cycle_start_end_rung_number[1] != -1:
                for index, cycle_start_end_info in enumerate([cycle_start_end_rung_number, cycle_start_end_rung_number]):
                    cycle_start_end_coil_operand = program_cycle_start_end_operand_info[program_name][0] if index == 0 else program_cycle_start_end_operand_info[program_name][1]
                    target_rung = cycle_start_end_info[0] if index == 0 else cycle_start_end_info[1]
                    output_rows.append({
                        "Result": "OK",
                        "Task": program_name,
                        "Section": "AutoRun",
                ...
                output_rows = check_detail_cycle_start_end_info_programwise(program_df=program_df, program_name=program_name, program_comment_data=program_comment_data, section_name=section_name, cycle_start_end_rung_number=cycle_start_end_rung_number, output_rows=output_rows)
            else:
                output_rows.append({
                    "Result": "NG",
                    "Task": program_name,
...
def execute_rule_1_functionwise(input_function_file:str, 
    ...
    try:
        ...
        function_cycle_start_end_rungnumber_info , function_cycle_start_end_operand_info = extract_cycle_start_end_functionwise(function_df=function_df, section_name=section_name, section_name_with_star=section_name_with_star, function_comment_data=function_comment_data)
        for function_name, cycle_start_end_rung_number in function_cycle_start_end_rungnumber_info .items():
            if cycle_start_end_rung_number[0] != -1 and cycle_start_end_rung_number[1] != -1:
                for index, cycle_start_end_info in enumerate([cycle_start_end_rung_number, cycle_start_end_rung_number]):
                    cycle_start_end_coil_operand = function_cycle_start_end_operand_info[function_name][0] if index == 0 else function_cycle_start_end_operand_info[function_name][1]
                    target_rung = cycle_start_end_info[0] if index == 0 else cycle_start_end_info[1]
                ...
                output_rows = check_detail_cycle_start_end_info_functionwise(function_df=function_df, function_name=function_name, function_comment_data=function_comment_data, section_name=section_name, cycle_start_end_rung_number=cycle_start_end_rung_number, output_rows=output_rows)
            else:
