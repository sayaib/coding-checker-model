...
check_detail_content = {"cc1":"When ③ or ④ is not found in the detection target in ①, it is  NG.", 
...
ng_content = {"cc1":"Gripper搬送回路だが,記憶送り回路がコーディング基準に沿っていない(開始記憶or記憶送り完了コイル無し) Gripper transport circuit, but the memory feed circuit does not follow coding standards (no start memory or memory feed complete coil).", 
...
def check_start_memory_outcoil_from_program(row, program_name:str, transport_comment:str, start_comment:str, memory_comment, program_comment_data:dict):
    try:
        ...
        if isinstance(attr, dict) and 'operand' in attr and 'latch' in attr:
            if attr.get('latch') == 'set':
                ...
                if isinstance(comment, list):
                    if regex_pattern_check(transport_comment, comment) and regex_pattern_check(start_comment, comment) and regex_pattern_check(memory_comment, comment):
                        return attr.get('operand')
    except Exception:
        return None
    return None

def check_memory_feed_outcoil_from_program(row, program_name:str, memory_feed_comment_list:str , program_comment_data:dict):
    try:
        attr = ast.literal_eval(row['ATTRIBUTES'])
        if isinstance(attr, dict) and 'operand' in attr:
                ...
                if isinstance(comment, list):
                    if (regex_pattern_check(memory_feed_comment_list[0], comment) or regex_pattern_check(memory_feed_comment_list[1], comment) or regex_pattern_check(memory_feed_comment_list[2], comment) or regex_pattern_check(memory_feed_comment_list[3], comment) or regex_pattern_check(memory_feed_comment_list[4], comment) or regex_pattern_check(memory_feed_comment_list[5], comment) or regex_pattern_check(memory_feed_comment_list[6], comment) or regex_pattern_check(memory_feed_comment_list[7], comment)):
                        return attr.get('operand')
    except Exception:
        return None
    return None
...
def check_start_memory_outcoil_from_function(row, function_name:str, transport_comment:str, start_comment:str, memory_comment, function_comment_data:dict):
    try:
        attr = ast.literal_eval(row['ATTRIBUTES'])
        if isinstance(attr, dict) and 'operand' in attr and 'latch' in attr:
            if attr.get('latch') == 'set':
...
def check_memory_feed_outcoil_from_function(row, function_name:str, memory_feed_comment_list:str , function_comment_data:dict):
    try:
        attr = ast.literal_eval(row['ATTRIBUTES'])
        if isinstance(attr, dict) and 'operand' in attr:
                comment = get_the_comment_from_function(attr.get('operand'), function_name, function_comment_data)
...
def extract_rung_group_data_programwise(program_df:pd.DataFrame, program_name:str, section_name:str) -> pd.DataFrame:
    logger.info(f"Rule 18 Group Rung and filter data on program {program_name} and section name {section_name}")
    program_rows = program_df[program_df['PROGRAM'] == program_name].copy()
    memory_feeding_section_rows = program_rows[program_rows['BODY'].str.lower() == section_name.lower()]
    rung_groups_df = memory_feeding_section_rows.groupby('RUNG')
    return rung_groups_df

def detection_range_programwise(memory_feeding_rung_groups_df: pd.DataFrame, transport_comment:str, start_comment:str, memory_comment:str, memory_feed_comment_list:List, program_name:str, program_comment_data:str, section_name:str) -> dict:
    
    logger.info(f"Rule 18 Executing detection range on program {program_name} and section name {section_name}")
    ...
    try:
        for _, rung_df in memory_feeding_rung_groups_df:
            ...
            rung_df['start_memory_check_outcoil'] = rung_df.apply(
                lambda row: check_start_memory_outcoil_from_program(row=row, program_name=program_name, transport_comment=transport_comment, start_comment=start_comment, memory_comment=memory_comment, program_comment_data=program_comment_data) if row['OBJECT_TYPE_LIST'].lower() == 'coil' else None,
                axis=1
            )

            start_memory_match_outcoil = rung_df[rung_df['start_memory_check_outcoil'].notna()]
            if not start_memory_match_outcoil.empty:
                memory_start_comment_operand = start_memory_match_outcoil.iloc[0]['start_memory_check_outcoil']
                memory_start_comment_rung_number = start_memory_match_outcoil.iloc[0]['RUNG'] 

            rung_df['memory_feed_check_outcoil'] = rung_df.apply(
                lambda row: check_memory_feed_outcoil_from_program(row=row, program_name=program_name, memory_feed_comment_list=memory_feed_comment_list, program_comment_data=program_comment_data) if row['OBJECT_TYPE_LIST'].lower() == 'coil' else None,
                axis=1
            )

            memory_feed_match_outcoil = rung_df[rung_df['memory_feed_check_outcoil'].notna()]
            if not memory_feed_match_outcoil.empty:
                memory_feed_comment_operand = memory_feed_match_outcoil.iloc[0]['memory_feed_check_outcoil']
...
def check_detail_1_programwise(detection_result:dict) -> dict:
    ...
    outcoil = [operand for operand in [ detection_result['start_memory'][0], detection_result['memory_feed'][0] ] if operand]
...
def check_detail_2_programwise(memory_feeding_rung_groups_df:pd.DataFrame, start_memory_operand:str) -> dict:
    ...
    if start_memory_operand and isinstance(start_memory_operand, str):
        ...
        try:
            for _, rung_df in memory_feeding_rung_groups_df:
                ...
                rung_df['ATTR_DICT'] = rung_df['ATTRIBUTES'].apply(lambda x: (lambda y: ast.literal_eval(y) if pd.notna(y) else {})(x) if pd.notna(x) else {})
                ...
                for _, rung_row in rung_df.iterrows():
                    ...
                    if start_memory_operand == contact_operand and rising_contact=='rising' and rung_row['OBJECT_TYPE_LIST'].lower()=='contact':
                ...
                if rising_contact_operand_match_found and (move_status or memcopy_status):
                    return {
                        "cc" : "cc2",
                        "status" : "OK",
...
def check_detail_3_programwise(memory_feeding_rung_groups_df:pd.DataFrame, start_memory_operand:str) -> dict:
...
def check_detail_4_programwise(memory_feeding_rung_groups_df:pd.DataFrame, start_memory_operand:str, memory_feed_operand:str) -> dict:
        ...
        try:
            for _, rung_df in memory_feeding_rung_groups_df:
                ...
                for _, rung_row in rung_df.iterrows():
                    ...
                    rising_contact = rung_row['ATTR_DICT'].get("edge", False)
                    if start_memory_operand == operand and rising_contact=='rising' and rung_row['OBJECT_TYPE_LIST'].lower()=='contact':
                        rising_contact_operand_match_found = True
                        rising_contact_operand_match_attr = rung_row['ATTR_DICT']
                    if memory_feed_operand == operand and rung_row['OBJECT_TYPE_LIST'].lower()=='coil':
                ...
                if rising_contact_operand_match_found and memory_feed_operand_present:
                    rising_contact_outcoil_match_out_list = rising_contact_operand_match_attr['out_list']
                    memory_feed_operand_present_in_list = memory_feed_operand_present_attr['in_list']
...
def check_detail_5_programwise(MD_Out_section_rung_group_df:pd.DataFrame, memory_feed_operand:str) -> dict:
    ...
    try:
        for _ , md_out_rung_df in MD_Out_section_rung_group_df:
            ...
            for _, rung_row in md_out_rung_df.iterrows():
                ...
                if rung_row['OBJECT_TYPE_LIST'].lower() == "contact" and rung_row['ATTR_DICT'].get('negated') =='false':
                    if rung_row['ATTR_DICT'].get('operand') == memory_feed_operand:
                        is_memory_feed_match_found_in_detection_4 = True
                
                if is_memory_feed_match_found_in_detection_4 and rung_row['OBJECT_TYPE_LIST'].lower() == "coil":
                    memory_feed_match_outcoil_in_MD = rung_row['ATTR_DICT'].get('operand')
...
def store_program_csv_results_programwise(output_rows:List, all_cc_status:List, program_name:str, section_name:str, ng_content:dict, check_detail_content:str) -> List:
    ...
    for index, cc_status in enumerate(all_cc_status):
        ...
        try:
            rung_number = (rung_number - 1 if isinstance((rung_number := cc_status.get('rung_number')), int) and rung_number >= 0 else '')
        except Exception as e:
...
def extract_rung_group_data_functionwise(function_df:pd.DataFrame, function_name:str, section_name:str) -> pd.DataFrame:
    logger.info(f"Rule 18 Group Rung and filter data on function {function_name} and section name {section_name}")
    function_rows = function_df[function_df['FUNCTION_BLOCK'] == function_name].copy()
    memory_feeding_section_rows = function_rows[function_rows['BODY_TYPE'].str.lower() == section_name.lower()]
    rung_groups_df = memory_feeding_section_rows.groupby('RUNG')
    return rung_groups_df

def detection_range_functionwise(memory_feeding_rung_groups_df: pd.DataFrame, transport_comment:str, start_comment:str, memory_comment:str, memory_feed_comment_list:List, function_name:str, function_comment_data:str, section_name:str) -> dict:
    
    logger.info(f"Rule 18 Executing detection range on function {function_name} and section name {section_name}")
    ...
    try:
        for _, rung_df in memory_feeding_rung_groups_df:
            ...
            rung_df['start_memory_check_outcoil'] = rung_df.apply(
                lambda row: check_start_memory_outcoil_from_function(row=row, function_name=function_name, transport_comment=transport_comment, start_comment=start_comment, memory_comment=memory_comment, function_comment_data=function_comment_data) if row['OBJECT_TYPE_LIST'].lower() == 'coil' else None,
                axis=1
            )
            ...
            rung_df['memory_feed_check_outcoil'] = rung_df.apply(
                lambda row: check_memory_feed_outcoil_from_function(row=row, function_name=function_name, memory_feed_comment_list=memory_feed_comment_list, function_comment_data=function_comment_data) if row['OBJECT_TYPE_LIST'].lower() == 'coil' else None,
                axis=1
            )
        ...
        logger.error(f"Error in detection_range_functionwise: {e}")
...
def check_detail_2_functionwise(memory_feeding_rung_groups_df:pd.DataFrame, start_memory_operand:str) -> dict:
    logger.info(f"Rule 18 Executing check detail 2 ")
    ...
    if start_memory_operand and isinstance(start_memory_operand, str):
        ...
        try:
            ...
            logger.error(f"Error in check_detail_2_functionwise: {e}")   
...
def check_detail_3_functionwise(memory_feeding_rung_groups_df:pd.DataFrame, start_memory_operand:str) -> dict:
    ...
    if start_memory_operand:
        try:
            ...
            logger.error(f"Error in check_detail_3_functionwise: {e}")   
...
def check_detail_4_functionwise(memory_feeding_rung_groups_df:pd.DataFrame, start_memory_operand:str, memory_feed_operand:str) -> dict:
    ...
    if start_memory_operand:
        ...
        try:
            ...
            logger.error(f"Error in check_detail_4_functionwise: {e}")
...
def check_detail_5_functionwise(MD_Out_section_rung_group_df:pd.DataFrame, memory_feed_operand:str) -> dict:
    ...
    try:
        for _ , md_out_rung_df in MD_Out_section_rung_group_df:
            ...
            for _, rung_row in md_out_rung_df.iterrows():
                ...
                if rung_row['OBJECT_TYPE_LIST'].lower() == "contact":
                    if rung_row['ATTR_DICT'].get('operand') == memory_feed_operand:
                        is_memory_feed_match_found_in_detection_4 = True
        ...
        logger.error(f"Error in check_detail_5_functionwise: {e}")
...
def execute_rule_18_programwise(input_program_file:str, 
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            logger.info(f"Rule 18 Executing in Program {program} and section {section_name}")
            ...
            # Save output to CSV
            output_row = store_program_csv_results_programwise(
        ...
        final_output_df = pd.DataFrame(output_row)
...
def execute_rule_18_functionwise(input_function_file:str, 
    ...
    try:
        ...
        for function in unique_function_values:
            ...
            logger.info(f"Rule 18 Executing in function {function} and section {section_name}")
            ...
            # Define list of memory feed-related comments
            memory_feed_comment_list = [
                memory_feed_full_complete_comment,
                memory_feed_half_complete_comment,
                memory_feed_full_timing_comment,
            ...
            cc4_result = check_detail_4_functionwise(
                memory_feeding_rung_groups_df=memory_feeding_rung_groups_df,
                start_memory_operand=start_memory_operand,
                memory_feed_operand=memory_feed_operand
            )
            ...
            # MD_Out section checks
            MD_Out_section_rung_group_df = extract_rung_group_data_functionwise(
                function_df=function_df,
                function_name=function,
                section_name=MD_OUT_section
            )

            cc5_result = check_detail_5_functionwise(
                MD_Out_section_rung_group_df=MD_Out_section_rung_group_df,
                memory_feed_operand=memory_feed_operand
            )
            ...
            # Save output to CSV
            output_row = store_function_csv_results_functionwise(
                output_rows=output_rows,
                all_cc_status=all_cc_status,
                function_name=function,
                section_name=section_name,
...
#     input_program_file = "C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Data_Modelling/version2/csv_json_output_file/data_model_Rule_18_programwise.csv"
#     input_program_comment_file = "C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Data_Modelling/version2/csv_json_output_file/data_model_Rule_18_programwise.json"
#     input_function_file = "C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Data_Modelling/version2/csv_json_output_file/data_model_Rule_18_functionwise.csv"
#     input_function_comment_file = "C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Data_Modelling/version2/csv_json_output_file/data_model_Rule_18_functionwise.json"
#     output_folder_path = 'C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Rules_implementation/pythoncode/output_csv/'
#     program_output_file = 'Rule_18_program.csv'
#     function_output_file = 'Rule_18_function.csv'


#     final_csv = execute_rule_18_programwise(input_program_file=input_program_file, input_program_comment_file=input_program_comment_file)
#     final_csv.to_csv(f"{output_folder_path}/{program_output_file}", index=False, encoding='utf-8-sig')

#     final_csv = execute_rule_18_functionwise(input_function_file=input_function_file, input_function_comment_file=input_function_comment_file)
