...
check_detail_content = {"cc1":" If ① but not ③, it is assumed to be NG.", "cc2":" Check that contact A includes the variable comment of ”電源入り(power on)"+"遅延(delay)” in the out coil condition detected in ③.", "cc3":"Check that contact B includes the variable comment of ”非常停止(emergency stop)"+"でない(not)” in the out coil condition detected in ③.", "cc4":"Check that the out coil is connected in series (AND) with only the contacts of ❶and❷ within the self-holding of the out coil detected by ③."}
ng_content = {"cc1":"Fault回路が標準通りに作られていない(異常回路の抜け・漏れ)可能性あり(Fault circuit may not be built to standard (abnormal circuit missing or leaking))", "cc2":"非常停止操作異常ALの条件に”電源入り遅延”のA接点が存在しない(There is no A contact for “power-on delay” in the emergency stop operation error AL conditions.)", "cc3":"非常停止操作異常ALの条件に”非常停止でない”のB接点が存在しない(There is no B contact for “not emergency stop” in the emergency stop operation error AL conditions.)", "cc4":"非常停止操作の異常回路が標準通りに構成されていない(Abnormal circuit for emergency stop operation not configured as per standard.)"}
...
def detection_range_programwise(fault_section_df: pd.DataFrame, program_name:str, program_comment_data:dict) -> dict:
    ...
    for _, coil_row in coil_df.iterrows():
        ...
        if 'AL' in coil_operand:
            ...
            if isinstance(coil_comment, list):
                if regex_pattern_check(emergency_stop_comment, coil_comment) and regex_pattern_check(operation_comment, coil_comment):
                    return {
                        "status":"OK",
...
def check_detail_1_programwise(fault_section_df:pd.DataFrame, detection_result:dict) -> dict:
    cc1_result = {}
...
def check_detail_2_programwise(fault_section_df:pd.DataFrame, rung_number:int, program_name:str, program_comment_data:str) -> dict:
...
def check_detail_3_programwise(fault_section_df:pd.DataFrame, rung_number:int, program_name:str, program_comment_data:str) -> dict:
    ...
    for _, contact_row in contact_df.iterrows():
        ...
        if isinstance(contact_comment, list) and contact_comment:
            if regex_pattern_check(emergency_stop_comment, contact_comment) and regex_pattern_check(not_comment, contact_comment) and negated_operand == 'true':
                status = "OK"
                match_contact_operand = contact_operand
                break
...
def check_detail_4_programwise(fault_section_df:pd.DataFrame, detection_coil:str, cc2_contact:str, cc3_contact:str, rung_number:int, program_name:str) -> dict:
...
def detection_range_functionwise(fault_section_df: pd.DataFrame, function_name:str, function_comment_data:dict) -> dict:
    
    logger.info(f"Executing rule 47 detection range on function {function_name}")
...
def check_detail_1_functionwise(fault_section_df:pd.DataFrame, detection_result:dict):
    cc1_result = {}
...
def check_detail_2_functionwise(fault_section_df:pd.DataFrame, rung_number:int, function_name:str, function_comment_data:str) -> dict:

    logger.info(f"Executing rule no 47 check detail 1 in function {function_name}")
...
def check_detail_3_functionwise(fault_section_df:pd.DataFrame, rung_number:int, function_name:str, function_comment_data:str) -> dict:

    logger.info(f"Executing rule no 47 check detail 2 in function {function_name}")
...
def check_detail_4_functionwise(fault_section_df:pd.DataFrame, detection_coil:str, cc2_contact:str, cc3_contact:str, rung_number:int, function_name:str) -> dict:

    logger.info(f"Executing rule no 47 check detail 3 in function {function_name}")
...
def execute_rule_47_programwise(input_program_file:str, 
    ...
    try:
        ...
        for program_name in unique_program_values:
            ...
            if 'main' in program_name.lower():
                ...
                # if status == "NG":
                cc1_result = check_detail_1_programwise(fault_section_df=fault_section_df, detection_result=detection_result)
                cc2_result = check_detail_2_programwise(fault_section_df=fault_section_df, rung_number=rung_number, program_name=program_name, program_comment_data=program_comment_data)
                cc3_result = check_detail_3_programwise(fault_section_df=fault_section_df, rung_number=rung_number, program_name=program_name, program_comment_data=program_comment_data)
                
                cc2_contact = cc2_result.get('target_coil', '')
                cc3_contact = cc3_result.get('target_coil', '')
                cc4_result = check_detail_4_programwise(fault_section_df=fault_section_df, detection_coil=detection_coil, cc2_contact=cc2_contact, cc3_contact=cc3_contact, rung_number=rung_number, program_name=program_name)
...
def execute_rule_47_functionwise(input_function_file:str, 
                                 input_function_comment_file:str, ) -> pd.DataFrame:

    logger.info("Starting execution of Rule 47")

    try:
        function_df = pd.read_csv(input_function_file)
        with open(input_function_comment_file, 'r', encoding="utf-8-sig") as file:
        ...
        for function_name in unique_function_values:
            logger.info(f"Executing rule 47 in function {function_name}")
            ...
            if 'main' in function_name.lower():
                ...
                # if status == "NG":
                cc1_result = check_detail_1_functionwise(fault_section_df=fault_section_df, detection_result=detection_result)
                cc2_result = check_detail_2_functionwise(fault_section_df=fault_section_df, rung_number=rung_number, function_name=function_name, function_comment_data=function_comment_data)
                cc3_result = check_detail_3_functionwise(fault_section_df=fault_section_df, rung_number=rung_number, function_name=function_name, function_comment_data=function_comment_data)
                
                cc2_contact = cc2_result.get('target_coil', '')
                cc3_contact = cc3_result.get('target_coil', '')
                cc4_result = check_detail_4_functionwise(fault_section_df=fault_section_df, detection_coil=detection_coil, cc2_contact=cc2_contact, cc3_contact=cc3_contact, rung_number=rung_number, function_name=function_name)
...
#     program_output_file = 'Rule_47_programwise_NG_v2.csv'
#     function_output_file = 'Rule_47_functionwise_NG_v2.csv'

#     final_csv = execute_rule_47_programwise(input_program_file=input_program_file, input_program_comment_file=input_program_comment_file)
#     final_csv.to_csv(f"{output_folder_path}/{program_output_file}", index=False, encoding='utf-8-sig')

#     final_csv = execute_rule_47_functionwise(input_function_file=input_function_file, input_function_comment_file=input_function_comment_file)
