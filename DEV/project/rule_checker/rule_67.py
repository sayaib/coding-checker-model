...
check_detail_content = {"cc1":"If ① but not ③, ④, ⑤, ⑥, ⑦, ⑧, and ⑨, it is assumed to be NG.",
                        ...
                        "cc3": "Check that only one A contact containing ”自動(auto)"+"運転(run)"+"中(during)” in the variable comment is connected to the outcoil condition detected in ④.",
                        ...
                        "cc9" : "Check that an A contact containing ”緊急停止(emergency stop)"+"でない(not)” in the variable comment is connected to the outcoil condition detected in ⑨.",
                        "cc10" : "Check that an B contact containing ”設備(machine)"+"停止(stop)” in the variable comment is connected to the outcoil condition detected in ⑨.", 
                        "cc11" : " Check that ❽, ❾, ❿, and ⓫ are connected in series (AND) to the outcoil condition detected in ⑨."}


ng_content = {"cc1":"Mainタスクのdevice outセクションにて、機器出力の回路が存在しないためNG", 
...
def detection_range_programwise(deviceout_section_df: pd.DataFrame, 
                                program_name:str, 
                                ...
                                interlock_comment:str) -> dict:
    
    logger.info(f"Executing rule 67 detection range on program {program_name} and section name {deviceout_section_name} on rule 67")
    
    # Filter only 'Contact' rows
    coil_df = deviceout_section_df[deviceout_section_df['OBJECT_TYPE_LIST'].str.lower() == 'coil'].copy()

    match_coil = {"PL_masteron":[], 
                  "PL_auto":[],
                  "tower_red":[],
                  "tower_yellow":[],
                  "tower_green":[],
        ...
        for _, coil_row in coil_df.iterrows():
            ...
            if isinstance(coil_operand, str) and coil_operand:
                ...
                if isinstance(coil_comment, list) and coil_comment:
                    if regex_pattern_check(PL_comment, coil_comment) and regex_pattern_check(master_on_comment, coil_comment):
                        match_coil['PL_masteron'] = [coil_operand, coil_row['RUNG']]

                    if regex_pattern_check(PL_comment, coil_comment) and regex_pattern_check(auto_comment, coil_comment) and regex_pattern_check(start_comment, coil_comment):
                        match_coil['PL_auto'] = [coil_operand, coil_row['RUNG']]

                    if regex_pattern_check(tower_comment, coil_comment) and regex_pattern_check(red_comment, coil_comment):
                        match_coil['tower_red'] = [coil_operand, coil_row['RUNG']]

                    if regex_pattern_check(tower_comment, coil_comment) and regex_pattern_check(yellow_comment, coil_comment):
                        match_coil['tower_yellow'] = [coil_operand, coil_row['RUNG']]

                    if regex_pattern_check(tower_comment, coil_comment) and regex_pattern_check(green_comment, coil_comment):
                    ...
                    if regex_pattern_check(detection_emergency_stop_comment, coil_comment) and regex_pattern_check(interlock_comment, coil_comment):
                        match_coil['emergency_stop_interlock'] = [coil_operand, coil_row['RUNG']]
...
def check_detail_2_programwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               program_name:str,
                               program_comment_data:dict,
                               master_on_comment:str,
                               complete_comment:str) -> dict:
    ...
    if detection_result['PL_masteron']:
        ...
        current_rung_number_df = deviceout_section_df[deviceout_section_df['RUNG'] == rung_number]
        contact_df = current_rung_number_df[current_rung_number_df['OBJECT_TYPE_LIST'].str.lower() == 'contact'].copy()
            ...
            for _, contact_row in contact_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(master_on_comment, contact_comment) and regex_pattern_check(complete_comment, contact_comment) and negated_operand == 'false':
                            status = "OK"
...
def check_detail_3_programwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               program_name:str,
                               program_comment_data:dict,
                               auto_comment:str,
                               run_comment:str,
                               during_comment:str) -> dict:
        ...
        if len(contact_df) == 1:
            for _, contact_row in contact_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(auto_comment, contact_comment) and regex_pattern_check(run_comment, contact_comment) and regex_pattern_check(during_comment, contact_comment) and negated_operand=='false':
                            status = "OK"
                            operand = contact_operand
                            break
...
def check_detail_4_programwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               program_name:str,
                               program_comment_data:dict,
                               tower_comment:str,
                               red_comment:str) -> dict:
        ...
        if len(contact_df) == 1:
            for _, contact_row in contact_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(tower_comment, contact_comment) and regex_pattern_check(red_comment, contact_comment) and negated_operand == 'false':
                            status = "OK"
                            operand = contact_operand
                            break
...
def check_detail_5_programwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               program_name:str,
                               program_comment_data:dict,
                               tower_comment:str,
                               yellow_comment:str) -> dict:
        ...
        if len(contact_df) == 1:
            for _, contact_row in contact_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(tower_comment, contact_comment) and regex_pattern_check(yellow_comment, contact_comment) and negated_operand=='false':
                            status = "OK"
                            operand = contact_operand
                            break
...
def check_detail_6_programwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               program_name:str,
                               program_comment_data:dict,
                               tower_comment:str,
                               green_comment:str) -> dict:
        ...
        if len(contact_df) == 1:
            for _, contact_row in contact_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(tower_comment, contact_comment) and regex_pattern_check(green_comment, contact_comment) and negated_operand == 'false':
                            status = "OK"
                            operand = contact_operand
                            break
...
def check_detail_7_programwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               program_name:str,
                               program_comment_data:dict,
                               buzzer_comment:str) -> dict:
        ...
        if len(contact_df) == 1:
            for _, contact_row in contact_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(buzzer_comment, contact_comment) and negated_operand == 'false':
                            status = "OK"
                            operand = contact_operand
                            break
...
def check_detail_8_programwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               program_name:str,
                               program_comment_data:dict,
                               PLC_comment:str,
    ...
    if detection_result['emergency_stop_interlock']:
        ...
        for _, contact_row in contact_df.iterrows():
            ...
            if isinstance(contact_operand, str) and contact_operand:
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(PLC_comment, contact_comment) and regex_pattern_check(normal_comment, contact_comment) and  negated_operand == 'false':
                        status = "OK"
                        operand = contact_operand
                        break
...
def check_detail_9_programwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               program_name:str,
                               program_comment_data:dict,
    ...
    if detection_result['emergency_stop_interlock']:
        ...
        for _, contact_row in contact_df.iterrows():
            ...
            if isinstance(contact_operand, str) and contact_operand:
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(checking_content_emergency_stop_comment, contact_comment) and regex_pattern_check(not_comment, contact_comment) and  negated_operand == 'false':
                        status = "OK"
                        operand = contact_operand
                        break
...
def check_detail_10_programwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               program_name:str,
                               program_comment_data:dict,
                               machine_comment:str,
    ...
    if detection_result['emergency_stop_interlock']:
        ...
        for _, contact_row in contact_df.iterrows():
            ...
            if isinstance(contact_operand, str) and contact_operand:
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(machine_comment, contact_comment) and regex_pattern_check(stop_comment, contact_comment) and  negated_operand == 'true':
                        status = "OK"
                        operand = contact_operand
                        break
...
def check_detail_11_programwise(deviceout_section_df:pd.DataFrame,
                                program_name:str,
                                cc8_result:str,
                                cc9_result:str,
                                cc10_result:str) -> dict:
    ...
    if all(res.get("status") == 'OK' for res in [cc8_result, cc9_result, cc10_result]):
        ...
        current_rung_df = deviceout_section_df[deviceout_section_df['RUNG'] == rung_number]
        all_series_contact_data = get_series_contacts(pl.from_pandas(current_rung_df))
        series_contact_operand_list = [[step['operand'] for step in rung] for rung in all_series_contact_data]
        if series_contact_operand_list:
            for each_series_contact in series_contact_operand_list:
                if all(operand in each_series_contact for operand  in [cc8_operand, cc9_operand, cc10_operand]):
                    status = "OK"
                    break
...
def detection_range_functionwise(deviceout_section_df: pd.DataFrame, 
                                function_name:str, 
    ...
    logger.info(f"Executing rule 67 detection range on function {function_name} and section name {deviceout_section_name} on rule 67")
...
def check_detail_1_functionwise(detection_result:dict, 
    ...
    logger.info(f"Executing rule no 67 check detail 1 in function {function_name}")
...
def check_detail_2_functionwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               master_on_comment:str,
    ...
    logger.info(f"Executing rule no 67 check detail 2 in function {function_name}")
...
def check_detail_3_functionwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               auto_comment:str,
    ...
    logger.info(f"Executing rule no 67 check detail 3 in function {function_name}")
...
def check_detail_4_functionwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               tower_comment:str,
    ...
    logger.info(f"Executing rule no 67 check detail 4 in function {function_name}")
...
def check_detail_5_functionwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               tower_comment:str,
    ...
    logger.info(f"Executing rule no 67 check detail 5 in function {function_name}")
...
def check_detail_6_functionwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               tower_comment:str,
    ...
    logger.info(f"Executing rule no 67 check detail 6 in function {function_name}")
...
def check_detail_7_functionwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               buzzer_comment:str) -> dict:

    logger.info(f"Executing rule no 67 check detail 7 in function {function_name}")
...
def check_detail_8_functionwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               PLC_comment:str,
    ...
    logger.info(f"Executing rule no 67 check detail 8 in function {function_name}")
...
def check_detail_9_functionwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               checking_content_emergency_stop_comment:str,
    ...
    logger.info(f"Executing rule no 67 check detail 9 in function {function_name}")
...
def check_detail_10_functionwise(deviceout_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               machine_comment:str,
    ...
    logger.info(f"Executing rule no 67 check detail 10 in function {function_name}")
...
def check_detail_11_functionwise(deviceout_section_df:pd.DataFrame,
                                function_name:str,
                                cc8_result:str,
                                cc9_result:str,
                                cc10_result:str) -> dict:

    logger.info(f"Executing rule no 67 check detail 11 in function {function_name}")
...
def execute_rule_67_programwise(input_program_file:str, 
                                program_comment_file) -> pd.DataFrame:
    ...
    try:
        ...
        for program_name in unique_program_values:
            ...
            if "main" in program_name.lower():
                ...
                cc2_result = check_detail_2_programwise(deviceout_section_df=deviceout_section_df,
                ...
                cc3_result = check_detail_3_programwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        program_name=program_name, 
                                                        program_comment_data=program_comment_data,
                                                        auto_comment=auto_comment,
                ...
                cc4_result = check_detail_4_programwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        program_name=program_name, 
                                                        program_comment_data=program_comment_data,
                                                        tower_comment=tower_comment,
                ...
                cc5_result = check_detail_5_programwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        program_name=program_name, 
                                                        program_comment_data=program_comment_data,
                                                        tower_comment=tower_comment,
                ...
                cc6_result = check_detail_6_programwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        program_name=program_name, 
                                                        program_comment_data=program_comment_data,
                                                        tower_comment=tower_comment,
                ...
                cc8_result = check_detail_8_programwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        program_name=program_name, 
                                                        program_comment_data=program_comment_data,
                ...
                cc9_result = check_detail_9_programwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        program_name=program_name, 
                                                        program_comment_data=program_comment_data,
                ...
                cc10_result = check_detail_10_programwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        program_name=program_name, 
                                                        program_comment_data=program_comment_data,
                ...
                cc11_result = check_detail_11_programwise(deviceout_section_df=deviceout_section_df,
                                                          program_name=program_name,
                ...
                for cc_result in [cc1_result, cc2_result, cc3_result, cc4_result, cc5_result, cc6_result, cc7_result, cc8_result, cc9_result, cc10_result, cc11_result]:
...
def execute_rule_67_functionwise(input_function_file:str, 
                                function_comment_file) -> pd.DataFrame:
    ...
    try:
        ...
        for function_name in unique_function_values:
            ...
            logger.info(f"Executing rule 67 in function {function_name}")
            ...
            if "main" in function_name.lower():
                ...
                cc2_result = check_detail_2_functionwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        function_name=function_name, 
                                                        function_comment_data=function_comment_data,
                                                        master_on_comment=master_on_comment,
                ...
                cc3_result = check_detail_3_functionwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        function_name=function_name, 
                                                        function_comment_data=function_comment_data,
                                                        auto_comment=auto_comment,
                ...
                cc4_result = check_detail_4_functionwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        function_name=function_name, 
                                                        function_comment_data=function_comment_data,
                                                        tower_comment=tower_comment,
                ...
                cc5_result = check_detail_5_functionwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        function_name=function_name, 
                                                        function_comment_data=function_comment_data,
                                                        tower_comment=tower_comment,
                ...
                cc6_result = check_detail_6_functionwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        function_name=function_name, 
                                                        function_comment_data=function_comment_data,
                                                        tower_comment=tower_comment,
                ...
                cc8_result = check_detail_8_functionwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        function_name=function_name, 
                                                        function_comment_data=function_comment_data,
                                                        PLC_comment=PLC_comment,
                ...
                cc9_result = check_detail_9_functionwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        function_name=function_name, 
                                                        function_comment_data=function_comment_data,
                                                        checking_content_emergency_stop_comment=checking_content_emergency_stop_comment,
                ...
                cc10_result = check_detail_10_functionwise(deviceout_section_df=deviceout_section_df,
                                                        detection_result=detection_result, 
                                                        function_name=function_name, 
                                                        function_comment_data=function_comment_data,
                                                        machine_comment=machine_comment,
                ...
                cc11_result = check_detail_11_functionwise(deviceout_section_df=deviceout_section_df,
                                                          function_name=function_name,
                                                            cc8_result=cc8_result,
                                                            cc9_result=cc9_result,
                                                            cc10_result=cc10_result)
