...
check_detail_content = {
    "cc1": "If ① but not ③, it is assumed to be NG. If even one exists, check item 1 is OK.",
...
ng_content = {
    "cc1": "ワーク確認センサとワーク有無情報の不一致を検出できない可能性有",
...
def detection_range_programwise(
    fault_section_df: pd.DataFrame, program: str, program_comment_data: dict
):
    ...
    for _, coil_row in coil_df.iterrows():
        ...
        if isinstance(coil_operand, str) and coil_operand and "AL" in coil_operand:
            ...
            if isinstance(coil_comment, list) and coil_comment:
                if regex_pattern_check(workpiece_comment, coil_comment) and (
                    regex_pattern_check(unmatch_comment_1, coil_comment)
                    or regex_pattern_check(unmatch_comment_2, coil_comment)
                    or regex_pattern_check(match_comment, coil_comment)
                    or regex_pattern_check(state_comment, coil_comment)
                ):
                    match_outcoil[index] = {
                        "coil": coil_operand,
...
def check_detail_2_programwise(
    ...
    if detection_result:
        ...
        contact_match_rung_df = fault_section_df[
        ...
        for _, contact_row in contact_match_rung_df.iterrows():
            ...
            if isinstance(contact_operand, str) and contact_operand:
                ...
                if (
                    regex_pattern_check(workpiece_comment, contact_comment)
                    and (
                        regex_pattern_check(with_comment_1, contact_comment)
                        or regex_pattern_check(with_comment_2, contact_comment)
                        or regex_pattern_check(confirm_comment, contact_comment)
                    )
...
def check_detail_3_programwise(
    ...
    if detection_result and cc2_contact:
        rung_number = detection_result[0]["rung_number"]
        block_contact_df = fault_section_df[
            (fault_section_df["RUNG"] == rung_number)
        ...
        for _, block_contact_row in block_contact_df.iterrows():
            ...
            if (
                block_contact_row["OBJECT_TYPE_LIST"].lower() == "block"
                and attr_typename == "="
            ):
                equal_function_block_outlists.append(attr.get("_outVar_out_list"))

            if (
                block_contact_row["OBJECT_TYPE_LIST"].lower() == "contact"
                and operand == cc2_contact
            ):
        ...
        for _, contact_row in contact_match_rung_df.iterrows():
            ...
            if isinstance(contact_operand, str) and contact_operand:
                ...
                if (
                    regex_pattern_check(workpiece_comment, contact_comment)
                    and (
                        regex_pattern_check(without_comment_1, contact_comment)
                        or regex_pattern_check(without_comment_2, contact_comment)
                    )
                    and negated_operand == "false"
                ):
                    status = "OK"
...
def check_detail_6_programwise(
    ...
    if (
        all(
            cc.get("status") == "OK"
            for cc in [cc1_results, cc2_results, cc3_results, cc4_results, cc5_results]
        )
        ...
        if count_equal_block_present >= 2:
            ...
            if cc2_contact and cc4_contact:
                status = "OK"
                current_rung_contact_df = fault_section_df[
                    (fault_section_df["RUNG"] == rung_number)
                    & (fault_section_df["OBJECT_TYPE_LIST"].str.lower() == "contact")
                ]
                get_series_connect_data = get_series_contacts(
                    pl.from_pandas(current_rung_contact_df)
                )
...
def execute_rule_35_programwise(
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            is_memory_feeding_present = "memoryfeeding" in (
                val.lower() for val in unique_section_values
            )
            if is_memory_feeding_present:
                fault_section_df = current_program_df[
                    current_program_df["BODY"].str.lower() == "fault"
                ]
                ...
                if not fault_section_df.empty:
                    ...
                    cc6_results = check_detail_6_programwise(
                        fault_section_df=fault_section_df,
                        detection_result=detection_result,
                        cc1_results=cc1_results,
                        cc2_results=cc2_results,
                        cc3_results=cc3_results,
                        cc4_results=cc4_results,
                        cc5_results=cc5_results,
                    )
