...
ng_content = {
    "cc1": "各タスクの起動条件に原位置の回路が標準通りに作成されていないためNG",
            ...
            if isinstance(coil_operand, str) and coil_operand:
                ...
                if isinstance(coil_comment, list) and coil_comment:
                    if regex_pattern_check(
                        home_comment, coil_comment
                    ) and regex_pattern_check(pos_comment, coil_comment):
...
def check_detail_2_programwise(
    ...
    if detection_result:
        ...
        if match_rung_number != -1:
            match_rung_df = condition_section_df[
                condition_section_df["RUNG"] == match_rung_number
            ]
            if not match_rung_df.empty:
                contact_df = match_rung_df[
                    match_rung_df["OBJECT_TYPE_LIST"].str.lower() == "contact"
                ]
                if not contact_df.empty:
                    for _, contact_row in contact_df.iterrows():
                        ...
                        if (
                            isinstance(contact_operand, str)
                            and contact_operand
                            ...
                            if isinstance(contact_comment, list) and contact_comment:
                                if (
                                    regex_pattern_check(end_comment, contact_comment)
                                    or regex_pattern_check(pos_comment, contact_comment)
                                    or regex_pattern_check("POS", contact_comment)
                                ):
                                    status = "OK"
                                else:
                                    status = "NG"
...
def check_detail_3_programwise(
    condition_section_df: pd.DataFrame, detection_result: dict
):
...
def check_detail_4_programwise(
    hmi_out_main_program_df: pd.DataFrame,
    auto_comment: str,
    start_comment: str,
    ...
    if not hmi_out_main_program_df.empty:
        coil_df = hmi_out_main_program_df[
            hmi_out_main_program_df["OBJECT_TYPE_LIST"].str.lower() == "coil"
        ]
        if not coil_df.empty:
            for _, coil_row in coil_df.iterrows():
                attr = ast.literal_eval(coil_row["ATTRIBUTES"])
                ...
                if isinstance(coil_operand, str) and coil_operand:
                    ...
                    if isinstance(coil_comment, list) and coil_comment:
                        if (
                            ...
                            and regex_pattern_check(home_comment, coil_comment)
                            and regex_pattern_check(pos_comment, coil_comment)
                        ):
                            status = "OK"
...
def check_detail_5_programwise(hmi_out_main_program_df: pd.DataFrame, cc4_result: dict):
    ...
    if cc4_result and cc4_result.get("status") == "OK":
        rung_number = cc4_result.get("rung_number")
        if rung_number != -1:
            match_rung_df = hmi_out_main_program_df[
                hmi_out_main_program_df["RUNG"] == rung_number
            ]
            if not match_rung_df.empty:
                contact_df = match_rung_df[
                    match_rung_df["OBJECT_TYPE_LIST"].str.lower() == "contact"
...
def check_detail_6_programwise(
    ...
    if cc5_result and cc5_result.get("status") == "OK":
        ...
        if target_contact:
            coil_df = main_program_df[
                main_program_df["OBJECT_TYPE_LIST"].str.lower() == "coil"
            ]
            if not coil_df.empty:
                for _, coil_row in coil_df.iterrows():
                    ...
                    if (
                        ...
                        and coil_operand
                        and coil_operand == target_contact
                    ):
                        rung_number = coil_row["RUNG"]
                        match_rung_df = main_program_df[
                            (main_program_df["BODY"] == coil_row["BODY"])
                            & (main_program_df["RUNG"] == rung_number)
                        ]
                        if not match_rung_df.empty:
                            contact_df = match_rung_df[
                                match_rung_df["OBJECT_TYPE_LIST"].str.lower()
                                == "contact"
                            ]
                            if len(contact_df) == 1:
                                for _, contact_row in contact_df.iterrows():
                                    ...
                                    if (
                                        ...
                                        if (
                                            isinstance(contact_comment, list)
                                            and contact_comment
                                        ):
                                            if regex_pattern_check(
                ...
                for _, coil_row in coil_df.iterrows():
                    ...
                    if (
                        ...
                        if not match_rung_df.empty:
                            ...
                            current_section_df = main_program_df[
                                main_program_df["BODY"].str.lower()
                                == coil_row["BODY"].lower()
                            ]
                            match_coil_df = current_section_df[
                                current_section_df["OBJECT_TYPE_LIST"].str.lower()
                                == "coil"
                            ]
                            ...
                            if all_match_contact:
                                for _, match_coil_row in match_coil_df.iterrows():
                                    match_coil_attr = ast.literal_eval(
                                        match_coil_row["ATTRIBUTES"]
                                    )
                                    match_coil_operand = match_coil_attr.get("operand")
                                    if (
                                        isinstance(match_coil_operand, str)
                                        and match_coil_operand
                                        and match_coil_operand in all_match_contact
                                    ):
                                        current_match_contact_df = main_program_df[
                                            (
                                                main_program_df["BODY"]
                                                == coil_row["BODY"]
                                            )
                                            & (
                                                main_program_df["RUNG"]
                                                == match_coil_row["RUNG"]
                                            )
                                        ]
                                        for (
                                            _,
                                            current_contact_row,
                                        ) in current_match_contact_df.iterrows():
                                            current_contact_attr = ast.literal_eval(
                                                current_contact_row["ATTRIBUTES"]
                                            )
                                            current_contact_operand = (
                                                current_contact_attr.get("operand")
                                            )
                                            negated_operand = current_contact_attr.get(
                                                "negated"
                                            )
                                            if (
                                                isinstance(current_contact_operand, str)
                                                and current_contact_operand
                                                and negated_operand == "false"
                                            ):
                                                if (
                                                    current_contact_operand.lower()
                                                    != "gsb000"
                                                ):
                                                    get_final_match_contact.append(
                                                        current_contact_operand
                                                    )
                            ...
                            if get_final_match_contact and detection_result:
                                detected_3_outcoil = detection_result[0][0]
                                coil_df = program_df[
                                    program_df["OBJECT_TYPE_LIST"].str.lower() == "coil"
                                ]
                                for _, coil_row in coil_df.iterrows():
                                    coil_attr = ast.literal_eval(coil_row["ATTRIBUTES"])
                                    coil_operand = coil_attr.get("operand")
                                    if (
                                        isinstance(coil_operand, str)
                                        and coil_operand
                                        and coil_operand in get_final_match_contact
                                    ):
                                        rung_number = coil_row["RUNG"]
                                        match_section_rung_df = program_df[
                                            (
                                                program_df["PROGRAM"]
                                                == coil_row["PROGRAM"]
                                            )
                                        ...
                                        match_contact_df = match_section_rung_df[
                                            match_section_rung_df[
                                                "OBJECT_TYPE_LIST"
                                            ].str.lower()
                                            == "contact"
                                        ]
                                        if len(match_contact_df) == 1:
                                            for (
                                                _,
                                                match_contact_row,
                                            ) in match_contact_df.iterrows():
                                                match_contact_attr = ast.literal_eval(
                                                    match_contact_row["ATTRIBUTES"]
                                                )
                                                match_contact_operand = (
                                                    match_contact_attr.get("operand")
                                                )
                                                if (
                                                    isinstance(
                                                        match_contact_operand, str
                                                    )
                                                    and match_contact_operand
                                                    and match_contact_operand
                                                    == detected_3_outcoil
                                                ):
                                                    status = "OK"
                                                    final_rung_number = (
                                                        match_contact_row["RUNG"]
                                                    )
                                                    final_target_contact = (
                                                        match_contact_operand
                                                    )
...
def execute_rule_62_programwise(
    ...
    try:
        ...
        hmi_out_main_program_df = main_program_df[
            main_program_df["BODY"].str.lower() == hmi_out_section_name
        ]
        ...
        for program in unique_program_values:
            ...
            if is_task_name_in_range:
                ...
                cc4_result = check_detail_4_programwise(
                    hmi_out_main_program_df=hmi_out_main_program_df,
