...
check_detail_content = {
    "cc1": "The input variable to be checked by the program is used as an input variable of the comparison function (ZoneCmp,<,>,=).",
    "cc2": "The input variable to be checked by the program is an external variable (the variable name contains 'PD' or 'X').",
}

ng_content = {
    "cc1": "演算に使用しているデータの演算前チェックができていない可能性有 (There is a possibility that the data used in the calculation is not checked before the calculation.)",
...
def merge_program_function_csv_data(
    program_df: pd.DataFrame, function_df: pd.DataFrame
) -> pd.DataFrame:
    ...
    function_df_renamed = function_df.rename(
        columns={"FUNCTION_BLOCK": "PROGRAM", "BODY_TYPE": "BODY"}
    )
    ...
    merged_program_function_df = pd.concat(
        [program_df, function_df_renamed], ignore_index=True
    )
...
def check_detail_programwise(
    merged_program_function_df: pd.DataFrame,
    process_data_detection_range_df: pd.DataFrame,
):

    unique_proecss_data_detection_range_program = process_data_detection_range_df[
        "task_name"
    ].unique()
    ...
    for process_data_program_name in unique_proecss_data_detection_range_program:
        ...
        process_data_program_df = merged_program_function_df[
            merged_program_function_df["PROGRAM"] == process_data_program_name
        ]
        unique_section_block = process_data_program_df["BODY"].unique()
        for unique_section in unique_section_block:
            unique_section_df = process_data_program_df[
                process_data_program_df["BODY"] == unique_section
            ]
            unique_rung_block = unique_section_df["RUNG"].unique()
            for unique_rung in unique_rung_block:
                unique_rung_df = unique_section_df[
                    unique_section_df["RUNG"] == unique_rung
                ]
        ...
        current_program_process_data_df = process_data_detection_range_df[
            process_data_detection_range_df["task_name"] == process_data_program_name
        ]
        # 1. Filter out rows where Function_name contains "_TO_"
        remove_to_filtered_df = current_program_process_data_df[
            ~current_program_process_data_df["function_name"]
            .astype(str)
            .str.lower()
            .str.contains("_to_|move|@move")
        ]

        ## calling convert_block_data_to_custom_format to convert block data into strucutred format
        current_block_data = convert_block_data_to_custom_format(
            current_program_block_data, input_variable_comparison_function
        )
        ...
        for unique_section in unique_section_filter:
            ...
            seen_inputs = []
            section_wise_df = remove_to_filtered_df[
                remove_to_filtered_df["section"] == unique_section
            ]
                ...
                if sectionwise_all_input_data:
                    for data in sectionwise_all_input_data:
                        ...
                        if data not in seen_inputs:
                            ...
                            for index, (_, current_block_value) in enumerate(
                                current_block_data.items()
                            ):
...
def execute_rule_40_program_functionwise(
    input_program_file: str, input_function_file: str
):
    ...
    try:
        ...
        merge_program_function_names = (
            unique_program_names.tolist() + unique_function_names.tolist()
        )
        merged_program_function_df = merge_program_function_csv_data(
            program_df=program_df, function_df=function_df
        )
        ...
        for program_function_names in merge_program_function_names:
            all_program_function_data = []
            logger.info(
                f"Rule 40 processing program/function: {program_function_names}"
            )
            ...
            """
            program_function_filtered_df = merged_program_function_df[
                merged_program_function_df["PROGRAM"] == program_function_names
            ]
            ...
            if not program_function_filtered_df.empty:
                for section_name in unique_section_names:
                    logger.info(
                        f"Processing section_name: {section_name}, unique section {unique_section_names}"
                    )
                    section_filtered_df = program_function_filtered_df[
                        program_function_filtered_df["BODY"] == section_name
                    ]

                    unique_rung_number = section_filtered_df[
                        section_filtered_df["BODY"] == section_name
                    ]["RUNG"].unique()
                    for rung_number in unique_rung_number:
                        logger.info(f"Rung Number: {rung_number}")

                        rung_filtered_df = section_filtered_df[
                            section_filtered_df["RUNG"] == rung_number
                        ]
                        block_data = get_block_connections(rung_filtered_df)

                        if not block_data:
                            logger.warning(
                                f"No block data found for {program_function_names} in section {section_name} and rung {rung_number}"
                            )
                            continue
                        ...
                        for block in block_data:
                            for block_name, params in block.items():
                                ...
                                if (
                                    ...
                                    or (
                                        re.search("TO", block_name.upper())
                                        and not re.search("_", block_name)
                                    )
                                ):
                                    logger.info(
                                        f"Block data exact found for {program_function_names} in section {section_name} and rung {rung_number}, block_data {block_data}"
                                    )
                                    inputs = []
                                    outputs = []
                                        ...
                                        for param in params:
                                            for key, value in param.items():
                                                if key.lower().startswith("in"):
                                                    ...
                                                    "RightPowerRail" not in value
                                                    or "LeftPowerRail" not in value
                                                ):  # Handle empty keys separately if needed
                                                    outputs.extend(value)
                                    ...
                                    row = {
                                        ...
                                        "output1": (
                                            outputs[0] if len(outputs) > 0 else None
                                        ),
                                        "output2": (
                    ...
                    if all_program_function_data:
                        ...
                        current_program_data = flatten_singleton_lists(
                            current_program_data, input_cols + output_cols
                        )
                        for col in cols_to_clean:
                            current_program_data[col] = current_program_data[col].apply(
                        ...
                        remove_duplicate_df = remove_cross_row_duplicates(
                            current_program_data.copy()
                        )  # Step 1: Clean duplicates
                        replace_output_with_input_df = replace_to_outputs_with_inputs(
                            remove_duplicate_df
                        )  # Step 2: Replace using TO map
                        final_remove_duplicate_df = remove_cross_row_duplicates(
                            replace_output_with_input_df
                        )  # Step 3: Clean again after replacements
                        process_data_detection_range_list.append(
                            final_remove_duplicate_df
                        )

        """
        removing duplicated in there any occurs
        """
        process_data_detection_range_df = pd.concat(
            process_data_detection_range_list, ignore_index=True
        )
        process_data_detection_range_df = (
            process_data_detection_range_df.drop_duplicates()
        )
        """
        applying logic for check contect 1,2 as mentioned in rule after applying all filter processing
        """
        final_data_list = check_detail_programwise(
            merged_program_function_df=merged_program_function_df,
            process_data_detection_range_df=process_data_detection_range_df,
        )
