...
# ============================================ Comments referenced in Rule 27 processing ============================================
...
# ============================ Rule 27: Definitions, Content, and Configuration Details ============================
...
check_detail_content = {
    "cc1": "When ③ and ⑤ is not found in the detection target in ①, it is set to NG.",
...
ng_content = {
    "cc1_6": "ロボット搬送回路だが,記憶送り回路がコーディング基準に沿っていない(Robot transport circuit, but the memory feed circuit does not follow coding standards.)"
}
    ...
    for sublist in data:
        ...
        if last_item.get("operand") == target:
            ...
            simplified_sublist = [
                {
                    k: item[k]
                    for k in ["operand", "negated", "edge", "latch"]
                    if k in item
                }
                for item in sublist
            ]
...
def extract_rung_group_data_programwise(
    ...
    deviceout_section_name: str,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    logger.info(
        f"Group Rung and filter data on program {program_name} and section name {deviceout_section_name, memoryfeeding_section_name}"
    )
    ...
    deviceout_section_df = program_rows[
        program_rows["BODY"].str.lower() == deviceout_section_name.lower()
    ]
...
def detection_range_programwise(
    ...
    deviceout_rung_groups_df: pd.DataFrame,
    chuck_full_width_comment: str,
    chuck_half_width_comment: str,
    unchuck_full_width_comment: str,
    unchuck_half_width_comment: str,
    load_comment: str,
    unload_comment: str,
    memory_comment: str,
    timing_comment: str,
    discharge_comment: str,
    recieve_comment: str,
    program_name: str,
    ...
    detection_3_1_operand = detection_3_2_operand = detection_5_1_operand = (
        detection_5_2_operand
    ) = ""
    detection_3_1_rung_number = detection_3_2_rung_number = (
        detection_5_1_rung_number
    ) = detection_5_2_rung_number = -1
    ...
    for _, memory_feeding_rung_df in memory_feeding_rung_groups_df:
        ...
        for _, coil_rows in coil_df.iterrows():
            ...
            if isinstance(coil_comment, list) and has_set_coil == "set":
                if (
                    (
                        regex_pattern_check(chuck_full_width_comment, coil_comment)
                        or regex_pattern_check(chuck_half_width_comment, coil_comment)
                        or regex_pattern_check(unload_comment, coil_comment)
                    )
                    and regex_pattern_check(memory_comment, coil_comment)
                    and regex_pattern_check(timing_comment, coil_comment)
                    and all(
                        unchuck_full_width_comment not in comment
                        for comment in coil_comment
                    )
                    and all(
                        unchuck_half_width_comment not in comment
                        for comment in coil_comment
                    )
                    and not detection_3_1_operand
                ):
                ...
                if (
                    (
                        regex_pattern_check(unchuck_full_width_comment, coil_comment)
                        or regex_pattern_check(unchuck_half_width_comment, coil_comment)
                        or regex_pattern_check(load_comment, coil_comment)
                        or regex_pattern_check(discharge_comment, coil_comment)
                    )
                    and regex_pattern_check(memory_comment, coil_comment)
                    and regex_pattern_check(timing_comment, coil_comment)
                    and not detection_3_2_operand
                ):
    ...
    for _, deviceout_rung_df in deviceout_rung_groups_df:
        coil_df = deviceout_rung_df[
            deviceout_rung_df["OBJECT_TYPE_LIST"].str.lower() == "coil"
        ]
        for _, coil_rows in coil_df.iterrows():
            attr = ast.literal_eval(coil_rows["ATTRIBUTES"])
            coil_operand = attr.get("operand")
            ...
            if isinstance(coil_comment, list) and has_set_coil == "set":
                if (
                    ...
                    and regex_pattern_check(recieve_comment, coil_comment)
                    and all(
                        unchuck_full_width_comment not in comment
                        for comment in coil_comment
                    )
    ...
    return {
        "detection_3_1_details": [detection_3_1_operand, detection_3_1_rung_number],
...
def check_detail_1_programwise(detection_result: dict, program_name: str) -> dict:
    ...
    status = "OK"
    if (
        detection_result["detection_3_1_details"][1] == -1
        or detection_result["detection_3_2_details"][1] == -1
        or detection_result["detection_5_1_details"][1] == -1
        or detection_result["detection_5_2_details"][1] == -1
    ):
        status = "NG"

    cc1_result = {}
    all_outcoil_operand = [
        detection_result["detection_3_1_details"][0],
        detection_result["detection_3_2_details"][0],
        detection_result["detection_5_1_details"][0],
        detection_result["detection_5_2_details"][0],
    ]
    ...
    all_rung_number = [
        x - 1 if isinstance(x, int) and x > 0 else x
        for x in [
            detection_result["detection_3_1_details"][1],
            detection_result["detection_3_2_details"][1],
            detection_result["detection_5_1_details"][1],
            detection_result["detection_5_2_details"][1],
        ]
    ]
...
def check_detail_2_1_programwise(
    memory_feeding_rung_groups_df: pd.DataFrame,
    detection_3_1_operand: str,
    program_name: str,
) -> dict:
    ...
    if detection_3_1_operand:
        ...
        for _, memory_feeding_df in memory_feeding_rung_groups_df:
            ...
            for _, contact_row in contact_df.iterrows():
                ...
                if (
                    attr.get("operand") == detection_3_1_operand
...
def check_detail_2_2_programwise(
    memory_feeding_rung_groups_df: pd.DataFrame,
    detection_3_2_operand: str,
    program_name: str,
) -> dict:
    ...
    if detection_3_2_operand:
        ...
        for _, memory_feeding_df in memory_feeding_rung_groups_df:
            ...
            for _, contact_row in contact_df.iterrows():
                ...
                rising_edge = attr.get("edge", None)
                if (
                    attr.get("operand") == detection_3_2_operand
                    and rising_edge == "rising"
                ):
...
def check_detail_3_1_programwise(
    memory_feeding_rung_groups_df: pd.DataFrame,
    detection_3_1_rung_number: str,
    chuck_full_width_comment: str,
    chuck_half_width_comment: str,
    unchuck_full_width_comment: str,
    unchuck_half_width_comment: str,
    ...
    if detection_3_1_rung_number != -1:
        ...
        current_rung_details = memory_feeding_rung_groups_df.get_group(
            detection_3_1_rung_number
        ).copy()

        ## getting all contact from autorun section
        contact_df = current_rung_details[
            current_rung_details["OBJECT_TYPE_LIST"] == "Contact"
        ]
        ...
        for _, contact_row in contact_df.iterrows():
            ...
            if contact_operand and isinstance(contact_operand, str):
                if "." in contact_operand:
                    contact_operand = contact_operand.replace(".", "")
            ...
            if rising_edge == "rising" and isinstance(contact_comment, list):
                if (
                    (
                        regex_pattern_check(chuck_full_width_comment, contact_comment)
                        or regex_pattern_check(
                            chuck_half_width_comment, contact_comment
                        )
                        or regex_pattern_check(unload_comment, contact_comment)
                    )
                    and regex_pattern_check(complete_comment, contact_comment)
                    and all(
                        unchuck_full_width_comment not in comment
                        for comment in contact_comment
                    )
                    and all(
                        unchuck_half_width_comment not in comment
                        for comment in contact_comment
...
def check_detail_3_2_programwise(
    memory_feeding_rung_groups_df: pd.DataFrame,
    detection_3_2_rung_number: str,
    unchuck_full_width_comment: str,
    unchuck_half_width_comment: str,
    load_comment: str,
    discharge_comment: str,
    ...
    if detection_3_2_rung_number >= 0:
        ...
        for _, contact_row in contact_df.iterrows():
            ...
            if rising_edge == "rising" and isinstance(contact_comment, list):
                if (
                    regex_pattern_check(unchuck_full_width_comment, contact_comment)
                    or regex_pattern_check(unchuck_half_width_comment, contact_comment)
                    or regex_pattern_check(load_comment, contact_comment)
                    or regex_pattern_check(discharge_comment, contact_comment)
                ) and regex_pattern_check(complete_comment, contact_comment):
                    return {
                        "cc": "cc3_2",
                        "status": "OK",
...
def check_detail_4_1_programwise(
    memory_feeding_rung_groups_df: pd.DataFrame,
    detection_3_1_operand: str,
    detection_3_2_operand: str,
    detection_3_2_rung_number: int,
    program_name: str,
) -> dict:
    ...
    if detection_3_1_operand and detection_3_2_operand:
        ...
        detection_3_2_rung_number_details = memory_feeding_rung_groups_df.get_group(
            detection_3_2_rung_number
        )
        detection_3_2_rung_number_details_polar = pl.from_pandas(
            detection_3_2_rung_number_details
        )
        detection_3_2_get_series_data = get_series_contacts_coil(
            detection_3_2_rung_number_details_polar
        )
        detection_3_2_condition = extract_first_sublist_with_last_operand(
            detection_3_2_get_series_data, target=detection_3_2_operand
        )
        ...
        for _, memory_rung_df in memory_feeding_rung_groups_df:
            ...
            for _, coil_row in coil_df.iterrows():
                ...
                if coil_operand == detection_3_1_operand and has_reset_coil == "reset":
                    detection_3_1_reset_coil_rung_detail = (
                        memory_feeding_rung_groups_df.get_group(coil_row["RUNG"])
                    )
                    detection_3_1_reset_coil_rung_detail_polar = pl.from_pandas(
                        detection_3_1_reset_coil_rung_detail
                    )
                    detection_3_1_get_series_data = get_series_contacts_coil(
                        detection_3_1_reset_coil_rung_detail_polar
                    )
                    detection_3_1_condition = extract_first_sublist_with_last_operand(
                        detection_3_1_get_series_data, target=detection_3_1_operand
                    )
                    detection_3_1_3_2_condition_status = conditions_match_except_last(
                        detection_3_1_condition, detection_3_2_condition
                    )
...
def check_detail_4_2_programwise(
    memory_feeding_rung_groups_df: pd.DataFrame,
    detection_3_1_operand: str,
    detection_3_2_operand: str,
    detection_3_1_rung_number: int,
    program_name: str,
) -> dict:
    ...
    if detection_3_1_operand and detection_3_2_operand:
        detection_3_1_rung_number_details = memory_feeding_rung_groups_df.get_group(
            detection_3_1_rung_number
        )
        detection_3_1_rung_number_details_polar = pl.from_pandas(
            detection_3_1_rung_number_details
        )
        detection_3_1_get_series_data = get_series_contacts_coil(
            detection_3_1_rung_number_details_polar
        )
        detection_3_1_condition = extract_first_sublist_with_last_operand(
            detection_3_1_get_series_data, target=detection_3_1_operand
        )
        ...
        for _, memory_rung_df in memory_feeding_rung_groups_df:
            ...
            for _, coil_row in coil_df.iterrows():
                ...
                if coil_operand == detection_3_2_operand and has_reset_coil == "reset":
                    detection_3_2_reset_coil_rung_detail = (
                        memory_feeding_rung_groups_df.get_group(coil_row["RUNG"])
                    )
                    detection_3_2_reset_coil_rung_detail_polar = pl.from_pandas(
                        detection_3_2_reset_coil_rung_detail
                    )
                    detection_3_2_get_series_data = get_series_contacts_coil(
                        detection_3_2_reset_coil_rung_detail_polar
                    )
                    detection_3_2_condition = extract_first_sublist_with_last_operand(
                        detection_3_2_get_series_data, target=detection_3_2_operand
                    )
                    detection_3_1_3_2_condition_status = conditions_match_except_last(
                        detection_3_2_condition, detection_3_1_condition
                    )
                    if detection_3_1_3_2_condition_status:
                        return {
                            "cc": "cc4_2",
...
def check_detail_5_1_programwise(
    memory_feeding_rung_groups_df: pd.DataFrame,
    detection_3_1_operand: str,
    robot_full_width_comment: str,
    robot_half_width_comment: str,
    specified_comment: str,
    home_comment: str,
    pos_comment: str,
    program_name: str,
            ...
            for _, coil_row in coil_df.iterrows():
                ...
                if coil_operand == detection_3_1_operand and has_reset_coil == "reset":
                    ...
                    for _, current_contact_row in current_contact_df.iterrows():
                        contact_attr = ast.literal_eval(
                            current_contact_row["ATTRIBUTES"]
                        )
                        ...
                        if contact_operand and isinstance(contact_operand, str):
                            ...
                            if isinstance(contact_comment, list):
                                if (
                                    (
                                        regex_pattern_check(
                                            robot_full_width_comment, contact_comment
                                        )
                                        or regex_pattern_check(
                                            robot_half_width_comment, contact_comment
                                        )
                                        or regex_pattern_check("RB", contact_comment)
                                    )
                                    and (
                                        regex_pattern_check(
                                            specified_comment, contact_comment
                                        )
                                        or regex_pattern_check(
                                            home_comment, contact_comment
                                        )
                                    )
                                    and regex_pattern_check(
                                        pos_comment, contact_comment
                                    )
                                ):
                                    return {
                                        "cc": "cc5_1",
...
def check_detail_5_2_programwise(
    memory_feeding_rung_groups_df: pd.DataFrame,
    detection_3_2_operand: str,
    robot_full_width_comment: str,
    robot_half_width_comment: str,
...
def check_detail_6_1_programwise(
    deviceout_rung_groups_df: pd.DataFrame,
    detection_3_1_operand: str,
    chuck_full_width_comment: str,
    chuck_half_width_comment: str,
    ...
    if detection_3_1_operand:
        ...
        for _, deviceout_rung_df in deviceout_rung_groups_df:
            contact_df = deviceout_rung_df[
                deviceout_rung_df["OBJECT_TYPE_LIST"] == "Contact"
            ]
            for _, contact_row in contact_df.iterrows():
                ...
                for _, coil_row in coil_df.iterrows():
                    ...
                    if isinstance(coil_comment, list):
                        if (
                            (
                                regex_pattern_check(
                                    chuck_full_width_comment, coil_comment
                                )
                                or regex_pattern_check(
                                    chuck_half_width_comment, coil_comment
                                )
                            )
                            and regex_pattern_check(recieve_comment, coil_comment)
                            and all(
                ...
                """
                if (
                    contact_operand == detection_3_1_operand
                    and has_receive_chuck_comment
                ):
...
def check_detail_6_2_programwise(
    deviceout_rung_groups_df: pd.DataFrame,
    detection_3_2_operand: str,
    unchuck_full_width_comment: str,
    unchuck_half_width_comment: str,
            ...
            for _, contact_row in contact_df.iterrows():
                ...
                for _, coil_row in coil_df.iterrows():
                    ...
                    if isinstance(coil_comment, list):
                        if (
                            regex_pattern_check(
                                unchuck_full_width_comment, coil_comment
                            )
                            or regex_pattern_check(
                                unchuck_half_width_comment, coil_comment
                            )
                        ) and regex_pattern_check(recieve_comment, coil_comment):
                ...
                """
                if (
                    contact_operand == detection_3_2_operand
                    and has_receive_unchuck_comment
                ):
                    contact_out_list = contact_attr["out_list"]
                    coil_in_list = coil_attr["in_list"]
...
def store_program_csv_results_programwise(
    ...
    for _, cc_status in enumerate(all_cc_status):
        ...
        rung_number = (
            rung_raw - 1
            if isinstance(rung_raw, int) and rung_raw >= 0
            else rung_raw if isinstance(rung_raw, list) else -1
        )
...
def execute_rule_27_programwise(
    ...
    try:
        ...
        for program_name in unique_program_values:
            ...
            if program_name.lower() in task_names:
                ...
                memory_feeding_rung_groups_df, deviceout_rung_groups_df = (
                ...
                # Run detection range logic as per Rule 27
                ...
                cc2_1_result = check_detail_2_1_programwise(
                    memory_feeding_rung_groups_df=memory_feeding_rung_groups_df,
                    detection_3_1_operand=detection_3_1_operand,
                    program_name=program_name,
                )
                cc2_2_result = check_detail_2_2_programwise(
                    memory_feeding_rung_groups_df=memory_feeding_rung_groups_df,
                    detection_3_2_operand=detection_3_2_operand,
                    program_name=program_name,
                )
                cc3_1_result = check_detail_3_1_programwise(
                    memory_feeding_rung_groups_df=memory_feeding_rung_groups_df,
                    detection_3_1_rung_number=detection_3_1_rung_number,
                    chuck_full_width_comment=chuck_full_width_comment,
                    chuck_half_width_comment=chuck_half_width_comment,
                    unchuck_full_width_comment=unchuck_full_width_comment,
                    unchuck_half_width_comment=unchuck_half_width_comment,
                ...
                cc3_2_result = check_detail_3_2_programwise(
                    memory_feeding_rung_groups_df=memory_feeding_rung_groups_df,
                    detection_3_2_rung_number=detection_3_2_rung_number,
                    unchuck_full_width_comment=unchuck_full_width_comment,
                    unchuck_half_width_comment=unchuck_half_width_comment,
                    load_comment=load_comment,
                    discharge_comment=discharge_comment,
                ...
                cc4_1_result = check_detail_4_1_programwise(
                    memory_feeding_rung_groups_df=memory_feeding_rung_groups_df,
                    detection_3_1_operand=detection_3_1_operand,
                    detection_3_2_operand=detection_3_2_operand,
                    detection_3_2_rung_number=detection_3_2_rung_number,
                ...
                cc4_2_result = check_detail_4_2_programwise(
                    memory_feeding_rung_groups_df=memory_feeding_rung_groups_df,
                    detection_3_1_operand=detection_3_1_operand,
                    detection_3_2_operand=detection_3_2_operand,
                    detection_3_1_rung_number=detection_3_1_rung_number,
                ...
                cc5_1_result = check_detail_5_1_programwise(
                    memory_feeding_rung_groups_df=memory_feeding_rung_groups_df,
                    detection_3_1_operand=detection_3_1_operand,
                    robot_full_width_comment=robot_full_width_comment,
                    robot_half_width_comment=robot_half_width_comment,
                    specified_comment=specified_comment,
                    home_comment=home_comment,
                    pos_comment=pos_comment,
                    program_name=program_name,
                    program_comment_data=program_comment_data,
                )
                cc5_2_result = check_detail_5_2_programwise(
                    memory_feeding_rung_groups_df=memory_feeding_rung_groups_df,
                    detection_3_2_operand=detection_3_2_operand,
                    robot_full_width_comment=robot_full_width_comment,
                    robot_half_width_comment=robot_half_width_comment,
                ...
                cc6_1_result = check_detail_6_1_programwise(
                    deviceout_rung_groups_df=deviceout_rung_groups_df,
                    detection_3_1_operand=detection_3_1_operand,
                    chuck_full_width_comment=chuck_full_width_comment,
                    chuck_half_width_comment=chuck_half_width_comment,
                ...
                cc6_2_result = check_detail_6_2_programwise(
                    deviceout_rung_groups_df=deviceout_rung_groups_df,
                    detection_3_2_operand=detection_3_2_operand,
                    unchuck_full_width_comment=unchuck_full_width_comment,
                    unchuck_half_width_comment=unchuck_half_width_comment,
