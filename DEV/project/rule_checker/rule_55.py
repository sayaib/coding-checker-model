...
check_detail_content = {"cc1":" If there is no ④ in the rung of rung No. 0, then NG.", 
...
ng_content = {"cc1":"Fault'セクションであるのに、異常リセット回路が存在しないのでNG(NG because 'Fault' section, but there is no fault reset circuit.)", 
...
def is_valid_contact_pair_programwise(row_a, row_b, clear_row, fault_reset_comment, autorun_comment,  program_name, program_comment_data):
    ...
    comment_a = get_the_comment_from_program(operand_a, program_name, program_comment_data)
    comment_b = get_the_comment_from_program(operand_b, program_name, program_comment_data)
    ...
    is_a_fault_reset = regex_pattern_check(fault_reset_comment, comment_a) and negated_a == 'false'
    is_b_autorun = regex_pattern_check(autorun_comment, comment_b) and negated_b == 'true'

    return is_a_fault_reset and is_b_autorun

def is_valid_contact_pair_functionwise(row_a, row_b, clear_row, fault_reset_comment, autorun_comment,  program_name, program_comment_data):
    attr_a = ast.literal_eval(row_a['ATTRIBUTES'])
    attr_b = ast.literal_eval(row_b['ATTRIBUTES'])
    attr_clear = ast.literal_eval(clear_row['ATTRIBUTES'])
...
def detection_range_programwise(fault_section_first_rung_df: pd.DataFrame, program_name:str) -> dict:
    
    logger.info(f"Executing rule 55 detection range on program {program_name}")

    first_rung_fault_section_block_df = fault_section_first_rung_df[fault_section_first_rung_df['OBJECT_TYPE_LIST'] == 'Block']
    if not first_rung_fault_section_block_df.empty:
        for _, first_rung_all_block_row in first_rung_fault_section_block_df.iterrows():
            ...
            if block_type and isinstance(block_type,str) and block_type.lower() == 'clear':
                return {
                        "status":"OK",
                        "rung_number" : first_rung_all_block_row['RUNG']
                    }
    return {
        "status" : "NG",
        "rung_number" : -1
...
def check_detail_2_programwise(fault_section_first_rung_df:pd.DataFrame) -> dict:
    ...
    if isinstance(input_val, list) and input_val and isinstance(output_val, list) and output_val:
...
def check_detail_3_programwise(fault_section_first_rung_df:pd.DataFrame, cc2_status:str, fault_reset_comment:str, autorun_comment:str,  program_name:str, program_comment_data:str) -> dict:
    ...
    try:
        if cc2_status == "OK" and not contact_block_first_rung_df.empty:
            row0, row1, row2 = contact_block_first_rung_df.iloc[0], contact_block_first_rung_df.iloc[1], contact_block_first_rung_df.iloc[2]

            if row0['OBJECT_TYPE_LIST'] == 'Contact' and row1['OBJECT_TYPE_LIST'] == 'Contact' and row2['OBJECT_TYPE_LIST'] == 'Block':
                attr_clear = ast.literal_eval(row2['ATTRIBUTES'])
                if attr_clear.get('typeName', '').lower() == 'clear':
                    # Check both permutations of contacts
                    if is_valid_contact_pair_programwise(row0, row1, row2, fault_reset_comment, autorun_comment, program_name, program_comment_data) or is_valid_contact_pair_programwise(row1, row0, row2, fault_reset_comment, autorun_comment,  program_name, program_comment_data):
...
def detection_range_functionwise(fault_section_first_rung_df: pd.DataFrame, function_name:str) -> dict:
...
def check_detail_3_functionwise(fault_section_first_rung_df:pd.DataFrame, cc2_status:str, fault_reset_comment:str, autorun_comment:str,  function_name:str, function_comment_data:str) -> dict:
    ...
    try:
        if cc2_status == "OK" and not contact_block_first_rung_df.empty:
            ...
            if row0['OBJECT_TYPE_LIST'] == 'Contact' and row1['OBJECT_TYPE_LIST'] == 'Contact' and row2['OBJECT_TYPE_LIST'] == 'Block':
                ...
                if attr_clear.get('typeName', '').lower() == 'clear':
                    # Check both permutations of contacts
                    if is_valid_contact_pair_functionwise(row0, row1, row2, fault_reset_comment, autorun_comment,  function_name, function_comment_data) or is_valid_contact_pair_functionwise(row1, row0, row2, fault_reset_comment, autorun_comment, function_name, function_comment_data):
                        is_contact_block_series = True
                        rung_number = row0['RUNG']
                    else:
                        is_contact_block_series = False
...
def execute_rule_55_programwise(input_program_file:str, 
    ...
    try:
        ...
        for program_name in unique_program_values:
            ...
            unique_section_name = [val.lower() for val in current_program_df['BODY'].unique()]
            ...
            if "fault" in unique_section_name:
                ...
                if not fault_section_df.empty:
                    fault_section_first_rung_df = fault_section_df[fault_section_df['RUNG']==1]
                    ...
                    if detection_result['status'] == "OK":
                        ...
                        cc2_result = check_detail_2_programwise(fault_section_first_rung_df=fault_section_first_rung_df)
                        cc2_status = cc2_result['status']
                        cc3_result = check_detail_3_programwise(fault_section_first_rung_df=fault_section_first_rung_df, cc2_status=cc2_status, fault_reset_comment=fault_reset_comment, autorun_comment=autorun_comment,  program_name=program_name, program_comment_data=program_comment_data)
...
def execute_rule_55_functionwise(input_function_file:str, 
    ...
    try:
        ...
        for function_name in unique_function_values:
            ...
            unique_section_name = [val.lower() for val in current_function_df['BODY'].unique()]
            ...
            if "fault" in unique_section_name:
                ...
                if not fault_section_df.empty:
                    ...
                    if detection_result['status'] == "OK":
                        ...
                        cc2_result = check_detail_2_functionwise(fault_section_first_rung_df=fault_section_first_rung_df)
                        cc2_status = cc2_result['status']
                        cc3_result = check_detail_3_functionwise(fault_section_first_rung_df=fault_section_first_rung_df, cc2_status=cc2_status, fault_reset_comment=fault_reset_comment, autorun_comment=autorun_comment,  function_name=function_name, function_comment_data=function_comment_data)
