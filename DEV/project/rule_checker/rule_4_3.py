...
from .rule_4_3_ladder_utils import (
...
def check_chuck_operation_condition_from_program(
    row,
    chuck_comment: str,
    unchuck_comment: str,
    ...
    try:
        ...
        if isinstance(attr, dict) and "operand" in attr:
            ...
            if isinstance(comment, list):
                ...
                unchuck_comment = "".join(
                    full_to_half_conversion.get(char, char) for char in unchuck_comment
                )
                comment = [
                ...
                if (
                    regex_pattern_check(chuck_comment, comment)
                    and regex_pattern_check(operation_comment, comment)
                    and regex_pattern_check(condition_comment, comment)
                    and unchuck_not_present_status
                ):
...
def check_unchuck_operation_condition_from_program(
    row,
    chuck_comment: str,
    unchuck_comment: str,
    operation_comment: str,
    ...
    try:
        ...
        if isinstance(attr, dict) and "operand" in attr:
            if isinstance(attr.get("operand"), str):
                ...
                if isinstance(comment, list):
                    if (
                        regex_pattern_check(unchuck_comment, comment)
                        and regex_pattern_check(operation_comment, comment)
                        and regex_pattern_check(condition_comment, comment)
                    ):
                        return attr.get("operand")
...
def detection_range_programwise(
    ...
    logger.info(
        f"Detection Range function for program: {program_name}, section: {section_name}"
    )
    ...
    for _, rung_df in memory_feeding_rung_groups:
        ...
        rung_df["chuck_operation_condition"] = rung_df.apply(
            lambda row: (
                check_chuck_operation_condition_from_program(
                    row=row,
                    chuck_comment=chuck_comment,
                    unchuck_comment=unchuck_comment,
        ...
        chuck_operation_condition_match_outcoil = rung_df[
            rung_df["chuck_operation_condition"].notna()
        ]
        if (
            not chuck_operation_condition_match_outcoil.empty
        ) and chuck_operation_condition_operand is None:
            chuck_operation_condition_operand = (
                chuck_operation_condition_match_outcoil.iloc[0][
                    "chuck_operation_condition"
                ]
            )  # First match
            chuck_operation_condition_rung_number = (
                chuck_operation_condition_match_outcoil.iloc[0]["RUNG"]
            )

        rung_df["unchuck_operation_condition"] = rung_df.apply(
            lambda row: (
                check_unchuck_operation_condition_from_program(
                    row=row,
                    chuck_comment=chuck_comment,
                    unchuck_comment=unchuck_comment,
                    operation_comment=operation_comment,
        ...
        unchuck_operation_condition_match_outcoil = rung_df[
            rung_df["unchuck_operation_condition"].notna()
        ]
        if (
            not unchuck_operation_condition_match_outcoil.empty
        ) and unchuck_operation_condition_operand is None:
            unchuck_operation_condition_operand = (
                unchuck_operation_condition_match_outcoil.iloc[0][
                    "unchuck_operation_condition"
                ]
            )  # First match
            unchuck_operation_condition_rung_number = (
                unchuck_operation_condition_match_outcoil.iloc[0]["RUNG"]
            )

    return {
        "chuck_unchuck_outcoil_operand": [
            chuck_operation_condition_operand,
            unchuck_operation_condition_operand,
        ],
        "chuck_unchuck_outcoil_rung_number": [
            int(chuck_operation_condition_rung_number) - 1,
            int(unchuck_operation_condition_rung_number) - 1,
        ],
...
def check_detail_1_programwise(detection_result: dict, program_name: str) -> dict:
    ...
    chuck_unchuck_rung_number = detection_result.get(
        "chuck_unchuck_outcoil_rung_number"
    )

    for chuck_type, operand, rung_number in zip(
        chuck_unchuck_type, chuck_unchuck_operands, chuck_unchuck_rung_number
    ):
        cc1_result.append(
            {
                "cc": "cc1",
    ...
    cc1_result.append(
        {
            ...
            "status": (
                "NG"
                if not chuck_unchuck_operands
                or any(x is None for x in chuck_unchuck_operands)
                else "OK"
            ),
            "chuck_type": None,
            "check_number": 1,
...
def check_detail_2_programwise(
    ...
    for chuck_type, chuck_unchuck_operand, rung_number in zip(
        chuck_unchuck_type, chuck_unchuck_operands, chuck_unchuck_rung_number
    ):
        chuck_contact_operand_found = False
        if chuck_type == "chuck":
            if chuck_unchuck_operand is not None:
                current_memory_feeding_rung_df = memory_feeding_rung_groups.get_group(
                    rung_number + 1
                ).copy()
                    ...
                    if row["OBJECT_TYPE_LIST"] == "Contact":
                        ...
                        if isinstance(contact_comment, list):
                            if regex_pattern_check(
                                unchuck_comment, contact_comment
                                ...
                                """
                                current_rung_parellel_check_df = (
                                    current_memory_feeding_rung_df[
                                        current_memory_feeding_rung_df["RUNG"]
                                        == row["RUNG"]
                                    ]
                                )
                                all_parellel_contact = get_parallel_contacts(
                                    pl.from_pandas(current_rung_parellel_check_df)
                                )
                                if all_parellel_contact:
                                    structured_parellel_contact = (
                                        get_format_parellel_contact_detail(
                                            all_parellel_contact
                                        )
                                    )
                                else:
                                    structured_parellel_contact = {}
                                ...
                                if (
                                    contact_operand
                                    not in structured_parellel_contact.keys()
                                ):
                                    cc2_result.append(
                                        {
                                            "cc": "cc2_1",
            ...
            if chuck_unchuck_operand is not None:
                ...
                for _, row in current_memory_feeding_rung_df.iterrows():
                    if row["OBJECT_TYPE_LIST"] == "Contact":
                        ...
                        if isinstance(contact_comment, list):
                            unchuck_comment = "".join(
                                full_to_half_conversion.get(char, char)
                                for char in unchuck_comment
                            )
                            ...
                            if (
                                regex_pattern_check(chuck_comment, contact_comment)
                                and regex_pattern_check(end_comment, contact_comment)
                                and unchuck_not_present_status
                            ):
...
def check_detail_programwise(
    ...
    logger.info(
        f"Checking details for program: {program_name}, section: {section_name}"
    )
...
def store_program_csv_results(
    ...
    for _, cc_results in enumerate(all_cc_status):
        for cc_result in cc_results:
            ...
            rung_number = (
                cc_result.get("rung") - 1 if cc_result.get("rung") != -1 else -1
            )
            outcoil = (
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            if program.lower() in task_names:
                ...
                try:
                    logger.info(
                        f"Filter rung based on program name {program} and section name {section_name}"
                    )
                    ...
                    if len(memory_feeding_rung_groups) != 0:
                        ...
                        output_rows = store_program_csv_results(
