...
from .rule_2_ladder_utils import get_block_connections, check_self_holding, get_in_parallel_A_contacts, get_parallel_contacts
...
def process_memory_feeding(ladder_df:pd.DataFrame, 
                           program_key:str, 
                           body_type_key:str)->Dict[str, Any]:
    ...
    for program in unique_program_values:
        ...
        ladder_memory_feeding=ladder_program.filter(pl.col(body_type_key).cast(pl.Utf8).str.to_lowercase() == 'memoryfeeding')

        #Get the outcoils
        out_coil_df=ladder_memory_feeding.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == 'coil')
        outcoil_rung_list=list(out_coil_df['RUNG'].unique())

        #Get the contacts
        contact_df=ladder_memory_feeding.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == 'contact')
        ...
        for outcoil_rung in outcoil_rung_list:
            ...
            for attr_ in  outcoil_rung_attributes_list:
                ...
                for contact_attr in contact_rung_attributes_list:
                    ...
                    if (coil_attr_latch !='NONE') and (contact_negated_status !='NONE') and (contact_negated_status =='false'):

                        #If the coil and contact are connected then not the contact
                        if set(coil_in_list) & set(contact_out_list):


                            outcoil_config_dict[f"{program}@{outcoil_rung}@{coil_operand}"]=[]
                            outcoil_config_dict[f"{program}@{outcoil_rung}@{coil_operand}"].append(contact_attr)
                            outcoil_config_dict[f"{program}@{outcoil_rung}@{coil_operand}"].append(coil_attr)
...
def condtion_section_B_contacts(ladder_df:pd.DataFrame, 
                                program_key:str, 
                                body_type_key:str)->Dict[str, Any]:
    ...
    for program in unique_program_values:
        ...
        ladder_condition=ladder_program.filter(pl.col(body_type_key).cast(pl.Utf8).str.to_lowercase() == 'condition')

        contact_df=ladder_condition.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == 'contact')
        ...
        for rung in contact_rung_list:
            ...
            for attr_ in contact_attributes:
                ...
                if (contact_negated_status!="NONE") and (contact_negated_status=="true"):

                    B_contact_dict[f'{program}@{rung}@{contact_operand}']=[]

                    B_contact_dict[f'{program}@{rung}@{contact_operand}'].append(contact_attr)


    return(B_contact_dict)

######################## create Dataframes ####################
def create_dataframes(task_name:str, section_name:str, rule_number:str, check_number:str, rule_content:str, status:str,
                      details:Dict, NG_exp_en:str, NG_exp_jp:str)-> List:
    ...
    output_dict={'Result':[], 'Task':[], 'Section': [], 'RungNo':[], "Target":[], 'CheckItem':[], 'Detail':[], 'Status': []}
    ...
    sub_df_en={'Result':[status], 'Task':[task_name], 'Section': [section_name], 
                    'RungNo':[details.get('rung', '')], "Target":[details], 
                    'CheckItem':rule_2_check_item,
                    'Detail': [NG_exp_en],  'Status': ['']}
    ...
    sub_df_jp={'Result':[status], 'Task':[task_name], 'Section': [section_name], 
                    'RungNo':[details.get('rung', '')], "Target":[details], 
                    'CheckItem':rule_2_check_item,
                    'Detail': [NG_exp_jp],  'Status': ['']}
...
def Rule_2_Check_2_program(ladder_df:pd.DataFrame, main_ladder:pd.DataFrame, ref_rung:int):
    logger.debug(f"Checking for Condition 2 Rule 2 {ref_rung}")
    gsb_regex=r"GSB"
    ld_regex=r"LD"
    class BreakAll(Exception): pass

    output_dict={}

    #First get the all the contacts, excluding GSB
    ladder_contacts=ladder_df.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase()=='contact')
    ...
    try:
        ...
        logger.info(f"Breaking early, Variable:{block_LD}, type: {block_key}  obtained ")
    ...
    if re.search(ld_regex, block_LD):
        ...
        try:
            ...
            logger.info(f"Breaking early, LD type is found in Rung: {move_ld_rung}, Input:{move_ld_inp} obtained ")


        if move_block_ld_flag==0:
            output_dict['status']='NG'
            output_dict['NG_explanantion']=f'LD type input {block_LD} was not found in output of MOVE block'
            output_dict['check_number']='5.4'
            output_dict['ref_rung']=f'{ref_rung}'

            logger.warning(f"LD type input {block_LD} was not found in output of MOVE block")
            return output_dict
        ...
        if (move_ld_rung!=''):
            ...
            try:
                        ...
                        if rung_block_key in type_list:
                            ...
                            if rung_block_key==block_key:
                                ...
                                for block_val in block_values:
                                    ...
                                    if all([In_2_flag==1, In_1_flag==1, In_1_val==move_ld_inp,In_2_val=='INT#0'  ]):
                ...
                logger.info(f"Breaking early, Variable :{move_ld_inp} set to zero Rung:{move_ld_rst_rung} obtained ")


        if move_ld_rst_flag==0:
            output_dict['status']='NG'
            output_dict['NG_explanantion']=f'Variable :{move_ld_inp} was not set to Zero'
        ...
        if  move_ld_rst_flag==1:
            ...
            series_rung_contacts=series_rung_df.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase()=='contact')
            ...
            for attr_ in contact_attributes:
                ...
                if all([attr_operand in contacts_list, attr_operand_negated_status!='NONE', attr_operand_negated_status=='true' ]):
        ...
        if series_B_contacts_flag==0:
            output_dict['status']='NG'
            output_dict['NG_explanantion']=f'All the Contacts were not associated with B counterparts'
        ...
        if series_B_contacts_flag==1:
            ...
            logger.info(f"ll the Contacts were associated with B counterparts in target rung:{move_ld_rst_rung}")
            return output_dict
...
def execute_rule_2(input_file : str, 
                   input_program_comment_file:str, 
                   program_key:str, 
                   body_type_key:str) -> pd.DataFrame:

    print("in execute_rule_2")
    logger.warning("======================================Executing Rule 2===========================")
    ...
    NG_exp_condition_en="Failure to perform OFF check of judgment memory before judgment"
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            out_coil_df=ladder_autorun.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == 'coil')
            outcoil_rung_list=list(out_coil_df['RUNG'])
            outcoil_attributes_list=list(out_coil_df['ATTRIBUTES'])

            for rung_num, coil_attribute in zip(outcoil_rung_list, outcoil_attributes_list):
                ...
                coil_comment_dict=get_the_comment_from_program(variable=coil_operand, program=program, input_comment_data=comment_data)

                #Chcek for cycle start and cycle end, create lists for each one of those, cycle start and cycle end
                if regex_pattern_check(cycle_half_jp, coil_comment_dict) or regex_pattern_check(cycle_full_jp, coil_comment_dict):
        ...
        for program in program_range_dict.keys():
            ...
            if (len(start_list)!=0 )and (len(end_list)!=0):
                logger.warning(f"Ranges detected for program: {program} start_list: {start_list}  end_list: {end_list}")
            if len(start_list)!=len(end_list):
                logger.warning(f"Open Ended Range: {program} start_list:{start_list} end_list:{end_list}")
                ...
                sub_df_en={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                    'RungNo':[""], "Target":[detail_dict], 
                    'CheckItem':rule_2_check_item,
                    'Detail': ['Open Ended Range'],  'Status': ['']}
                    ...
                    for start_rung, end_rung in zip(start_list, end_list):
                        ...
                        if int(end_rung)> int(start_rung):
                            ...
                            out_coil_df_rung_range=out_coil_df.filter(
                                                                    (pl.col("RUNG") >= start_rung) & (pl.col("RUNG") <= end_rung)
                            ...
                            rungs_in_the_range=list(ladder_program_for_rungs_in_range['RUNG'].unique())
                            rungs_in_the_range.sort(reverse=True)


                            out_coil_per_rung_frequency = sorted(
                                out_coil_df_rung_range
                                .select(pl.col("RUNG").value_counts())
                                .unnest("RUNG")
                            ...
                            rungs_with_two_or_more_outcoils=[ele['RUNG'] for ele in out_coil_per_rung_frequency]

                            logger.warning(f"Rungs with >2 coils: {rungs_with_two_or_more_outcoils} in program:{program}")
                            ...
                            for rung_with_two_or_more_outcoils in rungs_with_two_or_more_outcoils:

                                df_rung_for_check=ladder_program_for_rungs_in_range.filter(
                                                    (pl.col("RUNG") == rung_with_two_or_more_outcoils)
                                ...
                                outcoil_for_check_attributes=list(df_rung_outcoil_for_check['ATTRIBUTES'])
                                ...
                                for outcoil_attribute in outcoil_for_check_attributes:
                                    ...
                                    comment_list=get_the_comment_from_program(variable=coil_operand, program=program, input_comment_data=comment_data)

                                    comment_patterns_to_checked=[ok_jp, normal_jp, ng_jp, abnormal_jp]
                                    ...
                                    for pattern_ in comment_patterns_to_checked:
                                        ...
                                        if regex_pattern_check(pattern_, comment_list):
                                            ...
                                            if pattern_==normal_jp:
                                                normal_coil_operands.append(coil_operand)
                                            ...
                                            if pattern_==abnormal_jp:
                                                abnormal_coil_operands.append(coil_operand)



                            #Once the Ok/NG operands are extracted, begin the next step
                            logger.warning(f"program:{program} Coil Operands in Autorun \n Ok:{ok_coil_operands},\n normal:{normal_coil_operands},\n NG:{ng_coil_operands}, \n Abnormal:{abnormal_coil_operands}")
                            memory_feeding_attributes=list(ladder_memory_feeding['ATTRIBUTES'])
                            memory_feeding_objects=list(ladder_memory_feeding['OBJECT_TYPE_LIST'])
                            memory_feeding_rungs=list(ladder_memory_feeding['RUNG'].unique())
                            ...
                            for rung in memory_feeding_rungs:
                                ...
                                ladder_mem_rung=ladder_memory_feeding.filter(pl.col("RUNG") == rung)

                                mem_rung_coils=ladder_mem_rung.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == 'coil')
                                mem_rung_coils_attributes=list(mem_rung_coils['ATTRIBUTES'])

                                mem_rung_contacts=ladder_mem_rung.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == 'contact')
                                mem_rung_contacts_attributes=list(mem_rung_contacts['ATTRIBUTES'])

                                #Extract the Move and Clear block details if present for future use
                                block_connections_list.append(get_block_connections(ladder_mem_rung))
                                ...
                                for contact in mem_rung_contacts_attributes:
                                    ...
                                    if contact['negated']=='false':
                                        ...
                                        for coil in mem_rung_coils_attributes:
                                            ...
                                            if set(contact['out_list']) & set(coil['in_list']):
                            ...
                            memory_feeding_contacts=ladder_memory_feeding.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == 'contact')
                            memory_feeding_contacts_attributes=list(memory_feeding_contacts['ATTRIBUTES'])
                                ...
                                if contact_attr['negated']=='false':
                                    ...
                                    if contact_attr['operand'] in ok_coil_operands:
                                        for cc_pair in contact_coil_pairs:
                                            ...
                                            coil_match=cc_pair.get(contact_attr['operand'], 'NONE')
                                            if coil_match!='NONE':
                                                ok_contacts_found.append(contact_attr['operand'])
                                                ok_mem_coils.append(coil_match)

                                                logger.warning(f"Program:{program} Ok Coil operand : {contact_attr['operand']} found in Memory Feedings")
                                    ...
                                    if contact_attr['operand'] in normal_coil_operands:
                                            for cc_pair in contact_coil_pairs:
                                                ...
                                                coil_match=cc_pair.get(contact_attr['operand'], 'NONE')
                                                if coil_match!='NONE':
                                                    normal_contacts_found.append(contact_attr['operand'])
                                                    normal_mem_coils.append(coil_match)
                                                    logger.warning(f"Program:{program} Normal Coil operand : {contact_attr['operand']} found in Memory Feedings")
                                    ...
                                    if contact_attr['operand'] in ng_coil_operands:
                                            for cc_pair in contact_coil_pairs:
                                                ...
                                                if coil_match!='NONE':
                                                    ...
                                                    ng_contacts_found.append(contact_attr['operand'])
                                                    ng_mem_coils.append(coil_match)
                                                    logger.warning(f"Program:{program} NG Coil operand : {contact_attr['operand']} found in Memory Feedings")

                                                    break



                                    if contact_attr['operand'] in abnormal_coil_operands:
                                            for cc_pair in contact_coil_pairs:
                                                ...
                                                if coil_match!='NONE':
                                                    ...
                                                    abnormal_contacts_found.append(contact_attr['operand'])
                                                    abnormal_mem_coils.append(coil_match)
                                                    logger.warning(f"Program:{program} Abnormal Coil operand : {contact_attr['operand']} found in Memory Feedings")
                            ...
                            ok_missing_contacts=[item for item in ok_coil_operands if item not in ok_contacts_found]
                            normal_missing_contacts=[item for item in normal_coil_operands if item not in normal_contacts_found]
                            ng_missing_contacts=[item for item in ng_coil_operands if item not in ng_contacts_found]
                            abnormal_missing_contacts=[item for item in abnormal_coil_operands if item not in abnormal_contacts_found]
                            ...
                            sub_df_en={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                                'RungNo':[""], "Target":[detail_dict], 
                                'CheckItem':rule_2_check_item,
                                'Detail': [NG_exp_missing_contacts_en],  'Status': ['']}
                            ...
                            sub_df_jp={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                                'RungNo':[""], "Target":[detail_dict], 
                                'CheckItem':rule_2_check_item,
                                'Detail': [NG_exp_missing_contacts_jp],  'Status': ['']}
                            ...
                            detail_dict={}
                            detail_dict['normal_missings_contacts']=normal_missing_contacts
                            ...
                            detail_dict={}
                            detail_dict['abnormal_missings_contacts']=abnormal_missing_contacts
                            ...
                            if len(ok_contacts_found)>0:
                                ...
                                sub_df_en, sub_df_jp=  create_dataframes(task_name=program, section_name='AutoRun', rule_number='2',
                                                                    check_number='1', rule_content=rule_content_2,
                                                                    status='OK',
                                                                    details=detail_dict, NG_exp_en='NONE', NG_exp_jp='NONE')
                            ...
                            if len(normal_contacts_found)>0:
                                detail_dict={}
                                detail_dict['normal_contacts_found']=normal_contacts_found
                            ...
                            if len(abnormal_contacts_found)>0:
                                detail_dict={}
                                detail_dict['abnormal_contacts_found']=abnormal_contacts_found
                            ...
                            logger.warning(f"program:{program} Coil Counterparts in Memory Feedings \n Ok:{ok_mem_coils},\n normal:{normal_mem_coils},\n NG:{ng_mem_coils}, \n Abnormal:{abnormal_mem_coils}")
                            ...
                            memory_feeding_coils=ladder_memory_feeding.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == 'coil')
                            memory_feeding_coils_attributes=list(memory_feeding_coils['ATTRIBUTES'])
                            memory_feeding_coils_rungs=list(memory_feeding_coils['RUNG'])
                            ...
                            for coil_attr, mem_rung in zip(memory_feeding_coils_attributes, memory_feeding_coils_rungs):
                                ...
                                if coil_latch!='NONE':
                                    ...
                                    if coil_latch=='reset':
                                        mem_coils_reset_list.append(coil_attr['operand'])
                            ...
                            for ok_mem_coil in ok_mem_coils:
                                if ok_mem_coil['operand'] in mem_coils_set_list:
                                    ...
                                    sub_df_en={'Result':["OK"], 'Task':[program], 'Section': ['AutoRun'], 
                                        'RungNo':[""], "Target":[detail_dict], 
                                        'CheckItem':rule_2_check_item,
                                        'Detail': ["NONE"],  'Status': ['']}
                                    ...
                                    sub_df_jp={'Result':["OK"], 'Task':[program], 'Section': ['AutoRun'], 
                                        'RungNo':[""], "Target":[detail_dict], 
                                        'CheckItem':rule_2_check_item,
                                        'Detail': ["NONE"],  'Status': ['']}
    ...
    # )
                            ...
                            for normal_mem_coil in normal_mem_coils:
                                if normal_mem_coil['operand'] in mem_coils_set_list:
                                    ...
                                    detail_dict={}
                                    detail_dict['Normal_Set_Coil']=normal_mem_coil['operand']
                            ...
                            for abnormal_mem_coil in abnormal_mem_coils:
                                if abnormal_mem_coil['operand'] in mem_coils_set_list:
                                    ...
                                    detail_dict={}
                                    detail_dict['abnormal_Set_Coil']=abnormal_mem_coil['operand']
                            ...
                            set_reset_coils=list(set(mem_coils_reset_list) & set(mem_coils_set_list))
                            logger.warning(f"In program :{program} Coils that are both sEt and Reset :{set_reset_coils}")
                            ...
                            set_reset_check_dict={'ok_mem_coils': ok_mem_coils, 'normal_mem_coils': normal_mem_coils,
                                                    'ng_mem_coils': ng_mem_coils, 'abnormal_mem_coils': normal_mem_coils
                                                    }

                            set_reset_check_keys=set_reset_check_dict.keys()
                            set_reset_check_values=set_reset_check_dict.values()

                            for coil_list_keys, coil_list_values in zip(set_reset_check_keys, set_reset_check_values):
                                ...
                                for coil in coil_list_values:
                                    ...
                                    if coil['operand'] in set_reset_coils:
                                        ...
                                        if coil_list_keys=='ok_mem_coils':
                                            detail_dict['OK_reset_coils']=coil['operand']

                                        if coil_list_keys=='normal_mem_coils':
                                            detail_dict['normal_reset_coils']=coil['operand']


                                        if coil_list_keys=='ng_mem_coils':
                                            detail_dict['ng_reset_coils']=coil['operand']


                                        if coil_list_keys=='abnormal_mem_coils':
                                            detail_dict['abnormal_reset_coils']=coil['operand']
                                            ...
                                            for sub_dict in ele_block:
                                                ...
                                                if clear_block!="NONE":
                                                    for conns in clear_block:
                                                        clear_inout_conns=conns.get('InOut', 'NONE')
                                                        if clear_inout_conns!="NONE":
                                                            if coil['operand'] in clear_inout_conns:
                                                                coil_check_flag=1
                                                ...
                                                #Check in Move block
                                                move_block=sub_dict.get('MOVE', 'NONE')
                                                if move_block!="NONE":
                                                    for conns in move_block:
                                                        move_out_conns=conns.get('Out', 'NONE')
                                                        if move_out_conns!="NONE":
                                                            if coil['operand'] in move_out_conns:
                                                                coil_check_flag=1
                                    ...
                                    if coil_check_flag==0:
                                        ...
                                        sub_df_en={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                                            'RungNo':[""], "Target":[detail_dict], 
                                            'CheckItem':rule_2_check_item,
                                            'Detail': [NG_exp_set_reset_en],  'Status': ['']}
                                        ...
                                        sub_df_jp={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                                            'RungNo':[""], "Target":[detail_dict], 
                                            'CheckItem':rule_2_check_item,
                                            'Detail': [NG_exp_set_reset_jp],  'Status': ['']}
                            ...
                            condition_contacts=ladder_condition.filter(pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == 'contact')
                            condition_contact_attributes=list(condition_contacts['ATTRIBUTES'])
                            ...
                            for contact_attr in condition_contact_attributes:
                                ...
                                if all([contact_negated_status!='NONE',contact_negated_status=='true' ]):
                                    condtion_B_contact_dict_list.append(contact_attr)
                                    condtion_B_contact_list.append(contact_attr['operand'])


                            logger.warning(f"In program: {program} Condition Section B contacts are {condtion_B_contact_list}")
                            ...
                            for coil_list_keys, coil_list_values in zip(set_reset_check_keys, set_reset_check_values):
                                ...
                                for coil in coil_list_values:
                                    ...
                                    if coil['operand'] in condtion_B_contact_list:
                                        ...
                                        if coil_list_keys=='ok_mem_coils':
                                            detail_dict['OK_condtion_coils']=coil['operand']

                                        if coil_list_keys=='normal_mem_coils':
                                            detail_dict['normal_condition_coils']=coil['operand']


                                        if coil_list_keys=='ng_mem_coils':
                                            detail_dict['ng_condition_coils']=coil['operand']


                                        if coil_list_keys=='abnormal_mem_coils':
                                            detail_dict['abnormal_condition_coils']=coil['operand']
                                        ...
                                        if re.search(r"\[0\]", coil['operand']):
                                            ...
                                            sub_df_en={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                                                'RungNo':[""], "Target":[detail_dict], 
                                                'CheckItem':rule_2_check_item,
                                                'Detail': [NG_exp_condition_en],  'Status': ['']}
                                            ...
                                            sub_df_jp={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                                                'RungNo':[""], "Target":[detail_dict], 
                                                'CheckItem':rule_2_check_item,
                                                'Detail': [NG_exp_condition_jp],  'Status': ['']}
        ...
        for program in unique_program_values:
            ...
            out_coil_df=ladder_autorun.filter(ladder_autorun["OBJECT_TYPE_LIST"] == 'Coil')
            outcoil_rung_list=list(out_coil_df['RUNG'])
            ...
            ladder_pd_autorun=ladder_pd_program[ladder_pd_program[body_type_key] == 'AutoRun']
            ...
            for ok_coil, ok_rung in zip(ok_coil_list, ok_coil_rung_list):
                ...
                for unique_rung in outcoil_rung_unique_list:
                    ...
                    if all([unique_rung <= ok_rung, unique_rung >= start_list[0]]):
                        ...
                        autorun_rung_contacts_pd=ladder_pd_autorun[ladder_pd_autorun["RUNG"] == unique_rung]
                        autorun_rung_contacts=ladder_autorun.filter(ladder_autorun["RUNG"] == unique_rung)

                        coil_df_unique_rung=autorun_rung_contacts.filter(pl.col('OBJECT_TYPE_LIST')=='Coil')

                        #In each of the rung, check for only rungs that have two or more outcoils
                        if len(coil_df_unique_rung)>1:


                            self_holding_contacts=check_self_holding(autorun_rung_contacts_pd)
                            logger.warning(f"In program : {program}, Self holding contacts are \nrung:{unique_rung} \n {self_holding_contacts}")
                            ...
                            if len(self_holding_contacts)>0:
                                ...
                                sub_df_en, sub_df_jp=  create_dataframes(task_name=program, section_name='AutoRun', rule_number='2',
                                                                        check_number='5', rule_content=rule_content_2,
                                                                        status='OK',
                                                                        details=detail_dict, NG_exp_en='NONE', NG_exp_jp='NONE')
                                ...
                                sub_df_en, sub_df_jp=  create_dataframes(task_name=program, section_name='AutoRun', rule_number='2',
                                                                        check_number='5', rule_content=rule_content_2,
                                                                        status='NG',
                                                                        details=detail_dict, NG_exp_en='Self Holding Contacts Not Found',
                        ...
                        if ok_coil in self_holding_contacts:
                            ...
                            parallel_connections=list(get_parallel_contacts(autorun_rung_contacts).values())

                            parallel_contacts=get_in_parallel_A_contacts(parallel_connections, ok_coil)
                            ...
                            if len(parallel_contacts)>0:
                                ...
                                sub_df_en, sub_df_jp=  create_dataframes(task_name=program, section_name='AutoRun', rule_number='2',
                                                                        check_number='5', rule_content=rule_content_2,
                                                                        status='NG',
                                                                        details=detail_dict, NG_exp_en='Parallel A Contacts not found',
                            ...
                            logger.warning(f"In program : {program}, Parallel contacts are \nrung:{unique_rung} \n {parallel_contacts}")
                            ...
                            for parallel_contact in parallel_contacts:
                                ...
                                try:
                                    ...
                                    for prev_rung in range(unique_rung-1, 0, -1):
                                        out_coil_previous_df=out_coil_df.filter(pl.col("RUNG") == prev_rung)
                                        out_coil_previous_attributes=list(out_coil_previous_df['ATTRIBUTES'])
                                        ...
                                        for prev_attr in out_coil_previous_attributes:
                                            ...
                                            if parallel_contact==prev_attr_operand:
                                                ...
                                                rule_2_ladder=ladder_autorun.filter(pl.col('RUNG')==prev_rung)
                                                check_2_results=Rule_2_Check_2_program(ladder_df=rule_2_ladder, main_ladder=ladder_autorun, ref_rung=prev_rung)

                                                logger.warning(f"In program :{program} AutoRun section \n ref_rung:{prev_rung} \n parallel contact:{parallel_contact} \n Results :{check_2_results} ")

                                                if check_2_results['status']=='OK':

                                                    detail_dict={}
                                                    detail_dict['rung']=check_2_results['ref_rung']
                                                    detail_dict['target_rung']=check_2_results['target_rung']
                                                    detail_dict['MOVE_Inp']=check_2_results['MOVE_Inp']
                                                    detail_dict['comment_type']='OK'



                                                    sub_df_en, sub_df_jp=  create_dataframes(task_name=program, section_name='AutoRun', rule_number='2',
                                                                                            check_number=check_2_results['check_number'],
                                                                                            rule_content=rule_content_2,
                                                                                            status=check_2_results['status'],
                                                                                            details=detail_dict,
                                                                                            NG_exp_en=check_2_results['NG_explanantion'],
                                                                                            NG_exp_jp=check_2_results['NG_explanantion'])
                                                ...
                                                if check_2_results['status']=='NG':
                                                    ...
                                                    check_2_condition_results=Rule_2_Check_2_program(ladder_df=rule_2_ladder, main_ladder=ladder_condition, ref_rung=prev_rung)
                                                    logger.warning(f"In program :{program} Condition section \n ref_rung:{prev_rung} \n parallel contact:{parallel_contact} \n Results :{check_2_condition_results} ")

                                                    if check_2_condition_results['status']=='OK':

                                                        detail_dict={}
                                                        detail_dict['rung']=check_2_condition_results['ref_rung']
                                                        detail_dict['target_rung']=check_2_condition_results['target_rung']
                                                        detail_dict['MOVE_Inp']=check_2_condition_results['MOVE_Inp']
                                                        detail_dict['comment_type']='OK'

                                                        sub_df_en, sub_df_jp=  create_dataframes(task_name=program, section_name='Condition', rule_number='2',
                                                                                                check_number=check_2_condition_results['check_number'],
                                                                                                rule_content=rule_content_2,
                                                                                                status=check_2_condition_results['status'],
                                                                                                details=detail_dict,
                                                                                                NG_exp_en=check_2_condition_results['NG_explanantion'],
                                                                                                NG_exp_jp=check_2_condition_results['NG_explanantion'])
            ...
            for normal_coil, normal_rung in zip(normal_coil_list, normal_coil_rung_list):
                ...
                for unique_rung in outcoil_rung_unique_list:
                    ...
                    if all([unique_rung <= normal_rung, unique_rung >= start_list[0]]):
                        ...
                        if len(coil_df_unique_rung)>1:
                            ...
                            self_holding_contacts=check_self_holding(autorun_rung_contacts_pd)
                            logger.warning(f"In program : {program}, Self holding Normal contacts are \nrung:{unique_rung} \n {self_holding_contacts}")
                            ...
                            if len(self_holding_contacts)>0:
                                ...
                                sub_df_en, sub_df_jp=  create_dataframes(task_name=program, section_name='AutoRun', rule_number='2',
                                                                        check_number='5', rule_content=rule_content_2,
                                                                        status='NG',
                                                                        details=detail_dict, NG_exp_en='Self Holding Contacts Not Found', NG_exp_jp='Self Holding Contacts Not Found')
                        ...
                        if normal_coil in self_holding_contacts:
                            ...
                            parallel_contacts=get_in_parallel_A_contacts(parallel_connections, normal_coil)
                            ...
                            logger.warning(f"In program : {program}, Parallel Normal contacts are \nrung:{unique_rung} \n {parallel_contacts}")
