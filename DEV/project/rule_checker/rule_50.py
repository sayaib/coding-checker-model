...
def execute_rule_50(input_file : str, 
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            if 'main' in program.lower():
                ...
                ladder_fault=ladder_program.filter(ladder_program[body_type_key] == 'Fault')
                
                #Pandas specific dataframe
                ladder_program_pd=ladder_df_pd[ladder_df_pd[program_key] == program]
                ladder_fault_pd=ladder_program_pd[ladder_program_pd[body_type_key] == 'Fault']
                ...
                if len(ladder_fault)>0:
                    ...
                    out_coil_df=ladder_fault.filter(ladder_fault["OBJECT_TYPE_LIST"] == 'Coil')
                    ...
                    for rung_num, rung_name, coil_attribute in zip(outcoil_rung_list, outcoil_rung_name_list, outcoil_attributes_list):
                        ...
                        if re.search(AL_var_pattern, coil_operand):
                            if regex_pattern_check(air_source_pattern, coil_comment_dict) and regex_pattern_check(drop_comment_pattern, coil_comment_dict):
                                ...
                                sub_dict={'Result':["OK"], 'Task':[program], 'Section': ['Fault'], 
                                        'RungNo':[rung_num], "Target":[detail_dict], 
                                ...
                                logger.warning(f"Outcoil: {coil_operand} detected in \n program:{program} \n Body : Fault \n Rung: {rung_num}")
                                
                                
                                
                                series_operand_list=[]
                                #Performing check 1
                                check_1_rung_df=ladder_fault.filter(ladder_fault["RUNG"] == rung_num)
                                check_1_contacts_df=check_1_rung_df.filter(check_1_rung_df['OBJECT_TYPE_LIST']=='Contact')
                                                               

                                contact_rung_list=list(check_1_contacts_df['RUNG'])
                                contact_rung_name_list=list(check_1_contacts_df['RUNG_NAME'])
                                
                                contact_attributes_list=list(check_1_contacts_df['ATTRIBUTES'])
                                
                                
                                
                                
                                
                                for rung_num_co, rung_name_co, contact_attribute in zip(contact_rung_list, contact_rung_name_list, contact_attributes_list):
                                    ...
                                    contact_comment_dict=get_the_comment_from_program(variable=contact_operand, program=program, input_comment_data=comment_data)
                                    ...
                                    if all([contact_type=='false']):
                                        if regex_pattern_check(master_on_pattern, contact_comment_dict) and regex_pattern_check(complete_pattern, contact_comment_dict):
                                            ...
                                            logger.warning(f" A Contact: {contact_operand} detected in \n program:{program} \n Body : Fault \n Rung: {rung_num}")
                                    ...
                                    if contact_type=='true':
                                        if regex_pattern_check(confirmation_pattern, contact_comment_dict) and regex_pattern_check(air_source_pattern, contact_comment_dict):
                                            series_operand_list.append(contact_operand)
                                            ...
                                            logger.warning(f" B Contact : {contact_operand} detected in \n program:{program} \n Body : Fault \n Rung: {rung_num}")
                                            # print(contact_comment_dict, contact_operand)
                                            check_3_flag=1
                               
                               
                                #Check 4, This happens outside the contact loop
                                block_check_rung_df=ladder_fault.filter(ladder_fault["RUNG"] == rung_of_interest)
                                block_df=block_check_rung_df.filter(block_check_rung_df["OBJECT_TYPE_LIST"] == 'Block')
                                ...
                                for block_attr in block_attributes_list:
                                    ...
                                    if block_attr['typeName']=='TON':
                                        block_conns=get_block_connections(block_check_rung_df)
                                                ...
                                                for inner_dict in nested_list:
                                                    for inner_key, inner_value in inner_dict.items():
                                                        ...
                                                        if inner_key=='PT':
                                                            ...
                                                            if pt_value_to_check in inner_value:
                                                                check_4_flag=1
                                                                                                              
                                                                #Write the data to dataframe
                                                                detail_dict={}
                                                                detail_dict['target_rung']=rung_of_interest
                                                                detail_dict['PT_value']=inner_value
                                                                ...
                                                                logger.warning(f" TON block with inner value {inner_value} detected in \n program:{program} \n Body : Fault \n Rung: {rung_of_interest}")
                        ...
                        if all([check_2_flag==1, check_3_flag==1, check_4_flag==1]):
                            ...
                            if (A_contact in TON_in) and (B_contact in TON_out):
                                ...
                                logger.warning(f"A contact :{A_contact}, TON Block and B contact:{B_contact} are in series connection in \n program:{program} \n Body : Fault \n Rung: {rung_num}")
                  
                                 
                                            
                        #Check for Self holding, 5.2
                        if rung_of_interest!=0:
                            rung_df_pd=ladder_fault_pd[ladder_fault_pd["RUNG"] == rung_of_interest]
                            ...
                            if outcoil_of_interest in self_holding_contacts:
                                ...
                                detail_dict={}
                                detail_dict['target_rung']=rung_of_interest
                                detail_dict['self_holding_contacts']=self_holding_contacts
                                ...
                                logger.warning(f" Outcoil {outcoil_of_interest} is self-holding in \n program:{program} \n Body : Fault \n Rung: {rung_num}")
                            
                          
                          
                        ### Write the data for NG cases############
                        
                        if all([check_1_flag==1, check_2_flag==0, check_2_NG_flag==0]):
                            ...
                            sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                        'RungNo':[rung_of_interest], "Target":[detail_dict], 
                                        'CheckItem':rule_50_check_item,
                                        'Detail': ['A contact with master on and complete not found'],  'Status': ['']}
                        ...
                        if all([check_1_flag==1, check_3_flag==0, check_3_NG_flag==0]):
                            ...
                            sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                    'RungNo':[rung_of_interest], "Target":[detail_dict], 
                                    'CheckItem':rule_50_check_item,
                                    'Detail': ['B contact with conformation and air source not found'],  'Status': ['']}
                        ...
                        if all([check_1_flag==1, check_4_flag==0, check_4_NG_flag==0]):
                            ...
                            sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                    'RungNo':[rung_of_interest], "Target":[detail_dict], 
                                    'CheckItem':rule_50_check_item,
                                    'Detail': ['Either TON block not found or PT input is not associated with t#1.0s'],  'Status': ['']}
                        ...
                        if all([check_1_flag==1,  check_5_1_flag==0, check_5_1_NG_flag==0]):
                            ...
                            sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                'RungNo':[rung_of_interest], "Target":[detail_dict], 
                                'CheckItem':rule_50_check_item,
                                'Detail': ['A Contact, TON Block and B Contact are not in seriesE'],  'Status': ['']}
                        ...
                        if all([check_1_flag==1, check_5_2_flag==0, check_5_2_NG_flag==0 ]):
                            ...
                            sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                'RungNo':[rung_of_interest], "Target":[detail_dict], 
                                'CheckItem':rule_50_check_item,
                                'Detail': ['Self Holding Contact not found'],  'Status': ['']}
                    ...
                    if all([check_1_NG_flag==0, AL_coil_found_flag==0 ]):
                        ...
                        sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['Fault'], 
                                'RungNo':[rung_of_interest], "Target":[detail_dict], 
                                'CheckItem':rule_50_check_item,
                                'Detail': ['AL variable with air_source and drop not found'],  'Status': ['']}
