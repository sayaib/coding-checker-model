...
check_detail_content = "Check that the function name “TON” exists in the out coil condition detected by ③."
...
def detection_range_programwise(device_in_program_df:pd.DataFrame, program_name:str, program_comment_data:dict):
    ...
    if not device_in_program_df.empty:
        coil_df = device_in_program_df[device_in_program_df['OBJECT_TYPE_LIST'].str.lower() == 'coil']
            ...
            for _, coil_row in coil_df.iterrows():
                ...
                if isinstance(coil_operand, str) and coil_operand:
                    ...
                    if isinstance(coil_comment, list) and coil_comment:
                        if regex_pattern_check(workpiece_comment, coil_comment) or regex_pattern_check(pallet_comment, coil_comment) or regex_pattern_check(tray_1_comment, coil_comment) or regex_pattern_check(tray_2_comment, coil_comment) or regex_pattern_check(magazine_comment, coil_comment) or regex_pattern_check(hoop_comment, coil_comment) or regex_pattern_check(stick_comment, coil_comment) or regex_pattern_check(skewer_comment, coil_comment) or regex_pattern_check(box_comment, coil_comment):
...
def detection_range_functionwise(device_in_function_df:pd.DataFrame, function_name:str, function_comment_data:dict):
    ...
    if not device_in_function_df.empty:
        coil_df = device_in_function_df[device_in_function_df['OBJECT_TYPE_LIST'].str.lower() == 'coil']
        if not coil_df.empty:
            for _, coil_row in coil_df.iterrows():
                attr = ast.literal_eval(coil_row['ATTRIBUTES'])
                coil_operand = attr.get('operand')
...
def execute_rule_70_programwise(input_program_file:str, 
            ...
            if is_task_name_in_range:
                ...
                if detection_results:
                    for coil_data in detection_results:
                        coil_rung_data_df = device_in_program_df[device_in_program_df['RUNG'] == coil_data[1]]
                        status = "NG"
                        ng_name = ng_content
                        if not coil_rung_data_df.empty:
                            for _, coil_rung_row in coil_rung_data_df.iterrows():
                                ...
                                if operand == coil_data[0] and coil_rung_row['OBJECT_TYPE_LIST'].lower() == 'coil':
                                    break

                                if coil_rung_row['OBJECT_TYPE_LIST'].lower() == 'block' and attr.get('typeName') == 'TON':
                                    status = "OK"
                                    ng_name = "" 
                                    break
...
def execute_rule_70_functionwise(input_function_file:str, 
    ...
    try:
        ...
        for function in unique_function_values:
            ...
            if is_task_name_in_range:
                ...
                if detection_results:
                    for coil_data in detection_results:
                        coil_rung_data_df = device_in_function_df[device_in_function_df['RUNG'] == coil_data[1]]
                        status = "NG"
                        ng_name = ng_content
                        if not coil_rung_data_df.empty:
                            for _, coil_rung_row in coil_rung_data_df.iterrows():
