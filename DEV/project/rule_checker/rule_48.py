...
check_detail_content = {
    "cc1": " If ① but not ③ or not ⑤, it is assumed to be NG.",
    "cc2": " Checks that all of the out coil condition detected in ③ are connected in series (AND) only with the A contact that contains ”ヒューズ(fuse)"
    + "正常(normal)” in the variable comment. *NG if other contacts exist.",
    "cc3": "Check that contact A includes the variable comment of ”電源入り(power on)"
    + "遅延(delay)” in the out coil condition detected in ③.",
    "cc4": "Check that contact B includes the variable comment of ”ヒューズ(fuse)"
    + "正常(normal)”+でない(not) in the out coil condition detected in ③.",
...
def detection_range_programwise(
    devicein_section_df: pd.DataFrame,
    fault_section_df: pd.DataFrame,
    fuse_full_width_comment: str,
    fuse_half_width_comment: str,
    program_name: str,
    program_comment_data: dict,
) -> dict:
    ...
    devicein_coil_df = devicein_section_df[
        devicein_section_df["OBJECT_TYPE_LIST"] == "Coil"
    ].copy()
    fault_coil_df = fault_section_df[
        fault_section_df["OBJECT_TYPE_LIST"] == "Coil"
    ].copy()
    ...
    for _, devicein_coil_row in devicein_coil_df.iterrows():
        ...
        if devicein_coil_operand and isinstance(devicein_coil_operand, str):
            devicein_coil_comment = get_the_comment_from_program(
                devicein_coil_operand, program_name, program_comment_data
            )
        else:
        ...
        if isinstance(devicein_coil_comment, list):
            if (
                regex_pattern_check(fuse_full_width_comment, devicein_coil_comment)
                or regex_pattern_check(fuse_half_width_comment, devicein_coil_comment)
            ) and regex_pattern_check(normal_comment, devicein_coil_comment):
    ...
    for _, fault_coil_row in fault_coil_df.iterrows():
        ...
        if "AL" in fault_coil_operand:
            ...
            if isinstance(fault_coil_comment, list):
                if (
                    regex_pattern_check(fuse_full_width_comment, fault_coil_comment)
                    or regex_pattern_check(fuse_half_width_comment, fault_coil_comment)
                ) and regex_pattern_check(disconnection_comment, fault_coil_comment):
...
def check_detail_1_programwise(detection_3_details: dict, detection_5_details: dict):
    ...
    cc1_result["status"] = (
        "OK"
        if (detection_3_details[1] != -1 and detection_5_details[-1] != -1)
        else "NG"
    )
    cc1_result["cc"] = "cc1"
...
def check_detail_2_programwise(
    ...
    if rung_number != -1:
        detection_coil_rung_df = devicein_section_df[
            devicein_section_df["RUNG"] == rung_number
        ]
        ...
        for _, contact_row in contact_df.iterrows():
            ...
            if isinstance(contact_comment, list):
                if not (
                    (
                        regex_pattern_check(fuse_full_width_comment, contact_comment)
                        or regex_pattern_check(fuse_half_width_comment, contact_comment)
                    )
...
def check_detail_3_programwise(
    ...
    fault_detection_coil_rung_df = fault_section_df[
        fault_section_df["RUNG"] == rung_number
    ]
    contact_df = fault_detection_coil_rung_df[
        fault_detection_coil_rung_df["OBJECT_TYPE_LIST"] == "Contact"
    ].copy()
...
def check_detail_5_programwise(
    ...
    if cc3_contact and cc4_contact:
        detection_coil_rung_df = fault_section_df[
            fault_section_df["RUNG"] == rung_number
        ]
        contact_df = detection_coil_rung_df[
            detection_coil_rung_df["OBJECT_TYPE_LIST"] == "Contact"
...
def execute_rule_48_programwise(
    ...
    try:
        ...
        for program_name in unique_program_values:
            ...
            if "main" in program_name.lower():
                ...
                devicein_section_df = current_program_df[
                    current_program_df["BODY"].str.lower() == devicein_section_name
                ]
                ...
                cc1_result = check_detail_1_programwise(
                    detection_3_details=detection_3_details,
                    detection_5_details=detection_5_details,
                )
                ...
                cc5_result = check_detail_5_programwise(
                    fault_section_df=fault_section_df,
                    detection_coil=detection_5_coil,
                    cc3_contact=cc3_contact,
                    cc4_contact=cc4_contact,
                    rung_number=detection_5_rung_number,
                    program_name=program_name,
                )
