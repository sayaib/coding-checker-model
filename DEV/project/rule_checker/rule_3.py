...
def execute_rule_3(input_file : str, 
                   input_program_comment_file:str, 
                   program_key:str='PROGRAM', 
                   body_type_key:str='BODY') -> None:
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            ladder_autorun=ladder_program.filter((pl.col(body_type_key).cast(pl.Utf8).str.to_lowercase() == 'autorun') | (pl.col(body_type_key).cast(pl.Utf8).str.to_lowercase() == 'autorun★') )
            ...
            for rung_num, rung_name, coil_attribute in zip(outcoil_rung_list, outcoil_rung_name_list, outcoil_attributes_list):
                ...
                if regex_pattern_check(qr_jp, coil_comment_dict) and regex_pattern_check(read_jp, coil_comment_dict) and regex_pattern_check(start_jp, coil_comment_dict):
        ...
        for program in program_range_dict.keys():
            ...
            if (len(start_list)!=0 )and (len(end_list)!=0):
                logger.info(f"Rule 3 Ranges detected for program: {program} start_list: {start_list}  end_list: {end_list}")
            
            if len(start_list)!=len(end_list):
                
                logger.info(f"Rule 3 Open Ended Range: {program} start_list:{start_list} end_list:{end_list}")
                ...
                sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                    ...
                    'Detail': ["Range that has either Start point or End point not both"],  'Status': ['']}
                ...
                if len(start_list)!=0:
                    ...
                    for start_rung, end_rung in zip(start_list, end_list):
                        ...
                        if int(end_rung)> int(start_rung):
                            ...
                            ladder_program=ladder_df.filter(ladder_df[program_key] == program)
                            ladder_autorun=ladder_program.filter((pl.col(body_type_key) == 'AutoRun') | (pl.col(body_type_key) == 'AutoRun★'))
                            ...
                            contacts_program_for_rungs_in_range=ladder_program_for_rungs_in_range.filter(
                                                                    (pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == "contact")
                                                                    
                                                                        )
                            contacts_rungs=list(contacts_program_for_rungs_in_range['RUNG'])
                            contacts_attributes=list(contacts_program_for_rungs_in_range['ATTRIBUTES'])
                            


                            blocks_program_for_rungs_in_range=ladder_program_for_rungs_in_range.filter(
                                                                    (pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == "block")
                                                                    
                                                                        )
                            blocks_rungs=list(blocks_program_for_rungs_in_range['RUNG'])
                            blocks_attributes=list(blocks_program_for_rungs_in_range['ATTRIBUTES'])
                            



                            coils_program_for_rungs_in_range=ladder_program_for_rungs_in_range.filter(
                                                                    (pl.col("OBJECT_TYPE_LIST").cast(pl.Utf8).str.to_lowercase() == "coil")
                                                                    
                                                                        )
                            coils_rungs=list(coils_program_for_rungs_in_range['RUNG'])
                            coils_attributes=list(coils_program_for_rungs_in_range['ATTRIBUTES'])
                            ...
                            for rung_with_two_or_more_outcoils in rungs_with_two_or_more_outcoils:
                                ...
                                logger.info(f"Rule 3 In Rung with two or more coils, looking into {rung_with_two_or_more_outcoils-1}")
                                ...
                                for outcoil_attribute in outcoil_for_check_attributes:
                                    ...
                                    for pattern_ in comment_patterns_to_checked:
                                        ...
                                        if regex_pattern_check(pattern_, comment_list):
                                            ...
                                            logger.info(f"Rule 3 For start_rung:{start_rung-1}, end_rung:{end_rung-1}, pattern: {pattern_} found")
                                            ...
                                            detail_dict['coil_variable']=ok_outcoil_operand
                                            ...
                                            for blocks_rung, blocks_attribute in zip(blocks_rungs, blocks_attributes):
                                                blocks_attribute=eval(blocks_attribute)
                                                                                            
                                                
                                                if blocks_rung==rung_with_two_or_more_outcoils:
                                                    blocks_typename= blocks_attribute.get("typeName", 'NONE')
                                                    
                                                    #If the block type matches with any one in the acceptable list
                                                    if blocks_typename in acceptable_en_block_list:
                                                        ...
                                                        detail_dict['rung_range']=f"{start_rung-1}-{end_rung-1}"
                                                        detail_dict['target_EN_rung']=target_en_rung-1
                                                        detail_dict['OK_out_coil']=coil_operand
                                                        detail_dict['pattern_checked']=pattern_
                                                        ...
                                                        logger.info(f"Rule 3 EN {blocks_typename} block found in \n{blocks_rung} \n{ok_outcoil_operand} \nfor start_rung{start_rung} and end_rung:{end_rung}")
                                            ...
                                            if not(en_block_found_flag):
                                                ...
                                                for contacts_rung, contacts_attribute in zip(contacts_rungs, contacts_attributes):
                                                    
                                                    if contacts_rung <=rung_with_two_or_more_outcoils:
                                                    
                                                        contacts_attribute=eval(contacts_attribute)
                                                        contact_comment_list=get_the_comment_from_program(variable=contacts_attribute['operand'], 
                                                                                                        program=program, 
                                                                                                        input_comment_data=comment_data)
                                                        
                                                        
                                                        
                                                        if regex_pattern_check(pattern_, contact_comment_list) and (contacts_attribute['operand']!=ok_outcoil_operand) and (contacts_attribute['negated']=='false'):
                                                            
                                                            ok_contacts.append(contacts_attribute['operand'])
                                                ...
                                                if len(ok_contacts)==0:
                                                    ...
                                                    detail_dict['rung_range']=f"{start_rung-1}-{end_rung-1}"
                                                    detail_dict['target_rung']=rung_with_two_or_more_outcoils-1
                                                    detail_dict['pattern_checked']=pattern_
                                                    detail_dict['OK_out_coil']="NONE"
                                                    detail_dict['outcoil_with_no_contact']=coil_operand
                                                    ...
                                                    detail_dict['target_rung']=rung_with_two_or_more_outcoils-1
                                                    detail_dict['pattern_checked']=pattern_
                                                    detail_dict['OK_out_coil']=ok_contacts
                                                    detail_dict['outcoil_with_no_contact']=coil_operand
                                                ...
                                                for ok_contact in ok_contacts:
                                                    ...
                                                    for coils_rung, coils_attribute in zip(coils_rungs,coils_attributes):
                                                                                            
                                                            
                                                        
                                                        if coils_rung <=rung_with_two_or_more_outcoils:
                                                        
                                                            coils_attribute=eval(coils_attribute)
                                                            
                                                            if ok_contact==coils_attribute['operand']:
                                                                
                                                                for blocks_rung, blocks_attribute in zip(blocks_rungs, blocks_attributes):
                                                                    
                                                                    if blocks_rung==coils_rung:
                                                                        
                                                                        blocks_attribute=eval(blocks_attribute)
                                                                        
                                                                        blocks_attribute_typename=blocks_attribute.get("typeName", "NONE")
                                                                        if blocks_attribute_typename in acceptable_en_block_list:
                                                                        
                                                                            
                                                                            ok_contact_en_block_flag=1
                                                                            target_en_rung=blocks_rung
                                                                            ok_outcoil_operand=coils_attribute['operand']
                                                                            ...
                                                                            detail_dict['target_EN_rung']=target_en_rung-1
                                                                            detail_dict['pattern_checked']=pattern_
                                                                            detail_dict['OK_out_coil']=coils_attribute['operand']
                                                                            ...
                                                                            logger.info(f"Rule 3 EN = block found in preceeding rungs {blocks_rung-1} {ok_outcoil_operand} for start_rung{start_rung-1} and end_rung:{end_rung-1}")
                                                ...
                                                if(ok_contact_en_block_flag==0):
                                                    ...
                                                    sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                                                        'RungNo':[""], "Target":[detail_dict], 
                                                        'CheckItem':rule_3_check_item,
                                                        'Detail': ["EN block NOT detected in preceeding rungs"],  'Status': ['']}
                                                    ...
                                                    logger.info(f"Rule 3 EN = block NOT found in preceeding rungs {blocks_rung-1}  for start_rung{start_rung-1} and end_rung:{end_rung-1}")
                        ...
                        if (check_1_NG_flag==0):
                            ...
                            sub_dict={'Result':["NG"], 'Task':[program], 'Section': ['AutoRun'], 
                                'RungNo':[""], "Target":[detail_dict], 
                                'CheckItem':rule_3_check_item,
                                'Detail': [check_1_NG_comment],  'Status': ['']}

                            sub_df=pd.DataFrame(sub_dict)
                        
                            output_df=pd.concat([output_df, sub_df], ignore_index=True)
                            
                            logger.info(f"Rule 3 NG of nature: QRReadStart Found but not OK/NORMAL found in start_rung:{start_rung} end_rung:{end_rung}")
