...
check_detail_content = {"cc1":"When ③ is not found in the detection target in ①, it is set to NG.", 
                        "cc2":"Confirm that there is an A contact whose variable comments include ”搬送(transport) orリセット(reset)" and  "タイミング(timing)” in the out-coil condition detected in ③."}

ng_content = {"cc1":"加工もしくは検査工程であるのに、動作完了記憶がない(No memory of operation completion even though it is a processing or inspection process.)", 
...
def  check_operation_complete_memory_outcoil_from_program(row, program_name:str,  operation_comment:str, complete_comment:str, memory_comment:str, program_comment_data:dict):
    try:
        ...
        if isinstance(attr, dict) and 'operand' in attr and 'latch' in attr:
            if attr.get('latch') == 'reset':
                ...
                if isinstance(comment, list):
                    if regex_pattern_check(operation_comment, comment) and regex_pattern_check(complete_comment, comment) and regex_pattern_check(memory_comment, comment):
...
def check_operation_complete_memory_outcoil_from_function(row, function_name:str, operation_comment:str, complete_comment:str, memory_comment:str, function_comment_data:dict):
    try:
        attr = ast.literal_eval(row['ATTRIBUTES'])
        if isinstance(attr, dict) and 'operand' in attr and 'latch' in attr:
            if attr.get('latch') == 'reset':
...
def detection_range_programwise(memory_feeding_rung_groups_df: pd.DataFrame, operation_comment:str, complete_comment:str, memory_comment:str, program_name:str, program_comment_data:str, section_name:str) -> dict:
    
    logger.info(f"Executing detection range on program {program_name} and section name {section_name} on rule 20")
    ...
    for _, rung_df in memory_feeding_rung_groups_df:
        ...
        rung_df['operation_complete_memory_reset_check_outcoil'] = rung_df.apply(
            lambda row: check_operation_complete_memory_outcoil_from_program(row=row, program_name=program_name, operation_comment=operation_comment, complete_comment=complete_comment, memory_comment=memory_comment, program_comment_data=program_comment_data) if row['OBJECT_TYPE_LIST'] == 'Coil' else None,
            axis=1
        )

        operation_complete_memory_match_outcoil = rung_df[rung_df['operation_complete_memory_reset_check_outcoil'].notna()]
        if not operation_complete_memory_match_outcoil.empty:
            operation_complete_memory_comment_operand = operation_complete_memory_match_outcoil.iloc[0]['operation_complete_memory_reset_check_outcoil']
            operation_complete_memory_comment_rung_number = operation_complete_memory_match_outcoil.iloc[0]['RUNG'] 

        if operation_complete_memory_comment_operand:
            return {
                        "operation_complete_memory":[operation_complete_memory_comment_operand, operation_complete_memory_comment_rung_number], 
                    }
...
def check_detail_1_programwise(detection_result:dict) -> dict:
    ...
    cc1_result['status'] = "NG" if detection_result['operation_complete_memory'][1] == -1 else "OK"
...
def check_detail_2_programwise(program_name:str, program_comment_data:str,  memory_feeding_section_df: pd.DataFrame, transport_comment:str, reset_full_comment:str, timing_comment:str, reset_operation_complete_memory_operand:str,   reset_operation_complete_memory_rung_number:str):
    if reset_operation_complete_memory_rung_number != -1:

        transort_reset_timing_contact_operand = ""

        reset_operation_complete_memory_rung_df = memory_feeding_section_df[memory_feeding_section_df['RUNG']==reset_operation_complete_memory_rung_number]
        for _ , rung_row in reset_operation_complete_memory_rung_df.iterrows():
            ...
            if isinstance(contact_comment, list) and contact_comment:
                if rung_row['OBJECT_TYPE_LIST'] == "Contact" and attr.get('negated')=='false' and (regex_pattern_check(transport_comment, contact_comment) or regex_pattern_check(reset_full_comment, contact_comment)) and regex_pattern_check(timing_comment, contact_comment):
...
def detection_range_functionwise(memory_feeding_rung_groups_df: pd.DataFrame, operation_comment:str, complete_comment:str, memory_comment:str, function_name:str, function_comment_data:str, section_name:str) -> dict:
    
    logger.info(f"Executing detection range on function {function_name} and section name {section_name} on rule 20")

    operation_complete_memory_comment_operand = False
    operation_complete_memory_comment_rung_number = -1

    for _, rung_df in memory_feeding_rung_groups_df:

        rung_df['operation_complete_memory_reset_check_outcoil'] = rung_df.apply(
            lambda row: check_operation_complete_memory_outcoil_from_function(row=row, function_name=function_name, operation_comment=operation_comment, complete_comment=complete_comment, memory_comment=memory_comment, function_comment_data=function_comment_data) if row['OBJECT_TYPE_LIST'] == 'Coil' else None,
            axis=1
        )
...
def check_detail_2_functionwise(function_name:str, function_comment_data:str,  memory_feeding_section_df: pd.DataFrame, transport_comment:str, reset_full_comment:str, timing_comment:str, reset_operation_complete_memory_operand:str,   reset_operation_complete_memory_rung_number:str):
    if reset_operation_complete_memory_rung_number != -1:

        transort_reset_timing_contact_operand = None

        reset_operation_complete_memory_rung_df = memory_feeding_section_df[memory_feeding_section_df['RUNG']==reset_operation_complete_memory_rung_number]
        for _ , rung_row in reset_operation_complete_memory_rung_df.iterrows():
            attr = ast.literal_eval(rung_row['ATTRIBUTES'])
            contact_operand = attr.get('operand')
            rising_edge = attr.get('edge', 'False')
            if contact_operand and isinstance(contact_operand, str):
            ...
            if isinstance(contact_comment, list) and contact_comment:
                if rung_row['OBJECT_TYPE_LIST'] == "Contact" and attr.get('negated')=='false' and rising_edge=='rising' and (regex_pattern_check(transport_comment, contact_comment) or regex_pattern_check(reset_full_comment, contact_comment)) and regex_pattern_check(timing_comment, contact_comment):
                    transort_reset_timing_contact_operand = contact_operand
                    break
...
def execute_rule_20_programwise(input_program_file:str, 
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            if program.lower() in task_names:
                ...
                reset_operation_complete_memory_operand = detection_result['operation_complete_memory'][0]
                reset_operation_complete_memory_rung_number = detection_result['operation_complete_memory'][1]
...
def execute_rule_20_functionwise(input_function_file:str, 
    ...
    try:
        ...
        task_names = input_image_function_df[
                input_image_function_df["Process type"].astype(str).str.lower().isin(["processing", "inspection"])
            ]["Task name"].astype(str).str.lower().tolist()
...
#     input_program_file = "C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Data_Modelling/version3/data_model_Rule_20_NG/data_model_Rule_20_NG_programwise.csv"
#     input_program_comment_file = "C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Data_Modelling/version3/data_model_Rule_20_NG/data_model_Rule_20_NG_programwise.json"
#     input_function_file = "C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Data_Modelling/version3/data_model_Rule_20_NG/data_model_Rule_20_NG_functionwise.csv"
#     input_function_comment_file = "C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Data_Modelling/version3/data_model_Rule_20_NG/data_model_Rule_20_NG_functionwise.json"
#     output_folder_path = 'C:/Users/aniln/OneDrive - OPTIMIZED SOLUTIONS LTD/DENSO/Rules_implementation/pythoncode/output_csv/'
#     program_output_file = 'Rule_20_NG_program.csv'
#     function_output_file = 'Rule_20_NG_function.csv'

#     final_csv = execute_rule_20_programwise(input_program_file=input_program_file, input_program_comment_file=input_program_comment_file)
#     final_csv.to_csv(f"{output_folder_path}/{program_output_file}", index=False, encoding='utf-8-sig')

#     final_csv = execute_rule_20_functionwise(input_function_file=input_function_file, input_function_comment_file=input_function_comment_file)
