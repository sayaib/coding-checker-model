...
ng_content =  {"cc1":"各タスクの起動条件にサイクル停止のB接点が存在しないためNG",
...
def detection_range_programwise(condition_section_df:pd.DataFrame, program:str, program_comment_data:dict, cycle_comment:str, stop_comment:str):
            ...
            for _, contact_row in contact_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(cycle_comment, contact_comment) and regex_pattern_check(stop_comment, contact_comment) and negated_operand=='true':
...
def check_detail_2_programwise(condition_section_df:pd.DataFrame, detection_result:list, program:str, program_comment_data:dict, start_comment:str, condition_comment:str, auxiliary_comment:str):
    status = "NG"
    rung_number = -1
    target_coil = ""
    ...
    for detected_contact in detection_result:
        ...
        if not condition_section_df.empty:
            coil_current_rung_df = condition_section_df[(condition_section_df['OBJECT_TYPE_LIST'].str.lower() == 'coil') & (condition_section_df['RUNG'] == contact_rung)]
            current_rung_df = condition_section_df[condition_section_df['RUNG'] == contact_rung]
                ...
                for _, coil_row in coil_current_rung_df.iterrows():
                    ...
                    if isinstance(coil_operand, str) and coil_operand:
                        ...
                        if isinstance(coil_comment, list) and coil_comment:
                            if regex_pattern_check(start_comment, coil_comment) and regex_pattern_check(condition_comment, coil_comment) and not regex_pattern_check(auxiliary_comment, coil_comment):
...
def detection_range_functionwise(condition_section_df:pd.DataFrame, function:str, function_comment_data:dict, cycle_comment:str, stop_comment:str):
...
def check_detail_1_functionwise(detection_result:dict, function:str):
...
def check_detail_2_functionwise(condition_section_df:pd.DataFrame, detection_result:list, function:str, function_comment_data:dict, start_comment:str, condition_comment:str, auxiliary_comment:str):
    status = "NG"
    rung_number = -1
    target_coil = ""
...
def execute_rule_80_programwise(input_program_file:str, 
...
def execute_rule_80_functionwise(input_function_file:str, 
    ...
    try:
        ...
        for function in unique_function_values:
            ...
            logger.info(f"Executing Rule 34 in function {function}")

            is_task_name_in_range = is_allowed_task(function)

            if is_task_name_in_range:
                current_function_df = function_df[function_df['FUNCTION_BLOCK'] == function]
                condition_section_df = current_function_df[current_function_df['BODY_TYPE'].str.lower() == condition_section_name]

                detection_result = detection_range_functionwise(condition_section_df=condition_section_df,
                                            function=function,
                ...
                cc2_result = check_detail_2_functionwise(condition_section_df=condition_section_df,
                                                        detection_result=detection_result, 
                                                        function=function, 
                                                        function_comment_data=function_comment_data,
                                                        start_comment=start_comment,
