...
check_detail_content = {
    "cc1": "Check for the existence of function block instructions with “ZDS” in the name. If it does not exist in the range, it is NG.",
...
ng_content = {
    "cc1": " ”preparation”または”AutoRun”セクション内にZDSが使用されていないため,流動制御が成り立っていない可能性有",
...
def check_detail_1_programwise(
    autorun_preparation_section_df: pd.DataFrame, zds_keyword: str, body_type_key: str
):
    # pass
    status = "NG"
    zds_rung_number = -1
    ...
    block_df = autorun_preparation_section_df[
        autorun_preparation_section_df["OBJECT_TYPE_LIST"].str.lower() == "block"
    ]
...
def check_detail_2_programwise(
    autorun_preparation_section_df: pd.DataFrame, zfc_keyword: str, body_type_key: str
):
    status = "NG"
    zfc_rung_number = -1
    section_name = ""
        ...
        for program in unique_program_values:
            if program.lower() in task_names:
                autorun_preparation_section_df = program_df[
                    program_df[body_type_key]
                    .str.lower()
                    .isin(
                        [
                            autorun_section_name,
                            autorun_section_with_star_name,
                ...
                cc1_results = check_detail_1_programwise(
                    ...
                    zds_keyword="FlowControlDataJudge_ZDS",
                    body_type_key=body_type_key,
                )
                cc2_results = check_detail_2_programwise(
                    autorun_preparation_section_df=autorun_preparation_section_df,
                    zfc_keyword="FlowControlDataWrite_ZFC",
                    body_type_key=body_type_key,
                )
                ...
                for index, cc_status in enumerate(all_cc_status):
                    ...
                    rung_number = (
                        cc_status.get("rung_number") - 1
                        if cc_status.get("rung_number") != -1
                        else -1
                    )
