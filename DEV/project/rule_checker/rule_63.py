...
rule_content_63 = "・Abnormalities such as cycle stop or abnormal stop have not occurred."
...
def detection_range_programwise(hmi_out_section_df: pd.DataFrame, 
                                program_name:str, 
                                ...
                                fault_comment:str,
                                not_comment:str) -> dict:
    
    logger.info(f"Executing rule 63 detection range on program {program_name} and section name {hmi_out_section_name} on rule 63")
    
    # Filter only 'Contact' rows
    coil_df = hmi_out_section_df[hmi_out_section_df['OBJECT_TYPE_LIST'].str.lower() == 'coil'].copy()
        ...
        for _, coil_row in coil_df.iterrows():
            ...
            if isinstance(coil_operand, str) and coil_operand:
                ...
                if isinstance(coil_comment, list) and coil_comment:
                    if regex_pattern_check(auto_comment, coil_comment) and regex_pattern_check(start_comment, coil_comment) and regex_pattern_check(condition_comment, coil_comment) and regex_pattern_check(fault_comment, coil_comment) and  regex_pattern_check(not_comment, coil_comment):
...
def check_detail_2_programwise(hmi_out_section_df:pd.DataFrame,
    ...
    if detection_result:
        rung_number = detection_result.get('coil_data')[1]
        current_rung_number_df = hmi_out_section_df[hmi_out_section_df['RUNG'] == rung_number]
            ...
            for _, contact_row in contact_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(abnormal_comment, contact_comment) and regex_pattern_check(not_comment, contact_comment) and negated_operand == 'false':
                            status = "OK"
...
# ===============================================================================================
def detection_range_functionwise(hmi_out_section_df: pd.DataFrame, 
                                function_name:str, 
    ...
    logger.info(f"Executing rule 63 detection range on function {function_name} and section name {hmi_out_section_name} on rule 63")
...
def check_detail_1_functionwise(detection_result:dict, 
    ...
    logger.info(f"Executing rule no 63 check detail 1 in function {function_name}")
...
def check_detail_2_functionwise(hmi_out_section_df:pd.DataFrame,
                               detection_result:dict, 
                               function_name:str,
                               function_comment_data:dict,
                               abnormal_comment:str,
        ...
        if len(contact_df) == 1:
            for _, contact_row in contact_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(abnormal_comment, contact_comment) and regex_pattern_check(not_comment, contact_comment):
                            status = "OK"
                            operand = contact_operand
                            break
...
def execute_rule_63_programwise(input_program_file:str, 
    ...
    try:
        ...
        for program_name in unique_program_values:
            ...
            if "main" in program_name.lower():
                ...
                hmi_out_section_df = current_program_df[current_program_df['BODY'].str.lower() == hmi_out_section_name]
                ...
                cc2_result = check_detail_2_programwise(hmi_out_section_df=hmi_out_section_df,
...
def execute_rule_63_functionwise(input_function_file:str, 
    ...
    try:
        ...
        for function_name in unique_function_values:
            ...
            logger.info(f"Executing rule 63 in function {function_name}")

            if "main" in function_name.lower():

                current_function_df = function_df[function_df['FUNCTION_BLOCK']==function_name]
                hmi_out_section_df = current_function_df[current_function_df['BODY_TYPE'].str.lower() == hmi_out_section_name]
                ...
                cc2_result = check_detail_2_functionwise(hmi_out_section_df=hmi_out_section_df,
                                                        detection_result=detection_result, 
                                                        function_name=function_name, 
                                                        function_comment_data=function_comment_data,
                                                        abnormal_comment=abnormal_comment,
