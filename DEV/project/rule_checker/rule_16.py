...
check_detail_content = {
    "cc1": "Check that the outcoil conditions detected in ③.1 satisfy all of the following.",
    "cc2": "Check that the B contact of the out-coil detected in ③,1 exists in the condition of the out-coil detected in ③.2.",
}

ng_content = {
    "cc1": "判定回路において、正しいOK判定(OK信号のONとNG信号のOFFのAND回路)になっていないためNG",
...
def check_two_outcoil_exist_ok_ng_programwise(
    current_rung_df: pd.DataFrame, program_name: str, program_comment_data: dict
):
        ...
        for _, coil_row in coil_df.iterrows():
            ...
            if isinstance(coil_operand, str) and coil_operand:
                ...
                if isinstance(coil_comment, list) and coil_comment:
                    if regex_pattern_check("OK", coil_comment) or regex_pattern_check(
                        "正常", coil_comment
                    ):
                        ok_status = True
                        ok_operand = coil_operand

                    if regex_pattern_check("NG", coil_comment) or regex_pattern_check(
                        "異常", coil_comment
                    ):
                        ng_status = True
...
def check_detail_1_programwise(
    ...
    for _, contact_row in contact_df.iterrows():
        ...
        if isinstance(contact_operand, str) and contact_operand:
            ...
            if isinstance(contact_comment, list) and contact_comment:
                if (
                    regex_pattern_check("OK", contact_comment)
                    or regex_pattern_check("正常", contact_comment)
                    ...
                    regex_pattern_check("NG", contact_comment)
                    or regex_pattern_check("異常", contact_comment)
                    ...
                    (
                        regex_pattern_check("異常", contact_comment)
                        and negated_operand == "true"
                    )
                    or (
                        regex_pattern_check("異常", contact_comment)
                        and regex_pattern_check("でない", contact_comment)
                        and negated_operand == "false"
                    )
                    and not check_1_3
                ):
    ...
    if check_1_1 and check_1_2 and check_1_3:
        ...
        """
        if len(all_match_operand) == 3 and all(
            item <= max_self for item in all_match_operand_outlist
        ):
            ...
            for series_contact in series_contact_operands_only:
                all_series_present = all(
                    item in series_contact for item in all_match_operand
                )
                if all_series_present:
                    status = "OK"
                    break

    return {"status": status, "check_number": "cc1"}


def check_detail_2_programwise(
    current_rung_df: pd.DataFrame, ok_operand: str, ng_operand: str
):
    pass
    ...
    for _, curr_row in current_rung_df.iterrows():
        ...
        if (
            contact_match_found
            and curr_row["OBJECT_TYPE_LIST"].lower() == "coil"
            and operand == ng_operand
        ):
            status = "OK"
            break
    ...
    return {"status": status, "check_number": "cc2"}
...
def check_two_outcoil_exist_ok_ng_functionwise(
    current_rung_df: pd.DataFrame, function_name: str, function_comment_data: dict
):
    coil_df = current_rung_df[current_rung_df["OBJECT_TYPE_LIST"].str.lower() == "coil"]
    ok_status = False
    ng_status = False
...
def check_detail_1_functionwise(
    current_rung_df: pd.DataFrame,
    function_name: str,
    ...
    for _, contact_row in contact_df.iterrows():
        ...
        if isinstance(contact_operand, str) and contact_operand:
            contact_comment = get_the_comment_from_function(
                contact_operand, function_name, function_comment_data
            )
...
def execute_rule_16_programwise(
    ...
    try:
        ...
        for program in unique_program_values:
            ...
            autorun_section_df = curr_program_df[
                ...
                .str.lower()
                .isin([autorun_section_name, autorun_section_with_star_name])
            ]
            ...
            if not autorun_section_df.empty:
                for curr_rung in unique_rung_number:
                    current_rung_df = autorun_section_df[
                        autorun_section_df["RUNG"] == curr_rung
                    ]
                    two_outcoil_exist_status, two_coil_details = (
                        check_two_outcoil_exist_ok_ng_programwise(
                    ...
                    if two_outcoil_exist_status:
                        ...
                        for index, cc_status in enumerate(all_cc_status):
                            ng_name = (
                                ng_content.get(cc_status.get("check_number", ""))
...
def execute_rule_16_functionwise(
    ...
    try:
        ...
        for function in unique_function_values:
            ...
            for curr_rung in unique_rung_number:
                ...
                two_outcoil_exist_status, two_coil_details = (
                    check_two_outcoil_exist_ok_ng_functionwise(
                        current_rung_df=current_rung_df,
