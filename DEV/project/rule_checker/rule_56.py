...
from .rule_56_ladder_utils import get_series_contacts_coil, get_parallel_contacts, get_format_parellel_contact_detail
...
check_detail_content = {"cc1":" If ① but not ③ or ④ or ⑤, it is assumed to be NG.", 
...
ng_content = {"cc1":"ｼｸﾞﾅﾙﾀﾜｰ点灯ｺｲﾙなしのためNG(NG due to no signal tower lighting coil)", 
...
def detection_range_programwise(fault_section_df: pd.DataFrame,  st_r_comment:str, st_y_comment:str, st_g_comment:str, program_name:str, program_comment_data:str) -> dict:
...
def check_detail_1_programwise(detection_results:dict) -> dict:
    ...
    status = "OK"
    if detection_results['detection_3_details'][1]==-1 or detection_results['detection_4_details'][1]==-1 or detection_results['detection_5_details'][1]==-1:
        status = "NG"
...
def check_detail_2_programwise(fault_section_df:pd.DataFrame, cc_detection_3_coil_operand:str, cc_detection_3_rung_number:int, not_comment:str, abnormal_comment:str, buzzer_full_width_comment:str,  program_name:str, program_comment_data:dict) -> dict:
    ...
    if cc_detection_3_rung_number >= 0 and cc_detection_3_coil_operand and not contact_fault_df.empty:
        for _, contact_row in contact_fault_df.iterrows():
            ...
            if contact_operand and isinstance(contact_operand, str):
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(buzzer_full_width_comment, contact_comment) and regex_pattern_check(abnormal_comment, contact_comment) and not buzzer_abnormal_found and negated_operand=='true':
        ...
        if not_abnormal_operand and buzzer_abnormal_operand:
            ...
            if operand_list_per_rung:
                for rung_operands in operand_list_per_rung:
                    if not_abnormal_operand in rung_operands and buzzer_abnormal_operand in rung_operands:
...
def check_detail_3_programwise(fault_section_df:pd.DataFrame, cc_detection_3_coil_operand:str, cc_detection_3_rung_number:int, battery_comment:str, replace_comment:str, program_name:str, program_comment_data:dict) -> dict:
    ...
    if cc_detection_3_rung_number >= 0 and not contact_fault_df.empty:
        for _, contact_row in contact_fault_df.iterrows():
            ...
            if contact_operand and isinstance(contact_operand, str):
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check("PLC", contact_comment) and regex_pattern_check(battery_comment, contact_comment) and regex_pattern_check(replace_comment, contact_comment) and negated_operand == 'false'  and not rising_edge == 'rising' and not rising_edge == 'falling':
                        return {
                            "cc" : "cc3",
                            "status" : "OK",
                            "check_number" : 3,
...
def check_detail_4_programwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, servo_comment:str ,battery_comment:str, replace_comment:str, program_name:str, program_comment_data:dict) -> dict:
    ...
    if cc_detection_3_rung_number >= 0 and not contact_fault_df.empty:
        for _, contact_row in contact_fault_df.iterrows():
            ...
            if contact_operand and isinstance(contact_operand, str):
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(servo_comment, contact_comment) and regex_pattern_check(battery_comment, contact_comment) and regex_pattern_check(replace_comment, contact_comment) and negated_operand == 'false' and not rising_edge == 'rising' and not rising_edge == 'falling':
                        return {
                            "cc" : "cc4",
                            "status" : "OK",
                            "check_number" : 4,
...
def check_detail_5_programwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, cc2_result:dict, cc3_result:dict, cc4_result:dict, program_name:str, program_comment_data:dict) -> dict:
...
def check_detail_6_programwise(fault_section_df:pd.DataFrame, cc_detection_4_coil_operand:str, cc_detection_4_rung_number:int, not_comment:str, warning_comment:str, buzzer_full_width_comment:str,  program_name:str, program_comment_data:dict) -> dict:
    ...
    if cc_detection_4_rung_number >= 0 and cc_detection_4_coil_operand and not contact_fault_df.empty:
        for _, contact_row in contact_fault_df.iterrows():
            attr = ast.literal_eval(contact_row['ATTRIBUTES'])
            contact_operand = attr.get('operand')
            negated_operand = attr.get('negated')
            ...
            if contact_operand and isinstance(contact_operand, str):
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(buzzer_full_width_comment, contact_comment) and regex_pattern_check(warning_comment, contact_comment) and not buzzer_warning_found and negated_operand=='true':
                        buzzer_warning_found = True
                        buzzer_warning_operand = contact_operand
        
        if not_warning_operand and buzzer_warning_operand:

            cc_detection_4_rung_number_details = fault_section_df[fault_section_df['RUNG']==cc_detection_4_rung_number]
            cc_detection_4_rung_number_details_polar = pl.from_pandas(cc_detection_4_rung_number_details)
            cc2_detection_3_get_series_data = get_series_contacts_coil(cc_detection_4_rung_number_details_polar)

            operand_list_per_rung = [[step['operand'] for step in rung] for rung in cc2_detection_3_get_series_data]
            if operand_list_per_rung:
                for rung_operands in operand_list_per_rung:
                    if not_warning_operand in rung_operands and buzzer_warning_operand in rung_operands:
                        return {
                            "cc" : "cc6",
                            "status" : "OK",
                            "check_number" : 6,
...
def check_detail_7_programwise(fault_section_df:pd.DataFrame, cc_detection_5_rung_number:int, cc_detection_3_coil_operand:str, cc_detection_4_coil_operand:str, autorun_comment:str, program_name:str, program_comment_data:dict) -> dict:

    logger.info(f"Executing rule no 56 check detail 7")

    if cc_detection_3_coil_operand and cc_detection_4_coil_operand and not fault_section_df.empty:
        ...
        if not detection_5_rung_fault_section_df.empty:
            for _, fault_section_row in detection_5_rung_fault_section_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(autorun_comment, contact_comment) and not autorun_comment_exist and negated_operand=='false':
                            autorun_comment_exist = True
                            autorun_operand_details.append(attr)

                    if contact_operand == cc_detection_3_coil_operand and not cc7_detection_3_operand_exist and negated_operand=='true':
                        cc7_detection_3_operand_exist = True
                        cc7_detection_3_operand_details.append(attr)

                    if contact_operand == cc_detection_4_coil_operand and not cc7_detection_4_operand_exist and negated_operand=='true':
                        cc7_detection_4_operand_exist = True
                        cc7_detection_4_operand_details.append(attr)

                    if cc7_detection_3_operand_exist and cc7_detection_4_operand_exist and autorun_comment_exist:
                        
                        """
                        get the first contact to used for finding parellel contact as given in rules
                        """
                        try:
                            first_contact_from_series = get_first_contact_from_series(autorun_operand_details, cc7_detection_3_operand_details, cc7_detection_4_operand_details)
                        except:
...
def check_detail_8_programwise(fault_section_df:pd.DataFrame, cc_detection_5_rung_number:int, master_on_comment:str, complete_comment:str, program_name:str, program_comment_data:dict) -> dict:
        ...
        if not detection_5_rung_fault_section_df.empty:
            for _, fault_section_row in detection_5_rung_fault_section_df.iterrows():
                ...
                if isinstance(contact_operand, str) and contact_operand:
                    ...
                    if isinstance(contact_comment, list) and contact_comment:
                        if regex_pattern_check(master_on_comment, contact_comment) and regex_pattern_check(complete_comment, contact_comment) and not master_on_operand_exist and negated_operand=='false':
                            master_on_operand_exist = True
                            master_on_operand_details.append(attr)

                        if 'ap' in contact_operand.lower() and not ap_operand_exist and negated_operand=='false':
                    ...
                    if master_on_operand_exist and ap_operand_exist:
                        ...
                        try:
                            first_contact_from_series = get_first_contact_from_series(master_on_operand_details, ap_operand_details)
                        except:
                            first_contact_from_series = ""
...
def check_detail_9_programwise(fault_section_df:pd.DataFrame, cc_detection_5_rung_number:int, cc7_result:str, cc8_result:str, program_name:str, program_comment_data:dict) -> dict:
    ...
    if cc7_result['status'] == "OK" and cc8_result['status'] == "OK":
        ...
        if cc7_first_operand and cc8_first_operand and not cc9_current_fault_df.empty:
            ...
            for _, current_row in cc9_current_fault_df.iterrows():
                ...
                if contact_operand and isinstance(contact_operand, str) and contact_operand == cc7_first_operand:
                    cc7_in_list.extend(attr['in_list'])
                
                if contact_operand and isinstance(contact_operand, str) and contact_operand == cc8_first_operand:
            ...
            for _, current_row in cc9_current_fault_df.iterrows():
                ...
                if contact_operand and isinstance(contact_operand, str) and contact_operand in [cc7_first_operand, cc8_first_operand]:
                    return {
                            "cc" : "cc5",
                            "status" : "NG",
                            "check_number" : 5,
...
def detection_range_functionwise(fault_section_df: pd.DataFrame,  st_r_comment:str, st_y_comment:str, st_g_comment:str, function_name:str, function_comment_data:str) -> dict:
    
    logger.info(f"Executing rule 56 detection range on function {function_name}")
...
def check_detail_2_functionwise(fault_section_df:pd.DataFrame, cc_detection_3_coil_operand:str, cc_detection_3_rung_number:int, not_comment:str, abnormal_comment:str, buzzer_full_width_comment:str,  function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_3_functionwise(fault_section_df:pd.DataFrame, cc_detection_3_coil_operand:str, cc_detection_3_rung_number:int, battery_comment:str, replace_comment:str, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_4_functionwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, servo_comment:str ,battery_comment:str, replace_comment:str, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_5_functionwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, cc2_result:dict, cc3_result:dict, cc4_result:dict, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_6_functionwise(fault_section_df:pd.DataFrame, cc_detection_4_coil_operand:str, cc_detection_4_rung_number:int, not_comment:str, warning_comment:str, buzzer_full_width_comment:str,  function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_7_functionwise(fault_section_df:pd.DataFrame, cc_detection_5_rung_number:int, cc_detection_3_coil_operand:str, cc_detection_4_coil_operand:str, autorun_comment:str, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_8_functionwise(fault_section_df:pd.DataFrame, cc_detection_5_rung_number:int, master_on_comment:str, complete_comment:str, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_9_functionwise(fault_section_df:pd.DataFrame, cc_detection_5_rung_number:int, cc7_result:str, cc8_result:str, function_name:str, function_comment_data:dict) -> dict:
...
def execute_rule_56_programwise(input_program_file:str, 
        ...
        for program_name in unique_program_values:
            ...
            if 'main' in program_name.lower():
                ...
                if not fault_section_df.empty:
                    ...
                    cc7_result = check_detail_7_programwise(fault_section_df=fault_section_df, 
...
def execute_rule_56_functionwise(input_function_file:str, 
        ...
        for function_name in unique_function_values:
            ...
            if 'main' in function_name.lower():
                ...
                if not fault_section_df.empty:
                    ...
                    cc7_result = check_detail_7_functionwise(fault_section_df=fault_section_df, 
                                                            cc_detection_5_rung_number=cc_detection_5_rung_number,
                                                            cc_detection_3_coil_operand=cc_detection_3_coil_operand,
                                                            cc_detection_4_coil_operand = cc_detection_4_coil_operand,
                                                            autorun_comment=autorun_comment,
...
#     program_output_file = 'Rule_56_programwise_NG_v2.csv'
#     function_output_file = 'Rule_56_functionwise_NG_v2.csv'

#     final_csv = execute_rule_56_programwise(input_program_file=input_program_file, input_program_comment_file=input_program_comment_file)
#     final_csv.to_csv(f"{output_folder_path}/{program_output_file}", index=False, encoding='utf-8-sig')

#     final_csv = execute_rule_56_functionwise(input_function_file=input_function_file, input_function_comment_file=input_function_comment_file)
