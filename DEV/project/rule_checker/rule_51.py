...
from .rule_51_ladder_utils import get_series_contacts_coil, check_self_holding, get_format_parellel_contact_detail, get_parallel_contacts
...
check_detail_content = {"cc1":" If ① but not ③ or ④ or ⑤ or ⑦, it is assumed to be NG.", 
...
def detection_range_programwise(fault_section_df: pd.DataFrame, deviceout_section_df:pd.DataFrame, BZ_variable_comment:str, buzzer_full_width_comment:str, abnormal_comment:str, warning_comment:str, program_name:str, program_comment_data:dict) -> dict:
    ...
    deviceout_coil_df = deviceout_section_df[deviceout_section_df['OBJECT_TYPE_LIST'] == 'Coil']


    detection_3_coil = detection_4_coil = detection_5_coil = detection_7_coil  = ""
    detection_3_rung_number = detection_4_rung_number = detection_5_rung_number = detection_7_rung_number  = -1
    ...
    if not fault_coil_df.empty:
        for _, fault_coil_row in fault_coil_df.iterrows():
            ...
            if fault_coil_operand and isinstance(fault_coil_operand, str):
                ...
                if BZ_variable_comment in fault_coil_operand and not fault_check_BZ_twice:
                    fault_check_BZ_twice = True
                    detection_3_coil = fault_coil_operand
                    detection_3_rung_number = fault_coil_row['RUNG']
                
            if isinstance(fault_coil_comment, list) and fault_coil_comment and not fault_check_buzzer_abnormal_twice:
                if regex_pattern_check(buzzer_full_width_comment, fault_coil_comment) and regex_pattern_check(abnormal_comment, fault_coil_comment):
                    fault_check_buzzer_abnormal_twice = True
                    detection_4_coil = fault_coil_operand
                    detection_4_rung_number = fault_coil_row['RUNG']
            
            if isinstance(fault_coil_comment, list) and fault_coil_comment and not fault_check_buzzer_warning_twice:
                if regex_pattern_check(buzzer_full_width_comment, fault_coil_comment) and regex_pattern_check(warning_comment, fault_coil_comment):
    ...
    if not deviceout_coil_df.empty:
        for _, deviceout_coil_row in deviceout_coil_df.iterrows():
            ...
            if isinstance(deviceout_coil_operand, str) and deviceout_coil_operand:
                deviceout_coil_comment = get_the_comment_from_program(deviceout_coil_operand, program_name, program_comment_data)

                if isinstance(deviceout_coil_comment, list) and deviceout_coil_comment:
                    if (regex_pattern_check(BZ_variable_comment, deviceout_coil_comment) or regex_pattern_check(buzzer_full_width_comment, fault_coil_comment)) and not check_buzzer_twice_deviceout:
...
def check_detail_1_programwise(detection_results:dict) -> dict:
    ...
    status = "OK"
    if detection_results['detection_3_details'][1]==-1 or detection_results['detection_4_details'][1]==-1 or detection_results['detection_5_details'][1]==-1 or detection_results['detection_7_details'][1]==-1:
        status = "NG"
...
def check_detail_2_programwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, cc_detection_4_coil_operand:str, cc_detection_5_coil_operand:str, not_comment:str, abnormal_comment:str, warning_comment:str, program_name:str, program_comment_data:dict) -> dict:
    ...
    if cc_detection_3_rung_number >= 0 and cc_detection_4_coil_operand and not contact_fault_df.empty:
        for _, contact_row in contact_fault_df.iterrows():
            ...
            if contact_operand and isinstance(contact_operand, str):
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(not_comment, contact_comment) and regex_pattern_check(abnormal_comment, contact_comment) and not not_abnormal_found and negated_operand == 'true':
                ...
                """
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(not_comment, contact_comment) and regex_pattern_check(warning_comment, contact_comment) and not not_warning_found:
                        not_warning_found = True
                        not_warning_operand = contact_operand
                        
                if contact_operand == cc_detection_4_coil_operand and not cc2_detection_4_coil_operand_exist and negated_operand == 'true':
        ...
        if not_abnormal_operand and cc2_detection_4_coil_operand_exist:
            ...
            """
            cc_detection_3_rung_number_details = fault_section_df[fault_section_df['RUNG']==cc_detection_3_rung_number]
            cc_detection_3_rung_number_details_polar = pl.from_pandas(cc_detection_3_rung_number_details)
            cc2_detection_3_get_series_data = get_series_contacts_coil(cc_detection_3_rung_number_details_polar)

            operand_list_per_rung = [[step['operand'] for step in rung] for rung in cc2_detection_3_get_series_data]

            if operand_list_per_rung:
                for rung_operands in operand_list_per_rung:
                    if not_abnormal_operand in rung_operands and cc_detection_4_coil_operand in rung_operands and not (not_warning_operand in rung_operands or cc_detection_5_coil_operand in rung_operands):
...
def check_detail_3_programwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, cc_detection_4_coil_operand:str,  cc_detection_5_coil_operand:str, not_comment:str, abnormal_comment:str, warning_comment:str, program_name:str, program_comment_data:dict) -> dict:
    ...
    if cc_detection_3_rung_number >= 0 and cc_detection_5_coil_operand and not contact_fault_df.empty:
        for _, contact_row in contact_fault_df.iterrows():
            attr = ast.literal_eval(contact_row['ATTRIBUTES'])
            contact_operand = attr.get('operand')
            negated_operand = attr.get('negated')
            if contact_operand and isinstance(contact_operand, str):
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(not_comment, contact_comment) and regex_pattern_check(warning_comment, contact_comment) and not not_warning_found  and negated_operand == 'true':
                        not_warning_found = True
                        not_warning_operand = contact_operand
                ...
                """
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(not_comment, contact_comment) and regex_pattern_check(abnormal_comment, contact_comment) and not not_abnormal_found:
                        not_abnormal_found = True
                        not_abnormal_operand = contact_operand
            
            if contact_operand == cc_detection_5_coil_operand and not cc3_detection_5_coil_operand_exist and negated_operand == 'true':
        ...
        if not_warning_operand and cc3_detection_5_coil_operand_exist:
            ...
            if operand_list_per_rung:
                for rung_operands in operand_list_per_rung:
                    if not_warning_operand in rung_operands and cc_detection_5_coil_operand in rung_operands and not (not_abnormal_operand in rung_operands or cc_detection_4_coil_operand in rung_operands):
...
def check_detail_4_programwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, battery_comment:str, replace_comment:str, program_name:str, program_comment_data:dict) -> dict:
    ...
    if cc_detection_3_rung_number >= 0 and not contact_fault_df.empty:
        for _, contact_row in contact_fault_df.iterrows():
            ...
            if contact_operand and isinstance(contact_operand, str):
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check("PLC", contact_comment) and regex_pattern_check(battery_comment, contact_comment) and regex_pattern_check(replace_comment, contact_comment)  and negated_operand == 'false':
                        return {
                            "cc" : "cc4",
                            "status" : "OK",
                            "check_number" : 4,
...
def check_detail_5_programwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, servo_comment:str, battery_comment:str, replace_comment:str, program_name:str, program_comment_data:dict) -> dict:
    ...
    if cc_detection_3_rung_number >= 0 and not contact_fault_df.empty:
        for _, contact_row in contact_fault_df.iterrows():
            ...
            if contact_operand and isinstance(contact_operand, str):
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    if regex_pattern_check(servo_comment, contact_comment) and regex_pattern_check(battery_comment, contact_comment) and regex_pattern_check(replace_comment, contact_comment)  and negated_operand == 'false':
                        return {
                            "cc" : "cc5",
                            "status" : "OK",
                            "check_number" : 5,
...
def check_detail_6_programwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, cc2_result:dict, cc3_result:dict, cc4_result:dict, cc5_result:str, program_name:str, program_comment_data:dict) -> dict:
    ...
    if (
        ...
        try:
            rung_df = fault_section_df[fault_section_df['RUNG'] == cc_detection_3_rung_number]
            parallel_contacts = get_parallel_contacts(pl.from_pandas(rung_df))
            formatted_parallel_contacts = get_format_parellel_contact_detail(parallel_contacts)
        except:
...
def check_detail_7_programwise(deviceout_section_df:pd.DataFrame, cc_detection_3_coil_operand:str, cc_detection_7_coil_operand:str) -> dict:

    logger.info(f"Executing rule no 51 check detail 7")

    if cc_detection_3_coil_operand and cc_detection_7_coil_operand and not deviceout_section_df.empty:
            ...
            if len(contact_coil_fault_df) == 2 and not contact_coil_fault_df.empty:
                for _, contact_coil_rung_row in contact_coil_fault_df.iterrows():
                    ...
                    if operand and isinstance(operand, str):
                        if operand_type == "Contact" and operand == cc_detection_3_coil_operand and negated_operand == 'false':
                            cc7_detection_3_operand_exist = True

                        if operand_type == "Coil" and operand == cc_detection_7_coil_operand:
...
def check_detail_8_programwise(fault_section_df:pd.DataFrame, cc_detection_4_coil_operand:str, cc_detection_4_rung_number:int, program_name:str, program_comment_data:dict) -> dict:
    
    logger.info(f"Executing rule no 51 check detail 8")

    if cc_detection_4_rung_number >=0 and cc_detection_4_coil_operand and not fault_section_df.empty:

        is_check_8_1 = False
        is_check_8_2 = False
        is_check_8_3 = False

        detection_4_coil_rung_df = fault_section_df[fault_section_df['RUNG']==cc_detection_4_rung_number]
        contact_fault_df = detection_4_coil_rung_df[detection_4_coil_rung_df['OBJECT_TYPE_LIST'] == 'Contact'].copy()
        ...
        for _, contact_fault_row in contact_fault_df.iterrows():
            ...
            if isinstance(contact_operand, str) and contact_operand:
                ...
                if isinstance(contact_comment, list) and contact_comment:
                    ...
                    if "pb" in contact_operand.lower() and "al" in contact_operand.lower() and ("rst" in contact_operand.lower() or "reset" in contact_operand.lower()) and negated_operand == 'false':
        ...
        for _, contact_fault_row in contact_fault_df.iterrows():
            ...
            if isinstance(contact_operand, str) and contact_operand:
                ...
                if contact_operand and isinstance(contact_comment, list) and contact_comment:
                    
                    if regex_pattern_check(not_comment, contact_comment) and regex_pattern_check(abnormal_comment, contact_comment) and negated_operand == 'true':
...
def check_detail_9_programwise(fault_section_df:pd.DataFrame, cc_detection_5_coil_operand:str, cc_detection_5_rung_number:int, program_name:str, program_comment_data:dict) -> dict:
    
    logger.info(f"Executing rule no 51 check detail 9")

    if cc_detection_5_rung_number >=0 and cc_detection_5_coil_operand and not fault_section_df.empty:

        is_check_9_1 = False
        is_check_9_2 = False
        is_check_9_3 = False

        detection_5_coil_rung_df = fault_section_df[fault_section_df['RUNG']==cc_detection_5_rung_number]
        contact_fault_df = detection_5_coil_rung_df[detection_5_coil_rung_df['OBJECT_TYPE_LIST'] == 'Contact'].copy()
        ...
        for _, contact_fault_row in contact_fault_df.iterrows():
            ...
            if isinstance(contact_operand, str) and contact_operand:
                contact_comment = get_the_comment_from_program(contact_operand, program_name, program_comment_data)
                if contact_operand and isinstance(contact_comment, list) and contact_comment:
                    if "pb" in contact_operand.lower() and "al" in contact_operand.lower() and ("rst" in contact_operand.lower() or "reset" in contact_comment.lower()) and negated_operand == 'false':
        ...
        for _, contact_fault_row in contact_fault_df.iterrows():
            ...
            contact_comment = get_the_comment_from_program(contact_operand, program_name, program_comment_data)
            if contact_operand and isinstance(contact_comment, list) and contact_comment:
                if regex_pattern_check(not_comment, contact_comment) and regex_pattern_check(warning_comment, contact_comment) and negated_operand == 'true':
...
def detection_range_functionwise(fault_section_df: pd.DataFrame, deviceout_section_df:pd.DataFrame, BZ_variable_comment:str, buzzer_full_width_comment:str, abnormal_comment:str, warning_comment:str, function_name:str, function_comment_data:dict) -> dict:
    
    logger.info(f"Executing rule 51 detection range on function {function_name}")
    ...
    if not deviceout_coil_df.empty:
        for _, deviceout_coil_row in deviceout_coil_df.iterrows():
            ...
            deviceout_coil_operand = attr.get('operand')
            if isinstance(deviceout_coil_operand, str) and deviceout_coil_operand:
                deviceout_coil_comment = get_the_comment_from_function(deviceout_coil_operand, function_name, function_comment_data)
...
def check_detail_1_functionwise(detection_results:dict) -> dict:
...
def check_detail_2_functionwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, cc_detection_4_coil_operand:str, cc_detection_5_coil_operand:str, not_comment:str, abnormal_comment:str, warning_comment:str, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_3_functionwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, cc_detection_4_coil_operand:str,  cc_detection_5_coil_operand:str, not_comment:str, abnormal_comment:str, warning_comment:str, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_4_functionwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, battery_comment:str, replace_comment:str, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_5_functionwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, servo_comment:str, battery_comment:str, replace_comment:str, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_6_functionwise(fault_section_df:pd.DataFrame, cc_detection_3_rung_number:int, cc2_result:dict, cc3_result:dict, cc4_result:dict, cc5_result:str, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_7_functionwise(deviceout_section_df:pd.DataFrame, cc_detection_3_coil_operand:str, cc_detection_7_coil_operand:str) -> dict:
...
def check_detail_8_functionwise(fault_section_df:pd.DataFrame, cc_detection_4_coil_operand:str, cc_detection_4_rung_number:int, function_name:str, function_comment_data:dict) -> dict:
...
def check_detail_9_functionwise(fault_section_df:pd.DataFrame, cc_detection_5_coil_operand:str, cc_detection_5_rung_number:int, function_name:str, function_comment_data:dict) -> dict:
...
def execute_rule_51_programwise(input_program_file:str, 
        ...
        for program_name in unique_program_values:
            ...
            if 'main' in program_name.lower():
                ...
                if not fault_section_df.empty:
                    ...
                    cc1_result = check_detail_1_programwise(detection_results=detection_results)

                    cc_detection_3_rung_number = detection_results['detection_3_details'][1]
                    cc_detection_4_rung_number = detection_results['detection_4_details'][1]
                    cc_detection_5_rung_number = detection_results['detection_5_details'][1]
                    cc_detection_4_coil_operand = detection_results['detection_4_details'][0]
                    cc_detection_5_coil_operand = detection_results['detection_5_details'][0]
                    cc_detection_3_coil_operand = detection_results['detection_3_details'][0]
                    cc_detection_7_coil_operand = detection_results['detection_7_details'][0]

                    cc2_result = check_detail_2_programwise(fault_section_df=fault_section_df, 
                    ...
                    cc3_result = check_detail_3_programwise(fault_section_df=fault_section_df, 
                                                            cc_detection_3_rung_number=cc_detection_3_rung_number, 
                                                            cc_detection_4_coil_operand=cc_detection_4_coil_operand, 
                                                            cc_detection_5_coil_operand=cc_detection_5_coil_operand, 
                                                            not_comment = not_comment,
                    ...
                    cc4_result = check_detail_4_programwise(fault_section_df=fault_section_df, 
                                                            cc_detection_3_rung_number=cc_detection_3_rung_number, 
                    ...
                    cc5_result = check_detail_5_programwise(fault_section_df=fault_section_df, 
                                                            cc_detection_3_rung_number=cc_detection_3_rung_number, 
                    ...
                    cc6_result = check_detail_6_programwise(fault_section_df=fault_section_df, 
                                                            cc_detection_3_rung_number=cc_detection_3_rung_number, 
                    ...
                    cc7_result = check_detail_7_programwise(deviceout_section_df=deviceout_section_df, 
                                                            cc_detection_3_coil_operand=cc_detection_3_coil_operand,
                                                            cc_detection_7_coil_operand = cc_detection_7_coil_operand)
                    
                    cc8_result = check_detail_8_programwise(fault_section_df=fault_section_df, 
                                                            cc_detection_4_coil_operand=cc_detection_4_coil_operand,
                    ...
                    cc9_result = check_detail_9_programwise(fault_section_df=fault_section_df, 
                                                            cc_detection_5_coil_operand=cc_detection_5_coil_operand,
                    ...
                    all_cc_result = [cc1_result, cc2_result, cc3_result, cc4_result, cc5_result, cc6_result, cc7_result, cc8_result, cc9_result]
...
def execute_rule_51_functionwise(input_function_file:str, 
    ...
    try:
        ...
        for function_name in unique_function_values:
            logger.info(f"Executing rule 51 in function {function_name}")
            if 'main' in function_name.lower():
                ...
                deviceout_section_df = current_function_df[current_function_df['BODY_TYPE'].str.lower()==deviceout_section_name]
                ...
                if not fault_section_df.empty:
                    ...
                    cc1_result = check_detail_1_functionwise(detection_results=detection_results)
                    ...
                    cc2_result = check_detail_2_functionwise(fault_section_df=fault_section_df, 
                                                            cc_detection_3_rung_number=cc_detection_3_rung_number, 
                                                            cc_detection_4_coil_operand=cc_detection_4_coil_operand, 
                                                            cc_detection_5_coil_operand=cc_detection_5_coil_operand,
                                                            not_comment = not_comment,
                    ...
                    cc3_result = check_detail_3_functionwise(fault_section_df=fault_section_df, 
                                                            cc_detection_3_rung_number=cc_detection_3_rung_number, 
                                                            cc_detection_4_coil_operand=cc_detection_4_coil_operand, 
                                                            cc_detection_5_coil_operand=cc_detection_5_coil_operand, 
                                                            not_comment = not_comment,
                    ...
                    cc4_result = check_detail_4_functionwise(fault_section_df=fault_section_df, 
                                                            cc_detection_3_rung_number=cc_detection_3_rung_number, 
                                                            battery_comment = battery_comment,
                                                            replace_comment=replace_comment,
                                                            function_name=function_name, 
                    ...
                    cc5_result = check_detail_5_functionwise(fault_section_df=fault_section_df, 
                                                            cc_detection_3_rung_number=cc_detection_3_rung_number, 
                                                            servo_comment = servo_comment,
                                                            battery_comment = battery_comment,
                                                            replace_comment=replace_comment,
                    ...
                    cc6_result = check_detail_6_functionwise(fault_section_df=fault_section_df, 
                                                            cc_detection_3_rung_number=cc_detection_3_rung_number, 
                                                            cc2_result = cc2_result,
                                                            cc3_result = cc3_result,
                                                            cc4_result=cc4_result,
                    ...
                    cc7_result = check_detail_7_functionwise(deviceout_section_df=deviceout_section_df, 
                                                            cc_detection_3_coil_operand=cc_detection_3_coil_operand,
                                                            cc_detection_7_coil_operand = cc_detection_7_coil_operand)
                    
                    cc8_result = check_detail_8_functionwise(fault_section_df=fault_section_df, 
                                                            cc_detection_4_coil_operand=cc_detection_4_coil_operand,
                                                            cc_detection_4_rung_number = cc_detection_4_rung_number,
                                                            function_name=function_name,
                                                            function_comment_data=function_comment_data)
                    ...
                    cc9_result = check_detail_9_functionwise(fault_section_df=fault_section_df, 
                                                            cc_detection_5_coil_operand=cc_detection_5_coil_operand,
                                                            cc_detection_5_rung_number = cc_detection_5_rung_number,
                                                            function_name=function_name,
                                                            function_comment_data=function_comment_data)
...
if __name__=='__main__':
    ...
    program_output_file = 'Rule_51_programwise_NG_v2.csv'
    function_output_file = 'Rule_51_functionwise_NG_v2.csv'

    final_csv = execute_rule_51_programwise(input_program_file=input_program_file, input_program_comment_file=input_program_comment_file)
    final_csv.to_csv(f"{output_folder_path}/{program_output_file}", index=False, encoding='utf-8-sig')

    final_csv = execute_rule_51_functionwise(input_function_file=input_function_file, input_function_comment_file=input_function_comment_file)
